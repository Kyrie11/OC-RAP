...
def test_near_launcher_uses_dataset_owned_replay_source_and_clean_wrapper_defaults_to_validation() -> None:
    text = (ROOT / "scripts/run_external_baselines_near.sh").read_text()
    wrapper = (ROOT / "scripts/run_external_baselines.sh").read_text()
    assert ': "${CL_WOMD:=auto}"' in text
    assert 'runtime_resolve_bucket_womd_spec "$CL_BUCKET_DATASET" "$CL_BUCKET_SPLIT" "$WOMD_ROOT"' in text
    assert 'WOMD_ROLE="${PRIMARY_WOMD_ROLE:-validation}"' in wrapper
    assert 'CL_WOMD_ROLE="$WOMD_ROLE"' in wrapper
    assert 'CALIB_WOMD_ROLE="$WOMD_ROLE"' in wrapper
