...
def test_v27_rescue_challenge_requires_deployability_axes():
    from ocrap.planning.selector import calibrated_constrained_select
    utility = np.array([1.0, 0.9])
    r_dep = np.array([0.8, 0.79])
    hard = np.zeros(2)
    harm = np.zeros(2)
    feasible = np.ones(2, dtype=bool)
    gap = np.array([0.10, 0.20])
    drs = np.array([0.92, 0.93])
    # Candidate 1 is a brake rescue certificate and nominal is admitted, but it
    # does not improve enough deployability axes, so v27 DDC preserves nominal.
    sel = calibrated_constrained_select(
        utility, r_dep, hard, harm, feasible,
        gamma_rec=0.0, gamma_H=0.0, gamma_D=0.0,
        pred_gap=gap, pred_drs=drs, lcb_beta=0.1,
        regime_name="contact",
        candidate_macro_names=["nominal", "brake"],
        pcd_rescue_certificate=True,
        pcd_rescue_macro_allowlist="brake",
        pcd_rescue_min_pred_pcd=0.10,
        pcd_rescue_min_pred_drs=0.70,
        pcd_rescue_min_pred_r_dep=-1.0,
        pcd_rescue_min_candidate_gap=0.0,
        pcd_rescue_max_candidate_gap=1.0,
        stress_rescue_challenge_nominal=True,
        rescue_challenge_min_pred_drs=0.70,
        rescue_challenge_min_pred_pcd=0.10,
        rescue_challenge_min_pcd_gain=0.02,
        rescue_challenge_min_rec_lcb_gain=0.02,
        rescue_challenge_min_gap_reduction=0.02,
        rescue_challenge_min_improvement_axes=2,
        prefer_admitted=True,
    )
    assert sel.selected_index == 0
...
def test_v27_rescue_challenge_selects_when_axes_improve():
    from ocrap.planning.selector import calibrated_constrained_select
    utility = np.array([1.0, 0.9])
    r_dep = np.array([0.3, 0.8])
    hard = np.zeros(2)
    harm = np.zeros(2)
    feasible = np.ones(2, dtype=bool)
    gap = np.array([0.30, 0.10])
    drs = np.array([0.80, 0.94])
    sel = calibrated_constrained_select(
        utility, r_dep, hard, harm, feasible,
        gamma_rec=0.0, gamma_H=0.0, gamma_D=0.0,
        pred_gap=gap, pred_drs=drs, lcb_beta=0.1,
    ...
    assert sel.selected_index == 1
    assert sel.reason == "best_pcd_rescue_guarded_challenge"
