...
def _feat22():
    ...
    f[...,19]=float(np.tanh(5.0))  # must be ignored by v48.74 trust selector
...
def test_v4873_runner_contract_freezes_forbidden_directions():
    p=Path(__file__).resolve().parents[1]/"scripts/run_v48_74_dcp_drfc_bcde_rifa_svbw_two_gpu.sh"
    assert p.is_file()
    text=p.read_text()
    assert "N73_ANCHORED_HULL" in text and "O73_Main_OCSVBW" in text
...
def test_v4873_runner_preregisters_required_attribution_order():
    text = (
        ...
        / "scripts/run_v48_74_dcp_drfc_bcde_rifa_svbw_two_gpu.sh"
    ).read_text()
    assert 'train_svbw_arm "$N_RUN" N73_ANCHORED_HULL false' in text
    assert 'train_svbw_arm "$O_RUN" O73_Main_OCSVBW true' in text
    assert "audit_v48_74_interaction_response.py" in text
    assert "compare_v48_74_svbw.py" in text
    assert "check_v48_74_pipeline_complete.py" in text
...
def test_v4873_response_audit_preserves_exact_zero_teacher_feasibility():
    ...
    spec = importlib.util.spec_from_file_location(
        "audit_v48_74_interaction_response",
        root / "tools/audit_v48_74_interaction_response.py",
    )
