"""observation-consistent signed-viability barrier witness.
...
experimental overlay and leaves baseline behavior bitwise unchanged when the
signed-viability environment switch is absent.
"""
...
SIGNED_VIABILITY_ENV = "OCRAP_V48_74_SIGNED_VIABILITY"
SIGNED_VIABILITY_SOURCE = "signed_finite_time_viability_projected_recovery_witness"
SIGNED_VIABILITY_ENGINEERING_VERSION = "v48.74.2-OC-SVBW-ENGFIX"
SIGNED_VIABILITY_SCHEMA = 10
SIGNED_VIABILITY_FEATURE_DIM = 22
...
def enabled() -> bool:
    value = os.getenv(SIGNED_VIABILITY_ENV, "").strip().lower()
...
def _find_feature_leaf(value: Any, path: tuple[Any, ...] = ()) -> tuple[tuple[Any, ...], Any] | None:
    if torch is not None and isinstance(value, torch.Tensor):
        if value.ndim >= 1 and value.shape[-1] == SIGNED_VIABILITY_FEATURE_DIM:
            return path, value
        return None
...
def apply_signed_viability_feature_overlay(result: Any, context: Mapping[str, Any]) -> Any:
...
def runtime_contract_fragment() -> dict[str, Any]:
    return {
        "enabled": enabled(),
        "engineering_version": SIGNED_VIABILITY_ENGINEERING_VERSION,
        "schema": SIGNED_VIABILITY_SCHEMA,
        "feature_dim": SIGNED_VIABILITY_FEATURE_DIM,
        "source": SIGNED_VIABILITY_SOURCE,
        "coordinates": {
