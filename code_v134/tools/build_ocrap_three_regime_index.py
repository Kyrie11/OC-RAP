...
def main() -> int:
    ...
    for r in ('safe','near','contact'):
        ...
        artifact_complete=bool(result and progress and progress.get('status')=='complete' and journal.is_file())
        if artifact_complete and result.get('bucket_target_count') not in (None,0):
            artifact_complete=int(result.get('num_scenes') or 0)==int(result['bucket_target_count'])
        # Artifact completeness is authoritative. A stale phase file can remain
        # ``failed`` when simulation completed but the parent launcher was killed
        # during final aggregation/index writing.
        complete=artifact_complete
        if not complete: failed.append(r)
        row={'phase':phase,'phase_effective_status':'complete_from_artifacts' if complete and phase.get('status')!='complete' else phase.get('status'),'result':str(p),'result_exists':p.is_file(),'progress':str(prog),'progress_status':progress.get('status') if progress else None,'scene_journal':str(journal),'scene_journal_exists':journal.is_file(),'num_scenes':result.get('num_scenes') if result else None,'bucket_target_count':result.get('bucket_target_count') if result else None,'run_fingerprint':result.get('run_fingerprint') if result else None,'complete':complete}
        regimes[r]=row
        if result:
            keys=('method','source','num_scenes','num_decisions','collision_scene_rate','offroad_scene_rate','closed_loop_bounded_NUP','intervention_rate','scene_min_clearance_m_p05','scene_ttc_s_p05','critical_ttc_exposure_duration_s','post_contact_terminal_clearance_m','post_contact_free_space_auc_normalized_m','post_contact_clearance_gain_m','post_contact_escape_scene_rate','recontact_scene_rate','secondary_overlap_identity_available_scene_rate','secondary_overlap_scene_rate','stable_stop_eligible_scene_rate','new_stable_stop_quality_conditional_scene_rate','post_contact_overlap_duration_s','timing')
            summary[r]={k:result.get(k) for k in keys}
    complete=not failed and args.launcher_exit_code==0
    doc={'event':'ocrap_three_regime_closed_loop_v50_3','schema_version':2,'root':str(args.root),'launcher_exit_code':args.launcher_exit_code,'status':'complete' if complete else 'failed_or_incomplete','complete':complete,'failed_or_incomplete_regimes':failed,'regimes':regimes}
