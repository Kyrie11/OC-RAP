...
AUDIT_MAX_LABELS="${AUDIT_MAX_LABELS:-0}"
AUDIT_AUTO_MAX_LABELS="${AUDIT_AUTO_MAX_LABELS:-256}"
AUDIT_INTERVENTION_ONLY="${AUDIT_INTERVENTION_ONLY:-false}"
AUDIT_CANDIDATE_SCOPE="${AUDIT_CANDIDATE_SCOPE:-ranked_topk}"
AUDIT_STORE_CANDIDATE_RECORDS="${AUDIT_STORE_CANDIDATE_RECORDS:-false}"
PRIVILEGED_PCD_ORACLE_CEILING="${PRIVILEGED_PCD_ORACLE_CEILING:-false}"
PRIVILEGED_PCD_ORACLE_EPSILON="${PRIVILEGED_PCD_ORACLE_EPSILON:-1e-6}"
PRIVILEGED_NONFLOOR_PCD_ORACLE_CEILING="${PRIVILEGED_NONFLOOR_PCD_ORACLE_CEILING:-false}"
PRIVILEGED_NONFLOOR_ONE_SHOT_SEED_REALIZATION="${PRIVILEGED_NONFLOOR_ONE_SHOT_SEED_REALIZATION:-false}"
PRIVILEGED_NONFLOOR_PCD_ORACLE_EPSILON="${PRIVILEGED_NONFLOOR_PCD_ORACLE_EPSILON:-1e-6}"
PRIVILEGED_NONFLOOR_RDEP_FLOOR="${PRIVILEGED_NONFLOOR_RDEP_FLOOR:-0.5}"
PRIVILEGED_NONFLOOR_RDEP_TOL="${PRIVILEGED_NONFLOOR_RDEP_TOL:-1e-8}"
PRIVILEGED_NONFLOOR_SEED_PLAN_FILE="${PRIVILEGED_NONFLOOR_SEED_PLAN_FILE:-}"
PRIVILEGED_NONFLOOR_REQUIRE_NOMINAL_BEFORE_START="${PRIVILEGED_NONFLOOR_REQUIRE_NOMINAL_BEFORE_START:-true}"
...
LATENCY_EXECUTION_CONTRACT="${LATENCY_EXECUTION_CONTRACT:-throughput_or_unspecified}"
...
if runtime_bool_true "$RENDER_TRACE"; then
...
# Keep historical publication behavior by default, but make the deployment
# gate explicit so Contact recovery can be validated on a held-out split before
# freezing a Contact-specific execution profile.
REQUIRE_ABSOLUTE_ADMISSION_FOR_INTERVENTION="${REQUIRE_ABSOLUTE_ADMISSION_FOR_INTERVENTION:-true}"
OCRAP_SELECTOR="${OCRAP_SELECTOR:-}"
...
ARGS=(
  ...
  --set "closed_loop.audit_max_labels=$AUDIT_MAX_LABELS"
  --set "closed_loop.audit_auto_max_labels=$AUDIT_AUTO_MAX_LABELS"
  --set "closed_loop.audit_intervention_only=$AUDIT_INTERVENTION_ONLY"
  --set "closed_loop.audit_candidate_scope=$AUDIT_CANDIDATE_SCOPE"
  --set "closed_loop.audit_store_candidate_records=$AUDIT_STORE_CANDIDATE_RECORDS"
  --set "closed_loop.privileged_pcd_oracle_ceiling=$PRIVILEGED_PCD_ORACLE_CEILING"
  --set "closed_loop.privileged_pcd_oracle_epsilon=$PRIVILEGED_PCD_ORACLE_EPSILON"
  --set "closed_loop.privileged_nonfloor_pcd_oracle_ceiling=$PRIVILEGED_NONFLOOR_PCD_ORACLE_CEILING"
  --set "closed_loop.privileged_nonfloor_one_shot_seed_realization=$PRIVILEGED_NONFLOOR_ONE_SHOT_SEED_REALIZATION"
  --set "closed_loop.privileged_nonfloor_pcd_oracle_epsilon=$PRIVILEGED_NONFLOOR_PCD_ORACLE_EPSILON"
  --set "closed_loop.privileged_nonfloor_rdep_floor=$PRIVILEGED_NONFLOOR_RDEP_FLOOR"
  --set "closed_loop.privileged_nonfloor_rdep_tol=$PRIVILEGED_NONFLOOR_RDEP_TOL"
  --set "closed_loop.privileged_nonfloor_seed_plan_file=$PRIVILEGED_NONFLOOR_SEED_PLAN_FILE"
  --set "closed_loop.privileged_nonfloor_require_nominal_before_start=$PRIVILEGED_NONFLOOR_REQUIRE_NOMINAL_BEFORE_START"
  --set closed_loop.fast_waymax_history=true
  --set "closed_loop.profile_timing=$PROFILE_TIMING"
  --set "closed_loop.latency_execution_contract=$LATENCY_EXECUTION_CONTRACT"
  --set "closed_loop.latency_warmup_decisions=$LATENCY_WARMUP_DECISIONS"
  ...
  --set "selection.require_absolute_admission_for_intervention=$REQUIRE_ABSOLUTE_ADMISSION_FOR_INTERVENTION"
...
[[ -n "$OCRAP_SELECTOR" ]] && ARGS+=(--set "selection.ocrap_selector=$OCRAP_SELECTOR")
