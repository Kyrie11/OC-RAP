...
def main()->int:
    ap=argparse.ArgumentParser(description='v48.62 OC-CWRF balanced/precision provenance isolation')
    ap.add_argument('--reference-run',type=Path,required=True); ap.add_argument('--reference-contract',type=Path,required=True)
    ap.add_argument('--occwrf-run',type=Path,required=True); ap.add_argument('--output',type=Path,required=True)
    ...
    for v in ('balanced','precision'):
        ...
        dst=a.occwrf_run/'candidates'/v/'model_v48_trac_sr'/'best.pt'
        summ=a.occwrf_run/'candidates'/v/'model_v48_trac_sr'/'train_summary.json'
        state=a.occwrf_run/'candidates'/v/'V48_62_STAGE_I_STATE_ISOLATION.json'
        pol=a.occwrf_run/'candidates'/v/'POLICY_CONTRACT.env'
        ...
        ok={
          ...
          'trainable_prefix_exact':trainable==['direct_absolute_common_witness_gain'],
          'stage_i_isolation':bool(st.get('valid')) and bool(st.get('stage_i_bitwise_identity')) and bool(st.get('only_common_witness_gain_added')),
          'feature_contract':bool(st.get('common_witness_feature_contract_valid')),
        ...
        variants[v]={'valid':valid,'checks':ok,'reference_checkpoint':resolved(ref),'reference_sha256':refsha,
                     'occwrf_checkpoint':resolved(dst),'trainable_param_prefixes':trainable,'duplicate_critical_keys':dupcrit}
        learned_paths.append(resolved(dst))
    distinct=len(learned_paths)==2 and len(set(learned_paths))==2
    if not distinct: errors.append('balanced/precision OC-CWRF checkpoint paths not distinct')
    valid=not errors and distinct and all(x.get('valid') for x in variants.values())
    doc={'schema':'ocrap-v48.62-occwrf-variant-isolation-v1','valid':valid,'reference_run':resolved(a.reference_run),
         'occwrf_run':resolved(a.occwrf_run),'variants':variants,'distinct_occwrf_checkpoint_paths':distinct,'errors':errors,'test_roots_read':False}
    a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n')
    print(json.dumps({'event':'v48_62_occwrf_variant_isolation','valid':valid,'output':str(a.output)})); return 0 if valid else 30
