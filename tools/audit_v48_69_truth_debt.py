...
def summ(rows):
 ...
 sp=[r for r in rows if safe(r)]; tf=[r for r in rows if (f(r.get('teacher_candidate_r_dep')) or -1)>=0]
...
def main():
 ap=argparse.ArgumentParser(description='v48.69 read-only teacher structural-floor debt audit');ap.add_argument('--run',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();doc={'schema':'ocrap-v48.69-truth-debt-audit-v1','test_roots_read':False,'dataset_reconstruction':False,'arms':{'D69_DTRW':{}}}
