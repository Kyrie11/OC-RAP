...
def main()->int:
 ...
 summ={'schema':'ocrap-v48.91-common-exogenous-future-physical-response-summary-v1','engineering_version':'v48.91.0-OC-CEPMI','valid':not errors,'attribution_ready':not errors,'errors':errors[:100],'rows':len(out),'roles':{role:_summ([r for r in out if r['dataset_role']==role]) for role in roles},'sidecar':str(args.sidecar.resolve()),'sidecar_sha256':_sha(args.sidecar),'sidecar_summary_sha256':_sha(args.sidecar_summary),'output':str(args.output.resolve()),'output_sha256':_sha(args.output),'planner_parameters_trained':0,'teacher_labels_changed':False,'teacher_metadata_input_to_model':False,'dataset_reconstruction':False,'dataset_reselection':False,'test_roots_read':False,'boundary_transport':False,'regime_conditioning':False}
