...
def main() -> int:
    ...
    for bucket in ("near", "contact"):
        c = load(args.base_run / "calibration" / f"direct_value_risk_{bucket}_v45.json")
        if not c:
            failures.append(f"{bucket}: missing v45 risk certificate")
            continue
    ...
    for bucket in ("near_contact", "contact"):
        v_path = args.run / f"audit_{bucket}_selected_topk_v45_v45.json"
        v = load(v_path)
        if not v:
            failures.append(f"{bucket}: missing v45 audit")
            continue
        ...
        if direct <= 0:
            failures.append(f"{bucket}: OC-RAVE direct path never entered final decisions")
        if total > direct:
        ...
        if args.mode != "execution":
            s_path = args.run / f"audit_{bucket}_selected_topk_v45_scalar.json"
            scalar = load(s_path)
            if not scalar:
    ...
    print(f"V45 OC-RAVE {args.mode.upper()} GATE")
    for k, v in rows:
        print(f"{k}: {v}")
    for x in warnings:
        print("WARNING:", x)
