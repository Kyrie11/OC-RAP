...
def barrier_names(arm: str):
    ...
    if arm in {"P66_OCACRW", "Q67_CTRLPROJ", "T_FIDELITY", "U_OCCUPANCY", "V_Main_OCRTRW"}: names.append("route")
    if arm in {"P66_OCACRW", "Q67_CTRLPROJ", "T_FIDELITY", "U_OCCUPANCY", "V_Main_OCRTRW"}: names.append("reentry_persistence")
    return names
...
def main():
    ap=argparse.ArgumentParser(description='v48.68 OC-PBRW feasibility-role and active-constraint witness audit')
    ...
    doc={
        'schema':'ocrap-v48.68-rtrw-feasibility-role-audit-v1','positive_gain':a.positive_gain,'arms':out,
    ...
    print(json.dumps({'event':'v48_68_rtrw_feasibility_role_audit','output':str(a.output)}))
