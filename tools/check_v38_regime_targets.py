...
def main() -> int:
    ap = argparse.ArgumentParser(description="Check OC-RAP v38 tail-budget regime targets from closed-loop/audit JSON outputs.")
    ...
    ap.add_argument("--near-pcd-min", type=float, default=0.548)
    ap.add_argument("--near-fra-max", type=float, default=0.12)
    ap.add_argument("--near-drs-min", type=float, default=0.88)
    ...
    root = args.run_dir
    files = {
        "safe": root / "closed_loop_safe_fast_v38.json",
        "near": root / "audit_near_contact_selected_topk_v38_v38.json",
        "contact": root / "audit_contact_selected_topk_v38_v38.json",
    }
    ...
    ok_all &= metric_row("near_PCD", near.get("closed_loop_post_contact_deployability"), ">=", args.near_pcd_min)
    ok_all &= metric_row("near_FRA", near.get("closed_loop_FRA_exec"), "<=", args.near_fra_max)
    ok_all &= metric_row("near_DRS", near.get("closed_loop_DRS"), ">=", args.near_drs_min)
    ...
    offline = root / "eval_contact_v38_v38.json"
    if offline.exists():
        d = load(offline)
        ...
        scalar_path = root / "eval_contact_v38_scalar.json"
