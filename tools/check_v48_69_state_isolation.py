...
import torch
ALLOWED_KEY='direct_absolute_semantic_witness_gain'; EXPECTED_FEATURE_SCHEMA=5; EXPECTED_FEATURE_SOURCE='demand_tempered_projected_recovery_witness'
def load(p):
...
def sha(p):
 ...
 return h.hexdigest()
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--reference',type=Path,required=True);ap.add_argument('--adapted',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args()
 ...
 feature_ok=schema==EXPECTED_FEATURE_SCHEMA and source==EXPECTED_FEATURE_SOURCE;expected_flags={'semantic_correction':True,'active_set_alignment':True,'path_stop_alignment':False,'classlocal_transport':False,'route_alignment':True,'reentry_alignment':True,'control_projection':True,'boundary_transport':False,'projection_fidelity':True,'demand_normalized_fidelity':True,'robust_occupancy':False};flags_ok=flags==expected_flags
