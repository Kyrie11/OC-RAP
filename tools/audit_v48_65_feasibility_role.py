...
def semantic_summary(rows):
 ...
 d={'rows':len(rows),'any_positive_common_option_fraction':frac(rows,lambda r:float(r.get('semantic_positive_option_count') or 0)>0),
 ...
 if any(r.get('semantic_classlocal_lcvar_viability') is not None for r in rows):
  d['classlocal_transport']={
    'lcvar_viability_mean':mean(rows,'semantic_classlocal_lcvar_viability'),
    'positive_certificate_fraction':frac(rows,lambda r:r.get('semantic_classlocal_lcvar_viability') is not None and float(r['semantic_classlocal_lcvar_viability'])>0),
    'viable_root_mass_mean':mean(rows,'semantic_classlocal_viable_root_mass'),
    'selected_support_mean':mean(rows,'semantic_classlocal_selected_support_mean'),
  }
 return d
...
def main():
 ...
 doc={'schema':'ocrap-v48.65-clrw-feasibility-role-audit-v1','positive_gain':a.positive_gain,'arms':out,'interpretation_contract':{'teacher_boundary':'candidate R_dep >= 0','stage_ii_threshold':0.5,'safe_positive':'teacher_adv >= positive_gain AND not teacher_harmful','no_threshold_search':True,'test_roots_read':False,'semantic_diagnostics':'diagnostic only; class-local diagnostics expose correction geometry but are not extra training targets or thresholds','limiting_constraint_order':list(LIMITING)}}
 a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_65_clrw_feasibility_role_audit','output':str(a.output)}))
