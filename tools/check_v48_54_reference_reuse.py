...
def main()->int:
 ap=argparse.ArgumentParser(description='Semantic fail-closed reuse check for v48.54 A reference.')
 ...
 for f in ['AUTHORITATIVE_RUN_STATUS.json','SOURCE_CHECKPOINT_CONTRACT.json','GATE_SPEC.json','V48_53_FACTOR_CONTRACT.json']:
  if not (r/f).is_file(): e.append('missing_reference:'+f)
 if e:
  doc={'event':'v48_54_reference_reuse_contract','valid':False,'errors':e}; a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(doc,indent=2)+'\n'); return 4
 status=load(r/'AUTHORITATIVE_RUN_STATUS.json'); source=load(r/'SOURCE_CHECKPOINT_CONTRACT.json'); gate=load(r/'GATE_SPEC.json'); factor=load(r/'V48_53_FACTOR_CONTRACT.json')
 ...
 # v48.53-A is exactly the q-hard BC-FC + smooth-NAP reference required by v48.54.
 ...
 # Frozen gate semantics must match the preregistered v48.36 protocol expected by v48.54.
