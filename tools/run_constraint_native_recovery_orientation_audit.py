...
from ocrap.audits.constraint_native_orientation import (
    ...
    action_features,
    base_features,
    candidate_agent_signed_clearance_paths,
    derive_candidate_semantics,
    feature_only_dataset_cfg,
    fit_closed_form_ridge,
    fit_constraint_native_scaler,
    matched_features,
    pair_indices_from_clearance,
    raw_agent_set,
...
def main()->int:
    ap=argparse.ArgumentParser();ap.add_argument('--checkpoint',type=Path,required=True);ap.add_argument('--train-index',type=Path,required=True);ap.add_argument('--dev-index',type=Path,required=True);ap.add_argument('--certificate-index',type=Path,required=True);ap.add_argument('--v93-audit',type=Path,required=True);ap.add_argument('--cache-dir',type=Path,required=True);ap.add_argument('--device',default='cuda');ap.add_argument('--variant',required=True);ap.add_argument('--run-id',required=True);ap.add_argument('--output',type=Path,required=True);ap.add_argument('--state-output',type=Path,required=True);a=ap.parse_args();t0=time.perf_counter()
    ...
    result={
      'schema':'ocrap-v48.111-constraint-native-recovery-orientation-audit-v1','engineering_version':ENGINEERING_VERSION,'scientific_version':'v48.111-OC-CNRO','run_instance_id':a.run_id,'algorithm_name':ALGORITHM_NAME,'valid':True,'variant':a.variant,'audit_only':True,'checkpoint':str(a.checkpoint.resolve()),'checkpoint_sha256':sha256(a.checkpoint),
    ...
    a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(result,indent=2,sort_keys=True)+'\n');torch.save({'schema':'ocrap-v48.111-constraint-native-recovery-orientation-state-v1','engineering_version':ENGINEERING_VERSION,'scientific_version':'v48.111-OC-CNRO','run_instance_id':a.run_id,'algorithm_name':ALGORITHM_NAME,'variant':a.variant,'support':_pack_axis(fam['support']),'reserve':_pack_axis(fam['reserve']),'convex_closed_form_ridge':True,'strictly_convex_unique_solution':True,'iterative_optimizer_used':False,'checkpoint_sha256':sha256(a.checkpoint)},a.state_output)
