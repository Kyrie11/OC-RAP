#!/usr/bin/env python3
"""Compare v48.53 DCP-DRFC-BCDE-CSE A/B/C/D on development/certificate data only.

The report intentionally does not read test roots.  It extracts the exact metrics
needed for the pre-registered Certificate Structural Equivalence causal readout and recomputes component-level
...
def _factor_contract(run: Path) -> dict[str, Any]:
    for name in ("V48_53_FACTOR_CONTRACT.json", "V48_52_FACTOR_CONTRACT.json"):
        p = run / name
        if p.is_file():
            d = _json(p)
            d = dict(d)
            d["_source_contract_file"] = name
            return d
    raise FileNotFoundError(f"missing v48.52/v48.53 factor contract in {run}")
...
def _attribution_contract(arms: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for name,arm in arms.items():
        f=arm.get("factor_contract") or {}; teacher_expected, student_expected=expected[name]
        teacher=bool(f.get("physical_teacher_sign_alignment", False))
        student=bool(f.get("physical_student_sign_alignment", f.get("native_physical_student_drs", False)))
        expected_student_coord=(
            "q_selected_predicted_margin_physical_drs_exact_pcd"
            if student_expected else "hard_qbest_ge_zero_root_mass_exact_pcd"
        )
        expected_teacher_coord=(
            "q_selected_mstar_physical_drs_exact_pcd"
            if teacher_expected else "q_hard_proxy_drs_exact_pcd"
        )
        common_ok=(
            ...
            and f.get("new_tuned_thresholds") is False
            and not bool(f.get("decision_weighted_observation"))
            and not bool(f.get("root_logit_recalibration"))
            and f.get("strategy_regime_conditioning") is False
            and f.get("test_roots_read") is False
        )
        if not common_ok or teacher != teacher_expected or student != student_expected:
            factor_errors.append(name)
        if f.get("teacher_sign_coordinate") != expected_teacher_coord:
            factor_errors.append(name)
        # v48.52 reference contracts explicitly publish the q-hard student coordinate.
        if f.get("student_sign_coordinate") != expected_student_coord:
            factor_errors.append(name)
        if student_expected and f.get("native_physical_student_drs") is not True:
            factor_errors.append(name)
        if name in {"C","D"} and f.get("version") != "v48.53-DCP-DRFC-BCDE-CSE":
            factor_errors.append(name)
    ...
    return {"valid":valid,"reference_arm":"A","missing_reference_fields":missing,"mismatched_arms":sorted(mismatches),
            ...
            "meaning":"A/B preserve the validated v48.52 teacher-side q-proxy/PSA contrast with student q-hard deployment. C-A isolates student/deployment physical certificate alignment. D-B-C+A tests whether teacher and student hard certificates require structural equivalence: q selects the observation-consistent option, margin sign certifies physical root success, predicted/teacher root mass aggregates DRS. Smooth q geometry remains the common local-order channel; no regime-conditioned policy is introduced."}
...
def main() -> int:
    ...
    report = {
        "schema": "v48.53-dcp-drfc-bcde-cse-2x2-comparison-v1",
