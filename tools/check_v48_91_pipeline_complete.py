...
def main():
 ...
 for k in ('runtime','sidecar','sidecar_summary','audit','audit_summary','comparison','v48_90_comparison'):ap.add_argument('--'+k.replace('_','-'),dest=k,type=Path,required=True)
 ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();errors=[]
 docs={}
 for k in ('runtime','sidecar_summary','audit_summary','comparison','v48_90_comparison'):
  p=getattr(a,k)
 ...
 artifacts={k:{'path':str(getattr(a,k).resolve()),'sha256':sha(getattr(a,k))} for k in ('runtime','sidecar','sidecar_summary','audit','audit_summary','comparison') if getattr(a,k).is_file()}
 out={'schema':'ocrap-v48.91-pipeline-complete-v1','engineering_version':'v48.91.1-OC-CEPMI-PROVENANCEFIX','valid':not errors,'attribution_ready':not errors,'errors':errors,'experiment_type':'audit_only_common_exogenous_future_physical_margin_identifiability','planner_parameters_trained':0,'dataset_reconstruction':False,'dataset_reselection':False,'teacher_labels_changed':False,'teacher_metadata_input_to_model':False,'boundary_transport':False,'relative_ranker_modified':False,'regime_conditioning':False,'test_roots_read':False,'artifacts':artifacts,'preregistered_status':(docs.get('comparison',{}).get('preregistered_decision') or {}).get('status')}
