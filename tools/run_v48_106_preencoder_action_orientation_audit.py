...
from ocrap.v48_106_preencoder_action_orientation_audit import (
    ALGORITHM_NAME,
    ENGINEERING_VERSION,
    ORIENTATION_GROUP_ORDER,
    mean_difference_direction,
    preencoder_action_features,
    signed_orientation_cosine,
)
...
def _preencoder_memory(enc: StructuredTokenEncoder, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if len(enc.encoder.layers) < 2:
        raise RuntimeError("V48.106 requires the historical two-layer Stage-I Transformer")
    inp = _input_tokens(enc, x)
    h = inp
    for layer in enc.encoder.layers[:-1]:
        h = layer(h)
    prelast = h
    final = enc.norm(enc.encoder.layers[-1](prelast))
    return inp, prelast, final
...
def feature_cache_key(checkpoint: Path, index_path: Path, role_filter: str | None, v93_path: Path | None) -> str:
    payload = {
        ...
        "kind": "pre_encoder_stage_i_v102_summary",
    }
...
def extract_preencoder_features(*, checkpoint: Path, index_path: Path, role_filter: str | None, v93_path: Path | None,
                                cache_dir: Path, device: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    seen: set[str] = set(); paths: list[Path] = []
    for p in needed:
        q = str(p.resolve())
        if q not in seen:
    ...
    if not isinstance(model.encoder, StructuredTokenEncoder):
        raise RuntimeError("V48.106 requires StructuredTokenEncoder")
    enc = model.encoder.eval(); dev = bundle.device
    if len(enc.encoder.layers) != 2:
        raise RuntimeError(f"V48.106 preregistered for historical two-layer Stage-I, got {len(enc.encoder.layers)}")
    ...
    if ds.absolute_truth_contract_event.get("enabled") or ds.action_response_truth_event.get("enabled"):
        raise RuntimeError("V48.106 feature-only dataset unexpectedly attached truth sidecars")
    idx = {str(p.resolve()): i for i, p in enumerate(ds.paths)}
    records: list[dict[str, Any]] = []
    memory_shape: tuple[int, int] | None = None
    max_final_reconstruction = 0.0
    with torch.no_grad():
        for g in groups:
            ...
            preencoder, _prelast, historical_final = _preencoder_memory(enc, x)
            direct = model._scene_tokens(x)
            max_final_reconstruction = max(max_final_reconstruction, float((historical_final - direct).abs().max().item()))
            shape = (int(preencoder.shape[1]), int(preencoder.shape[2]))
            if memory_shape is None: memory_shape = shape
            elif memory_shape != shape: raise RuntimeError("V48.106 pre-encoder Stage-I memory geometry drift")
            state, delta, context = preencoder_action_features(preencoder)
            ...
            for j, c in enumerate(g["candidates"]):
                records.append({
                    ...
                    "safe_positive": bool(c["safe_positive"]), "teacher_harmful": bool(c["teacher_harmful"]),
                    "mediation_mode": c["mediation_mode"], "state": snp[j], "delta": dnp[j], "context": cnp[j],
                })
    if max_final_reconstruction > 1.0e-6:
        raise RuntimeError(f"V48.106 historical final reconstruction mismatch {max_final_reconstruction}")
    ...
    event = {
        ...
        "records": len(records), "groups": len(groups),
        "preencoder_memory_shape": list(memory_shape or (0, 0)),
        "stage_i_summary_dim": int(records[0]["state"].shape[0]) if records else 0,
        "semantic_token_count": SEMANTIC_TOKEN_COUNT, "agent_token_summary": "mean_std_max_min",
        "encoder_layer_count": int(len(enc.encoder.layers)), "preencoder_before_layer_count": 0,
        "historical_final_reconstruction_max_abs": max_final_reconstruction, "d_model": d_model,
    }
...
def _fit_action_pair(records: list[dict[str, Any]], key: str, device: str, sl: slice | None = None):
    ...
    if sl is not None: perm = perm[:, sl]
...
def _eval_action_pair(records, key, true_probe, shuf_probe, device, sl: slice | None = None):
    if not records:
        ...
        return z, dict(z)
    X = np.stack([r[key] for r in records]); Xs = permute_within_group(records, key)
    if sl is not None: X, Xs = X[:, sl], Xs[:, sl]
    tm, tmu, tsd = true_probe; pm, pmu, psd = shuf_probe
...
def _linear_cka(X: np.ndarray, y: np.ndarray) -> float | None:
    ...
    if len(X) < 2 or len(np.unique(y)) < 2: return None
    X = X - X.mean(axis=0, keepdims=True); y = y - y.mean(axis=0, keepdims=True)
    K, L = X @ X.T, y @ y.T
    num = float((K * L).sum()); den = float(np.sqrt((K * K).sum() * (L * L).sum()))
    return None if den <= 0.0 else float(num / den)


def _orientation_reference(records: list[dict[str, Any]], key: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    X = np.stack([r[key] for r in records]).astype(np.float64, copy=False)
    y = np.asarray([r["label"] for r in records], dtype=np.int64)
    mu = X.mean(axis=0, keepdims=True); sd = X.std(axis=0, keepdims=True); sd = np.where(sd < 1.0e-6, 1.0, sd)
    Z = (X - mu) / sd
    return mean_difference_direction(Z, y), mu, sd


def _localization_metrics(records: list[dict[str, Any]], key: str, d_model: int,
                          train_reference: tuple[np.ndarray, np.ndarray, np.ndarray]) -> dict[str, Any]:
    if not records: return {}
    y = np.asarray([r["label"] for r in records], dtype=np.int64)
    X = np.stack([r[key] for r in records]).astype(np.float64, copy=False)
    Xs = permute_within_group(records, key).astype(np.float64, copy=False)
    total = np.square(X).sum(axis=1)
    train_dir, mu, sd = train_reference
    Z, Zs = (X - mu) / sd, (Xs - mu) / sd
    groups = summary_group_slices(d_model)
    slices = {**groups, "action_interaction": action_interaction_slice(d_model)}
    out: dict[str, Any] = {}
    for name in ORIENTATION_GROUP_ORDER:
        sl = slices[name]; G, Gs = X[:, sl], Xs[:, sl]
        e = np.square(G).sum(axis=1); share = np.divide(e, total, out=np.zeros_like(e), where=total > 1.0e-12)
        cka, ckas = _linear_cka(G, y), _linear_cka(Gs, y)
        held_dir = mean_difference_direction(Z[:, sl], y)
        shuf_dir = mean_difference_direction(Zs[:, sl], y)
        tc = signed_orientation_cosine(train_dir[sl], held_dir)
        sc = signed_orientation_cosine(train_dir[sl], shuf_dir)
        out[name] = {
            "dimension": int(sl.stop - sl.start),
            "mean_action_energy_share": float(share.mean()) if len(share) else None,
            "label_linear_cka": cka, "shuffled_linear_cka": ckas,
            "cka_minus_shuffled": None if cka is None or ckas is None else float(cka - ckas),
            "signed_orientation_cosine": tc,
            "shuffled_orientation_cosine": sc,
            "orientation_minus_shuffled": None if tc is None or sc is None else float(tc - sc),
            "signed_orientation_diagnostic_only": True,
        }
    return out
...
def main() -> int:
    ...
    tr, etr = extract_preencoder_features(checkpoint=a.checkpoint, index_path=a.train_index, role_filter=None, v93_path=None, cache_dir=a.cache_dir / "train", device=resolved)
    dv: list[dict[str, Any]] = []; events: dict[str, Any] = {"train": etr}
    for role in ("dev_near", "dev_contact"):
        rr, e = extract_preencoder_features(checkpoint=a.checkpoint, index_path=a.dev_index, role_filter=role, v93_path=a.v93_audit, cache_dir=a.cache_dir / role, device=resolved)
        dv.extend(rr); events[role] = e
    ce: list[dict[str, Any]] = []
    for role in ("certificate_near", "certificate_contact"):
        rr, e = extract_preencoder_features(checkpoint=a.checkpoint, index_path=a.certificate_index, role_filter=role, v93_path=a.v93_audit, cache_dir=a.cache_dir / role, device=resolved)
        ce.extend(rr); events[role] = e
    if not tr or not dv or not ce: raise RuntimeError("V48.106 empty pre-encoder audit feature set")
    d_model = int(etr.get("d_model", 0))
    if d_model <= 0 or int(etr.get("stage_i_summary_dim", -1)) != 15 * d_model: raise RuntimeError("V48.106 pre-encoder summary dimension contract")
    action_sl = action_interaction_slice(d_model)

    st_tr = state_records(tr); sup_tr = action_subset(tr, "drs_activation"); res_tr = action_subset(tr, "deployability_gain")
    sm, smu, ssd = fit_binary(np.stack([r["state"] for r in st_tr]), np.asarray([r["label"] for r in st_tr]), resolved, seed=102)
    sup_true, sup_shuf = _fit_action_pair(sup_tr, "delta", resolved); res_true, res_shuf = _fit_action_pair(res_tr, "context", resolved)
    ais_true, ais_shuf = _fit_action_pair(sup_tr, "delta", resolved, action_sl); air_true, air_shuf = _fit_action_pair(res_tr, "context", resolved, action_sl)
    sup_orientation_ref = _orientation_reference(sup_tr, "delta"); res_orientation_ref = _orientation_reference(res_tr, "context")

    cells: dict[str, Any] = {}; ai_cells: dict[str, Any] = {}; localization: dict[str, Any] = {}
    for role in ROLES:
        src = dv if role.startswith("dev_") else ce; rr = split_role(src, role)
        sr, ur, qr = state_records(rr), action_subset(rr, "drs_activation"), action_subset(rr, "deployability_gain")
        ...
        localization[role] = {
            "support": _localization_metrics(ur, "delta", d_model, sup_orientation_ref),
            "reserve": _localization_metrics(qr, "context", d_model, res_orientation_ref),
        }

    result = {
        "schema": "ocrap-v48.106-preencoder-action-orientation-audit-v1", "engineering_version": ENGINEERING_VERSION,
        "algorithm_name": ALGORITHM_NAME, "valid": True, "variant": a.variant, "audit_only": True,
        "checkpoint": str(a.checkpoint.resolve()), "checkpoint_sha256": sha256(a.checkpoint), "elapsed_seconds": time.perf_counter() - t0,
        "cells": cells, "action_interaction_cells": ai_cells, "token_localization": localization, "events": events,
        ...
        "relative_ranker_modified": False, "boundary_transport": False, "regime_conditioning": False,
        "teacher_metadata_input_to_model": False, "test_roots_read": False,
        "same_v48_102_summary_operator": True, "same_v48_102_linear_probe_recipe": True,
        "preencoder_only": True, "signed_orientation_diagnostic": True,
        "action_interaction_subspace": {"start": int(action_sl.start), "stop": int(action_sl.stop), "dimension": int(action_sl.stop - action_sl.start),
            "definition": "control_plus_scene_context_plus_agent_set_moments_excluding_cls_and_ego_history"},
    }
    a.output.parent.mkdir(parents=True, exist_ok=True); a.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    state_obj = {
        "schema": "ocrap-v48.106-peao-probe-state-v1", "engineering_version": ENGINEERING_VERSION, "algorithm_name": ALGORITHM_NAME,
        "variant": a.variant, "d_model": d_model,
        "global_state_probe": _probe_state_dict(sm, smu, ssd),
        "global_support_probe": _probe_state_dict(*sup_true), "global_support_shuffle_probe": _probe_state_dict(*sup_shuf),
        "global_reserve_probe": _probe_state_dict(*res_true), "global_reserve_shuffle_probe": _probe_state_dict(*res_shuf),
        "action_interaction_support_probe": _probe_state_dict(*ais_true), "action_interaction_support_shuffle_probe": _probe_state_dict(*ais_shuf),
        "action_interaction_reserve_probe": _probe_state_dict(*air_true), "action_interaction_reserve_shuffle_probe": _probe_state_dict(*air_shuf),
        "checkpoint_sha256": sha256(a.checkpoint), "train_counts": result["train_counts"],
    }
