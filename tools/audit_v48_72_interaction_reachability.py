...
def summarize(base,new):
 ...
 return out
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--j71',type=Path,required=True);ap.add_argument('--l72',type=Path,required=True);ap.add_argument('--m72',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();doc={'schema':'ocrap-v48.72-iorw-interaction-reachability-audit-v1','test_roots_read':False,'dataset_reconstruction':False,'comparisons':{}}
 for v in VARIANTS:
  doc['comparisons'][v]={}
  for k in KINDS:
   j=rows(a.j71,v,k);l=rows(a.l72,v,k);m=rows(a.m72,v,k)
   doc['comparisons'][v][k]={'L72_minus_J71':summarize(j,l),'M72_minus_J71':summarize(j,m),'M72_minus_L72':summarize(l,m)}
 a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_72_iorw_interaction_reachability_audit','output':str(a.output)}))
