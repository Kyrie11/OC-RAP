...
def main()->int:
    ap=argparse.ArgumentParser(description='v48.63 OC-QARW balanced/precision provenance isolation')
    ap.add_argument('--reference-run',type=Path,required=True); ap.add_argument('--reference-contract',type=Path,required=True)
    ap.add_argument('--ocqarw-run',type=Path,required=True); ap.add_argument('--output',type=Path,required=True)
    ...
    for v in ('balanced','precision'):
        ...
        dst=a.ocqarw_run/'candidates'/v/'model_v48_trac_sr'/'best.pt'
        summ=a.ocqarw_run/'candidates'/v/'model_v48_trac_sr'/'train_summary.json'
        state=a.ocqarw_run/'candidates'/v/'V48_63_STAGE_I_STATE_ISOLATION.json'
        pol=a.ocqarw_run/'candidates'/v/'POLICY_CONTRACT.env'
        ...
        ok={
          ...
          'trainable_prefix_exact':trainable==['direct_absolute_quantifier_witness_gain'],
          'stage_i_isolation':bool(st.get('valid')) and bool(st.get('stage_i_bitwise_identity')) and bool(st.get('only_quantifier_witness_gain_added')),
          'feature_contract':bool(st.get('quantifier_witness_feature_contract_valid')),
        ...
        variants[v]={'valid':valid,'checks':ok,'reference_checkpoint':resolved(ref),'reference_sha256':refsha,
                     'ocqarw_checkpoint':resolved(dst),'trainable_param_prefixes':trainable,'duplicate_critical_keys':dupcrit}
        learned_paths.append(resolved(dst))
    distinct=len(learned_paths)==2 and len(set(learned_paths))==2
    if not distinct: errors.append('balanced/precision OC-QARW checkpoint paths not distinct')
    valid=not errors and distinct and all(x.get('valid') for x in variants.values())
    doc={'schema':'ocrap-v48.63-ocqarw-variant-isolation-v1','valid':valid,'reference_run':resolved(a.reference_run),
         'ocqarw_run':resolved(a.ocqarw_run),'variants':variants,'distinct_ocqarw_checkpoint_paths':distinct,'errors':errors,'test_roots_read':False}
    a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n')
    print(json.dumps({'event':'v48_63_ocqarw_variant_isolation','valid':valid,'output':str(a.output)})); return 0 if valid else 30
