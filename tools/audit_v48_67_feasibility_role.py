...
def barrier_names(arm: str):
    ...
    if arm in {"P66_OCACRW", "Q_CTRLPROJ", "R_BOUNDARY", "S_Main_OCPBRW"}: names.append("route")
    if arm in {"P66_OCACRW", "Q_CTRLPROJ", "R_BOUNDARY", "S_Main_OCPBRW"}: names.append("reentry_persistence")
    return names
...
def main():
    ap=argparse.ArgumentParser(description='v48.67 OC-PBRW feasibility-role and active-constraint witness audit')
    ...
    doc={
        'schema':'ocrap-v48.67-ocpbrw-feasibility-role-audit-v1','positive_gain':a.positive_gain,'arms':out,
    ...
    print(json.dumps({'event':'v48_67_ocpbrw_feasibility_role_audit','output':str(a.output)}))
