...
def main() -> int:
    ap = argparse.ArgumentParser(description='Fail-closed v48.52 PSA witness contract check.')
    ap.add_argument('--run', type=Path, required=True, help='Variant run root, e.g. candidates/balanced')
    ap.add_argument('--expect-physical', type=_bool_text, required=True)
    ...
    expected = bool(args.expect_physical)
    checks = {
        ...
        'physical_teacher_sign_alignment': bool(stage.get('physical_teacher_sign_alignment', False)) is expected,
        ...
        'checkpoint_physical_teacher_sign_alignment': bool(training.get('recovery_frontier_physical_teacher_sign_alignment', False)) is expected,
    ...
    doc = {
        'event': 'v48_52_psa_contract',
        'version': 'v48.52-DCP-DRFC-BCDE-PSA',
        'run': str(args.run),
        'expect_physical_teacher_sign_alignment': expected,
        'checks': checks,
