...
EXPECTED_SCHEMA=8
EXPECTED_SOURCE='interaction_oriented_history_reachability_projected_recovery_witness'
...
def sha(p):
    ...
    return h.hexdigest()
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--reference',type=Path,required=True);ap.add_argument('--adapted',type=Path,required=True);ap.add_argument('--box',required=True);ap.add_argument('--hull',required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args()
