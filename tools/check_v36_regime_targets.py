...
def main() -> int:
    ap = argparse.ArgumentParser(description="Check OC-RAP v36 tail-budget regime targets from closed-loop/audit JSON outputs.")
    ...
    root = args.run_dir
    files = {
        "safe": root / "closed_loop_safe_fast_v36.json",
        "near": root / "audit_near_contact_selected_topk_v36_v36.json",
        "contact": root / "audit_contact_selected_topk_v36_v36.json",
    }
    ...
    offline = root / "eval_contact_v36_v36.json"
    if offline.exists():
        d = load(offline)
