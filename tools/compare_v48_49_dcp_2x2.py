#!/usr/bin/env python3
"""Compare v48.49 DCP-DRFC A/B/C/D on development/certificate data only.
...
def _v4849_witness(run: Path, variant: str) -> dict[str, Any]:
...
def _factor_contract(run: Path) -> dict[str, Any]:
    return _json(run / "V48_49_FACTOR_CONTRACT.json")
...
def _attribution_contract(arms: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for name,arm in arms.items():
        f=arm.get("factor_contract") or {}; margin_complete,native_adv=expected[name]
        if (f.get("arm")!=name or f.get("training_option_execution_semantics")!="observation_class"
            ...
            or bool(f.get("native_margin_complete_preservation"))!=margin_complete
            or bool(f.get("native_advantage_preservation"))!=native_adv
    ...
    return {"valid":valid,"reference_arm":"A","missing_reference_fields":missing,"mismatched_arms":sorted(mismatches),
            ...
            "meaning":"All arms share v48.48-D NCP+DRFC and paper-consistent observation-class execution. B-A = margin-complete native risk transport; C-A = native recovery-advantage preservation; D-B-C+A = decision-complete interface interaction, only when valid."}
...
def _arm(run: Path, positive_gain: float) -> dict[str, Any]:
    ...
    return {
        ...
        "witness_precision": _v4849_witness(run, "precision"),
        "witness_balanced": _v4849_witness(run, "balanced"),
...
def main() -> int:
    ...
    report = {
        "schema": "v48.49-dcp-drfc-2x2-comparison-v1",
