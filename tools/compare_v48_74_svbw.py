...
def main():
 ap=argparse.ArgumentParser(description='v48.74 OC-SVBW current-state-anchored signed-viability reachability attribution')
 ...
 for v in VARIANTS:
  deltas[v]={}
  for k in KINDS:
   vals={arm:cell(a,arm,v,k,'absolute_feasibility_auc') for arm in ('B_native','F_ERWF','P66_OCACRW','M72_OCIORW','N73_ANCHORED_HULL','O73_OCSVBW')}
   nm=delta(vals['N73_ANCHORED_HULL'],vals['M72_OCIORW']);on=delta(vals['O73_OCSVBW'],vals['N73_ANCHORED_HULL']);om=delta(vals['O73_OCSVBW'],vals['M72_OCIORW']);ob=delta(vals['O73_OCSVBW'],vals['B_native'])
   ...
   oca=cell(a,'O73_OCSVBW',v,k,'positive_certificate_probability_auc_for_teacher_feasibility');mca=cell(a,'M72_OCIORW',v,k,'positive_certificate_probability_auc_for_teacher_feasibility')
   oh=cell(a,'O73_OCSVBW',v,k,'harmful_pass_fraction');oti=cell(a,'O73_OCSVBW',v,k,'teacher_infeasible_pass_fraction');mh=cell(a,'M72_OCIORW',v,k,'harmful_pass_fraction');mti=cell(a,'M72_OCIORW',v,k,'teacher_infeasible_pass_fraction')
   ...
   onf=cell(truth,'O73_OCSVBW',v,k,'absolute_feasibility_auc_excluding_exact_0p5_feasible');mnf=cell(truth,'M72_OCIORW',v,k,'absolute_feasibility_auc_excluding_exact_0p5_feasible');nonfloor.append({'variant':v,'split':k,'O_minus_M_nonfloor_auc':delta(onf,mnf),'O_nonfloor_auc':onf,'M_nonfloor_auc':mnf})
   ...
   os=cell(a,'O73_OCSVBW',v,k,'safe_positive_pass_fraction');ps=cell(a,'P66_OCACRW',v,k,'safe_positive_pass_fraction');safe.append({'variant':v,'split':k,'O_minus_P66':delta(os,ps),'valid':os is not None and ps is not None and float(os)>float(ps)})
