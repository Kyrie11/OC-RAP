...
ALLOWED_KEY = 'direct_absolute_executable_witness_weight'
EXPECTED_NUM_WEIGHTS = 6
EXPECTED_FEATURE_SCHEMA = 1
EXPECTED_FEATURE_SOURCE = 'option_resolved_executable_recovery_continuation_side_channel'
...
def main() -> int:
    ap=argparse.ArgumentParser(description='v48.61 fail-closed ERWF frozen-Stage-I isolation audit')
    ...
    feature_schema=int(da.get('direct_recovery_absolute_executable_witness_feature_schema',0) or 0)
    feature_source=str(da.get('direct_recovery_absolute_executable_witness_feature_source','') or '')
    ...
    doc={
      'schema':'ocrap-v48.61-erwf-state-isolation-v1','valid':valid,
      ...
      'only_executable_witness_weight_added':expected,
      'executable_witness_feature_schema':feature_schema,
      'executable_witness_feature_source':feature_source,
      'executable_witness_feature_contract_valid':feature_contract,
      'stage_i_bitwise_identity':not changed and not removed and not shape,
      'test_roots_read':False,
    }
    a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n')
    print(json.dumps({'event':'v48_61_erwf_state_isolation','valid':valid,'output':str(a.output)}))
