...
def main():
    ...
    for role,_ in roots:
        ...
        by[role]={'rows':len(rr),'informative_rows':len(inf),'informative_fraction':len(inf)/max(1,len(rr)),'exact_physical_rows':len(exact),'two_sided_rows':sum(r['lower_finite'] and r['upper_finite'] for r in inf),'lower_only_rows':sum(r['lower_finite'] and not r['upper_finite'] for r in inf),'upper_only_rows':sum(r['upper_finite'] and not r['lower_finite'] for r in inf),'median_finite_interval_width':float(np.median(widths)) if widths else None,'mean_exact_cell_fraction':float(np.mean([r['exact_cell_fraction'] for r in rr])) if rr else 0.0,'mean_inactive_structural_cell_fraction':float(np.mean([r['inactive_structural_cell_fraction'] for r in rr])) if rr else 0.0,'mean_mixed_structural_cell_fraction':float(np.mean([r['mixed_structural_cell_fraction'] for r in rr])) if rr else 0.0,'mean_incomplete_profile_cell_fraction':float(np.mean([r['incomplete_profile_cell_fraction'] for r in rr])) if rr else 0.0,'mean_inverse_contradiction_cell_fraction':float(np.mean([r['inverse_contradiction_cell_fraction'] for r in rr])) if rr else 0.0}
    sha=hashlib.sha256(a.output.read_bytes()).hexdigest();doc={'schema':'ocrap-v48.81-switch-inverse-truth-index-summary-v1','engineering_version':'v48.81.2-OC-SITC-ENGFIX','valid':True,'rows':len(rows),'roles':by,'output':str(a.output.resolve()),'output_sha256':sha,'build_seconds':time.perf_counter()-t0,'dataset_reconstruction':False,'teacher_labels_changed':False,'teacher_future_input_to_model':False,'test_roots_read':False}
