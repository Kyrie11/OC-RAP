...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--reference-run',type=Path,required=True);ap.add_argument('--reference-contract',type=Path,required=True);ap.add_argument('--dotw-run',type=Path,required=True);ap.add_argument('--demand',default='false');ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();errors=[];rc=load(a.reference_contract);snap=rc.get('reference_candidate_checkpoint_sha256') or {};variants={};learn=[]
 if not rc.get('valid'):errors.append('reference contract invalid')
 for v in ('balanced','precision'):
  base=a.dotw_run/'candidates'/v;ref=a.reference_run/'candidates'/v/'model_v48_trac_sr'/'best.pt';dst=base/'model_v48_trac_sr'/'best.pt';summ=base/'model_v48_trac_sr'/'train_summary.json';state=base/'V48_70_STAGE_I_STATE_ISOLATION.json';pol=base/'POLICY_CONTRACT.env';complete=base/'TRAINING_COMPLETE.json';ecp=base/'EVIDENCE_CORRECTION_COMPLETE.json';miss=[str(p) for p in (ref,dst,summ,state,pol,complete,ecp) if not p.is_file()]
  ...
  checks={'reference_snapshot_matches':str(snap.get(v,''))==refsha,'init_checkpoint_matches':res(sd.get('init_checkpoint',''))==res(ref),'output_checkpoint_matches':res(sd.get('checkpoint',''))==res(dst),'trainable_prefix_exact':trainable==['direct_absolute_semantic_witness_gain'],'stage_i_isolation':bool(st.get('valid')) and bool(st.get('stage_i_bitwise_identity')) and bool(st.get('only_semantic_witness_gain_added')),'feature_contract':bool(st.get('semantic_witness_feature_contract_valid')),'factor_flags':bool(st.get('factor_flags_valid')) and flags.get('control_projection') is True and flags.get('boundary_transport') is False and flags.get('projection_fidelity') is True and flags.get('demand_normalized_fidelity')==b(a.demand) and flags.get('robust_occupancy') is False and flags.get('soft_occupancy_disagreement') is True,'policy_mode':pe.get('ABSOLUTE_FEASIBILITY_MODE')=='learned','policy_threshold':pe.get('ABSOLUTE_FEASIBILITY_THRESHOLD')=='0.5','policy_order':pe.get('SELECTION_SEMANTICS')=='rank_topk_then_absolute_feasibility_then_relative_filter_then_evidence_rerank','training_complete_sha_matches':str(tc.get('checkpoint_sha256',''))==dstsha,'evidence_complete_sha_matches':str(ec.get('checkpoint_sha256',''))==dstsha,'evidence_complete_source_matches':res(ec.get('source_checkpoint',''))==res(ref),'evidence_complete_trainable_exact':list(ec.get('trainable_prefixes') or [])==['direct_absolute_semantic_witness_gain'] and int(ec.get('trainable_state_params',-1))==2,'evidence_complete_no_regime_input':ec.get('regime_id_exposed_to_evidence_model') is False and ec.get('test_roots_read') is False}
  ok=all(checks.values());
  if not ok:errors.append(f'{v}: failed {[k for k,z in checks.items() if not z]}')
  variants[v]={'valid':ok,'checks':checks,'reference_sha256':refsha,'dotw_checkpoint_sha256':dstsha,'semantic_witness_gain':st.get('effective_clamped_semantic_witness_gain')};learn.append(res(dst))
 distinct=len(learn)==2 and len(set(learn))==2
 if not distinct:errors.append('balanced/precision DOTW checkpoints not distinct')
 valid=not errors and all(z.get('valid') for z in variants.values()) and distinct;doc={'schema':'ocrap-v48.70-dotw-variant-isolation-v1','valid':valid,'reference_run':res(a.reference_run),'dotw_run':res(a.dotw_run),'active_set_alignment':True,'path_stop_alignment':False,'classlocal_transport':False,'route_alignment':True,'reentry_alignment':True,'control_projection':True,'boundary_transport':False,'projection_fidelity':True,'demand_normalized_fidelity':b(a.demand),'robust_occupancy':False,'soft_occupancy_disagreement':True,'variants':variants,'distinct_dotw_checkpoint_paths':distinct,'errors':errors,'test_roots_read':False}
 a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_70_dotw_variant_isolation','valid':valid,'output':str(a.output)}));return 0 if valid else 30
