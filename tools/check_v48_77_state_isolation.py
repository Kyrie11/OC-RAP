...
TYPES = ('clearance','stopping','control','stability','route','persistent_reentry')
...
def main() -> int:
    ...
    new_ok=added==[ALLOWED_KEY] and isinstance(new,torch.Tensor) and tuple(new.shape)==(6,2)
    ...
    expected={
      ...
      'projection_fidelity':fidelity,'active_constraint_typed_source':True,'demand_normalized_fidelity':False,
      'robust_occupancy':False,'soft_occupancy_disagreement':False,'boundary_localized_occupancy_trust':False,
    ...
    objective=str(tcfg.get('direct_value_absolute_feasibility_supervision_objective','binary_sign'))
    truth_ok=truth=='censor_exact_0p5'; objective_ok=objective=='signed_margin_huber'
    raw=new.detach().cpu().float().tolist() if isinstance(new,torch.Tensor) else None
    eff=[[min(2.0,max(0.0,float(x))) for x in row] for row in raw] if raw is not None else None
    valid=not removed and not shape and not changed and new_ok and feature_ok and flags_ok and truth_ok and objective_ok
    doc={
      'schema':'ocrap-v48.77-actsi-state-isolation-v1','valid':bool(valid),
      ...
      'only_semantic_witness_gain_added':bool(new_ok),'new_tensor_shape':list(new.shape) if isinstance(new,torch.Tensor) else None,'new_tensor_numel':int(new.numel()) if isinstance(new,torch.Tensor) else None,
      'active_constraint_rows':list(TYPES),'gain_columns':['positive_rescue','universal_failure'],
      'raw_semantic_witness_gain':raw,'effective_clamped_semantic_witness_gain':eff,
      ...
      'absolute_feasibility_supervision_objective':objective,'supervision_objective_valid':objective_ok,
    ...
    print(json.dumps({'event':'v48_77_actsi_state_isolation','valid':valid,'output':str(a.output)}));return 0 if valid else 30
