...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--reference-run',type=Path,required=True);ap.add_argument('--reference-contract',type=Path,required=True);ap.add_argument('--borw-run',type=Path,required=True);ap.add_argument('--boundary',required=True);ap.add_argument('--history',required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();errors=[];rc=load(a.reference_contract);snap=rc.get('reference_candidate_checkpoint_sha256') or {};variants={};learn=[]
 if not rc.get('valid'):errors.append('reference contract invalid')
 for v in ('balanced','precision'):
  base=a.borw_run/'candidates'/v;ref=a.reference_run/'candidates'/v/'model_v48_trac_sr'/'best.pt';dst=base/'model_v48_trac_sr'/'best.pt';summ=base/'model_v48_trac_sr'/'train_summary.json';state=base/'V48_71_STAGE_I_STATE_ISOLATION.json';pol=base/'POLICY_CONTRACT.env';complete=base/'TRAINING_COMPLETE.json';ecp=base/'EVIDENCE_CORRECTION_COMPLETE.json';miss=[str(p) for p in (ref,dst,summ,state,pol,complete,ecp) if not p.is_file()]
