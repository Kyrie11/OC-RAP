...
ALLOWED_KEY = 'direct_absolute_common_witness_gain'
EXPECTED_NUM_WEIGHTS = 2
EXPECTED_FEATURE_SCHEMA = 1
EXPECTED_FEATURE_SOURCE = 'observation_consistent_option_resolved_finite_time_recovery_witness'
...
def main() -> int:
    ap=argparse.ArgumentParser(description='v48.62 fail-closed OC-CWRF frozen-Stage-I isolation audit')
    ...
    feature_schema=int(da.get('direct_recovery_absolute_common_witness_feature_schema',0) or 0)
    feature_source=str(da.get('direct_recovery_absolute_common_witness_feature_source','') or '')
    ...
    doc={
      'schema':'ocrap-v48.62-occwrf-state-isolation-v1','valid':valid,
      ...
      'only_common_witness_gain_added':expected,
      'common_witness_feature_schema':feature_schema,
      'common_witness_feature_source':feature_source,
      'common_witness_feature_contract_valid':feature_contract,
      'stage_i_bitwise_identity':not changed and not removed and not shape,
      'test_roots_read':False,
    }
    a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n')
    print(json.dumps({'event':'v48_62_occwrf_state_isolation','valid':valid,'output':str(a.output)}))
