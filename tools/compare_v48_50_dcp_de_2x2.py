#!/usr/bin/env python3
"""Compare v48.50 DCP-DRFC-DE A/B/C/D on development/certificate data only.
...
def _native_geometry(rows: list[dict[str, Any]], positive_gain: float) -> dict[str, Any]:
    ...
    for r in safe:
        ...
        if math.isfinite(a) and math.isfinite(b): paired.append((a,b))
    return {
...
def _v4850_witness(run: Path, variant: str) -> dict[str, Any]:
...
def _factor_contract(run: Path) -> dict[str, Any]:
    return _json(run / "V48_50_FACTOR_CONTRACT.json")
...
def _attribution_contract(arms: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for name,arm in arms.items():
        f=arm.get("factor_contract") or {}; de_frontier,exact_adv=expected[name]
        if (f.get("arm")!=name or f.get("training_option_execution_semantics")!="observation_class"
            ...
            or bool(f.get("decision_equivalent_frontier"))!=de_frontier
            or bool(f.get("native_exact_advantage_preservation"))!=exact_adv
    ...
    return {"valid":valid,"reference_arm":"A","missing_reference_fields":missing,"mismatched_arms":sorted(mismatches),
            ...
            "meaning":"All arms share the validated v48.49-C NCP+DRFC+NAP baseline with MC-NCP disabled. B-A = forward-exact/backward-smooth decision-equivalent frontier calibration; C-A = exact hard-DRS native advantage transport; D-B-C+A = interaction, only when valid."}
...
def _arm(run: Path, positive_gain: float) -> dict[str, Any]:
    ...
    return {
        ...
        "witness_precision": _v4850_witness(run, "precision"),
        "witness_balanced": _v4850_witness(run, "balanced"),
...
def main() -> int:
    ...
    report = {
        "schema": "v48.50-dcp-drfc-de-2x2-comparison-v1",
