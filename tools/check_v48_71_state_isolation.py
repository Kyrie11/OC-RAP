...
EXPECTED_SCHEMA=7
EXPECTED_SOURCE='boundary_localized_history_reachability_projected_recovery_witness'
...
def sha(p):
    ...
    return h.hexdigest()
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--reference',type=Path,required=True);ap.add_argument('--adapted',type=Path,required=True);ap.add_argument('--boundary',required=True);ap.add_argument('--history',required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args()
    ...
    feature_ok=schema==EXPECTED_SCHEMA and source==EXPECTED_SOURCE;flags_ok=flags==expected
