...
def barrier_names(arm: str):
    ...
    if arm in {"N_ROUTE", "P_Main_OCACRW"}: names.append("route")
    if arm in {"O_REENTRY", "P_Main_OCACRW"}: names.append("reentry_persistence")
    return names
...
def main():
    ap=argparse.ArgumentParser(description='v48.66 OC-ACRW feasibility-role and active-constraint witness audit')
    ...
    doc={
        'schema':'ocrap-v48.66-ocacrw-feasibility-role-audit-v1','positive_gain':a.positive_gain,'arms':out,
    ...
    print(json.dumps({'event':'v48_66_ocacrw_feasibility_role_audit','output':str(a.output)}))
