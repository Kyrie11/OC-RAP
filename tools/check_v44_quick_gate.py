...
def main() -> int:
    ...
    for bucket in ("near", "contact"):
        c = load(args.base_run / "calibration" / f"direct_value_risk_{bucket}_v44.json")
        if not c:
            failures.append(f"{bucket}: missing v44 risk certificate")
            continue
        ...
        rows += [
            (f"cal_{bucket}_valid", c.get("valid_for_deployment")),
            ...
            (f"cal_{bucket}_harm_ucb", verify.get("harmful_group_exposure_ucb90")),
        ]
        if not c.get("valid_for_deployment", False):
            failures.append(f"{bucket}: certificate invalid")
        if not math.isfinite(score_t) or not math.isfinite(opp_t):
            failures.append(f"{bucket}: non-finite score/opportunity threshold")

    for bucket in ("near_contact", "contact"):
        v_path = args.run / f"audit_{bucket}_selected_topk_v44_v44.json"
        v = load(v_path)
        if not v:
            failures.append(f"{bucket}: missing v44 audit")
            continue
        ...
        if direct <= 0:
            failures.append(f"{bucket}: OC-RAVA direct path never entered final decisions")
        if total > direct:
        ...
        if args.mode != "execution":
            s_path = args.run / f"audit_{bucket}_selected_topk_v44_scalar.json"
            scalar = load(s_path)
            if not scalar:
    ...
    print(f"V44 OC-RAVA {args.mode.upper()} GATE")
    for k, v in rows:
        print(f"{k}: {v}")
    for x in warnings:
        print("WARNING:", x)
