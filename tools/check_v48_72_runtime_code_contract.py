...
def main():
 ...
 contracts={}
 for name,hull in [('L72_BOX_SUPPORT',False),('M72_Main_OCIORW',True)]:
  c=tr._semantic_witness_checkpoint_feature_contract({**base,'direct_recovery_semantic_witness_interaction_box_support':True,'direct_recovery_semantic_witness_interaction_hull_support':hull});contracts[name]={'schema':c[0],'source':c[1]}
  if c!=(8,'interaction_oriented_history_reachability_projected_recovery_witness'):errors.append(f'{name} serializer contract mismatch: {c}')
 doc={'schema':'ocrap-v48.72-runtime-code-contract-v1','valid':not errors,'repo':str(repo),'imported_modules':{k:str(v) for k,v in act.items()},'expected_modules':{k:str(v) for k,v in exp.items()},'module_sha256':{k:sha(v) for k,v in act.items()},'contracts':contracts,'python_executable':sys.executable,'errors':errors};a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_72_runtime_code_contract','valid':not errors,'output':str(a.output)}));return 0 if not errors else 30
