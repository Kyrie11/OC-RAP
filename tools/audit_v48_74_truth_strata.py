...
def main():
 ...
 doc={'schema':'ocrap-v48.74-truth-floor-strata-audit-v1','arms':arms,'floor_value':.5,'floor_tolerance':1e-8,'interpretation':'read-only diagnostic; never changes labels/datasets. A large gap between full AUC and AUC excluding exact-0.5 teacher-feasible rows indicates structural-floor truth-contract debt may dominate observed source errors.','test_roots_read':False,'dataset_reconstruction':False}
 a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_74_truth_floor_strata_audit','output':str(a.output)}))
