...
def state(run:Path,v:str):
 p=run/'candidates'/v/'V48_78_STAGE_I_STATE_ISOLATION.json'
 if not p.is_file():return None
 d=json.loads(p.read_text());return {'valid':d.get('valid'),'root_tail_source_scale':d.get('raw_root_tail_source_scale'),'scale_shape':d.get('new_tensor_shape'),'tail_localization':d.get('tail_localization'),'best_checkpoint':d.get('adapted')}

def main():
 ap=argparse.ArgumentParser()
 for x in ('c75','e76','i78','j78'):ap.add_argument('--'+x,type=Path,required=True)
 ap.add_argument('--output',type=Path,required=True);a=ap.parse_args()
 runs={'C75_NATIVE_SIGN_PROJ':a.c75,'E76_GLOBAL_SIGNED_SOURCE':a.e76,'I78_ROOT_SHAPE':a.i78,'J78_MAIN_RTSI':a.j78}
 arms={name:{v:{'state':(state(run,v) if name.startswith(('I78','J78')) else None),'splits':{k:summarize(read(run,v,k)) for k in KINDS}} for v in VARIANTS} for name,run in runs.items()}
 specs={'I78_minus_E76':(a.e76,a.i78),'J78_minus_I78':(a.i78,a.j78),'J78_minus_E76':(a.e76,a.j78),'I78_minus_C75':(a.c75,a.i78),'J78_minus_C75':(a.c75,a.j78),'E76_minus_C75':(a.c75,a.e76)}
 comps={name:{v:{k:compare(read(base,v,k),read(new,v,k)) for k in KINDS} for v in VARIANTS} for name,(base,new) in specs.items()}
 doc={'schema':'ocrap-v48.78-rtsi-audit-v1','arms':arms,'comparisons':comps,'truth_contract':'censor_exact_0p5','objective':'signed_margin_huber','huber_beta':1.0,'factorial':'C75/E76 historical native/global-translation references vs I78 zero-mean root-shape source and J78 nested-tail-localized root-shape source','teacher_labels_changed':False,'certificate_geometry_changed':False,'option_translation_zero_mean':True,'projection_fidelity':False,'active_constraint_typed_source':False,'boundary_transport':False,'dataset_reconstruction':False,'test_roots_read':False}
 a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_78_rtsi_audit','output':str(a.output)}))
