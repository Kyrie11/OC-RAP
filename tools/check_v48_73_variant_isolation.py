...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--reference-run',type=Path,required=True);ap.add_argument('--reference-contract',type=Path,required=True);ap.add_argument('--irrw-run',type=Path,required=True);ap.add_argument('--anchor',required=True);ap.add_argument('--response',required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();errors=[];rc=load(a.reference_contract);snap=rc.get('reference_candidate_checkpoint_sha256') or {};variants={};learn=[]
 if not rc.get('valid'):errors.append('reference contract invalid')
 for v in ('balanced','precision'):
  base=a.irrw_run/'candidates'/v;ref=a.reference_run/'candidates'/v/'model_v48_trac_sr'/'best.pt';dst=base/'model_v48_trac_sr'/'best.pt';summ=base/'model_v48_trac_sr'/'train_summary.json';state=base/'V48_73_STAGE_I_STATE_ISOLATION.json';pol=base/'POLICY_CONTRACT.env';complete=base/'TRAINING_COMPLETE.json';ecp=base/'EVIDENCE_CORRECTION_COMPLETE.json';miss=[str(p) for p in (ref,dst,summ,state,pol,complete,ecp) if not p.is_file()]
  ...
  variants[v]={'valid':ok,'checks':checks,'reference_sha256':refsha,'irrw_checkpoint_sha256':dstsha,'semantic_witness_gain':st.get('effective_clamped_semantic_witness_gain')};learn.append(res(dst))
 distinct=len(learn)==2 and len(set(learn))==2
 if not distinct:errors.append('balanced/precision IRRW checkpoints not distinct')
 valid=not errors and all(z.get('valid') for z in variants.values()) and distinct;doc={'schema':'ocrap-v48.73-irrw-variant-isolation-v1','valid':valid,'reference_run':res(a.reference_run),'irrw_run':res(a.irrw_run),'active_set_alignment':True,'path_stop_alignment':False,'classlocal_transport':False,'route_alignment':True,'reentry_alignment':True,'control_projection':True,'boundary_transport':False,'projection_fidelity':True,'demand_normalized_fidelity':False,'robust_occupancy':False,'soft_occupancy_disagreement':False,'boundary_localized_occupancy_trust':False,'history_occupancy_reachability':False,'interaction_box_support':True,'interaction_hull_support':True,'interaction_anchor_support':b(a.anchor),'interaction_response_support':b(a.response),'variants':variants,'distinct_irrw_checkpoint_paths':distinct,'errors':errors,'test_roots_read':False}
 a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_73_irrw_variant_isolation','valid':valid,'output':str(a.output)}));return 0 if valid else 30
