...
def main()->int:
    ap=argparse.ArgumentParser(description='v48.60 CPHR balanced/precision provenance isolation')
    ap.add_argument('--reference-run',type=Path,required=True); ap.add_argument('--reference-contract',type=Path,required=True)
    ap.add_argument('--cphr-run',type=Path,required=True); ap.add_argument('--output',type=Path,required=True)
    ...
    for v in ('balanced','precision'):
        ...
        dst=a.cphr_run/'candidates'/v/'model_v48_trac_sr'/'best.pt'
        summ=a.cphr_run/'candidates'/v/'model_v48_trac_sr'/'train_summary.json'
        state=a.cphr_run/'candidates'/v/'V48_60_STAGE_I_STATE_ISOLATION.json'
        pol=a.cphr_run/'candidates'/v/'POLICY_CONTRACT.env'
        ...
        ok={
          ...
          'trainable_prefix_exact':trainable==['direct_absolute_physical_headroom_weight'],
          'stage_i_isolation':bool(st.get('valid')) and bool(st.get('stage_i_bitwise_identity')) and bool(st.get('only_physical_headroom_weight_added')),
        ...
        variants[v]={'valid':valid,'checks':ok,'reference_checkpoint':resolved(ref),'reference_sha256':refsha,
                     'cphr_checkpoint':resolved(dst),'trainable_param_prefixes':trainable,'duplicate_critical_keys':dupcrit}
        learned_paths.append(resolved(dst))
    distinct=len(learned_paths)==2 and len(set(learned_paths))==2
    if not distinct: errors.append('balanced/precision CPHR checkpoint paths not distinct')
    valid=not errors and distinct and all(x.get('valid') for x in variants.values())
    doc={'schema':'ocrap-v48.60-cphr-variant-isolation-v1','valid':valid,'reference_run':resolved(a.reference_run),
         'cphr_run':resolved(a.cphr_run),'variants':variants,'distinct_cphr_checkpoint_paths':distinct,'errors':errors,'test_roots_read':False}
    a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n')
    print(json.dumps({'event':'v48_60_cphr_variant_isolation','valid':valid,'output':str(a.output)})); return 0 if valid else 30
