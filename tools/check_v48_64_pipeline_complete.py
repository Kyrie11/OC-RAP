...
def sha(p):
 ...
 return h.hexdigest()
def check_run(run:Path,active:bool,pathstop:bool,errors:list[str],hashes:dict):
 vi=run/'V48_64_VARIANT_ISOLATION.json';factor=run/'V48_64_FACTOR_CONTRACT.json';terminal=run/'dedicated_recalibration_status.json';top=[vi,factor,terminal]
 for p in top:
  if not p.is_file():errors.append(f'missing {p}')
 if vi.is_file():
  d=load(vi)
  if not d.get('valid'):errors.append(f'{run.name}: variant isolation invalid')
  if bool(d.get('active_set_alignment'))!=active or bool(d.get('path_stop_alignment'))!=pathstop:errors.append(f'{run.name}: variant factor flags mismatch')
 if factor.is_file():
  d=load(factor);req={'trainable_parameters':2,'threshold':.5,'threshold_search':False,'regime_id_input':False,'proposal_top_k':5,'proposal_expansion':False,'test_roots_read':False,'semantic_witness_feature_schema':1,'semantic_witness_feature_source':'semantics_aligned_common_executable_recovery_witness','teacher_future_input':False,'relative_score_intervention':False,'active_set_alignment':active,'path_stop_alignment':pathstop,'quantifier_logic':'exists positive common witness / forall failure veto'}
 ...
 for v in ('balanced','precision'):
  ...
  st=base/'V48_64_STAGE_I_STATE_ISOLATION.json'
  if st.is_file():
   sd=load(st)
   if not (sd.get('valid') and sd.get('semantic_witness_feature_contract_valid') and sd.get('factor_flags_valid')):errors.append(f'{run.name}/{v}: state/feature isolation invalid')
 for p in top:
  if p.is_file():hashes[str(p)]=sha(p)
def main():
 ap=argparse.ArgumentParser(description='v48.64 OC-SARW attribution-ready engineering sentinel');ap.add_argument('--reference-contract',type=Path,required=True);ap.add_argument('--v63-complete',type=Path,required=True);ap.add_argument('--active-run',type=Path,required=True);ap.add_argument('--path-run',type=Path,required=True);ap.add_argument('--main-run',type=Path,required=True);ap.add_argument('--feasibility-audit',type=Path,required=True);ap.add_argument('--comparison',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();errors=[];hashes={}
 ...
 if a.v63_complete.is_file():
  d=load(a.v63_complete)
  if not (d.get('valid') and d.get('attribution_ready') and d.get('engineering_version')=='v48.63.0-OC-QARW' and not d.get('test_roots_read')):errors.append('V48.63 prerequisite invalid')
