#!/usr/bin/env python3
"""Fit/verify v46 OC-RACE opportunity-gated selective recovery certificates.
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Fit/verify v46 OC-RACE opportunity-gated risk certificate.")
    ...
    if not bool(getattr(bundle.model, "direct_recovery_opportunity_head", False)):
        raise ValueError("v46 calibration requires model.direct_recovery_opportunity_head=true")
    ...
    for i, path in enumerate(paths, 1):
        ...
        if pred.direct_recovery_value is None or pred.direct_recovery_opportunity is None:
            raise ValueError("checkpoint does not expose v46 score and opportunity outputs")
        row = {
            "data": d,
        ...
        if i == 1 or i % 1000 == 0:
            print({"event": "v46_risk_calibration_progress", "bucket": args.bucket, "seen": i, "total": len(paths)}, flush=True)
    ...
    if not correlation_valid:
        warnings.append(
            "pred/teacher advantage correlation below requirement "
    ...
    development_valid = base_contract_valid and correlation_valid and conditional_risk_valid
    ...
    result = {
        "method": "oc_race_supervised_expert_relative_opportunity_selective_certificate",
        "bucket": args.bucket,
