#!/usr/bin/env python3
"""Selection-conditional conformal calibration for OCSAVA.

The selector first chooses one deterministic top-scoring actionable candidate
per scene-time group.  Calibration therefore scores only that selected
candidate, rather than taking the maximum residual over every candidate.  This
matches the deployed selection rule and avoids the excessive multiplicity
penalty that drove v41's q to about 0.57 and challenge rate to zero.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Calibrate top-1 selection-conditional additive recovery-advantage LCB.")
    ap.add_argument("--dataset", required=True)
    ap.add_argument("--checkpoint", required=True)
    ap.add_argument("--output", type=Path, required=True)
    ap.add_argument("--delta", type=float, default=0.10)
    ap.add_argument("--required-min-groups", type=int, default=30)
    ...
    ap.add_argument("--min-advantage-lcb", type=float, default=0.010)
    ap.add_argument("--numerical-margin", type=float, default=0.002)
    args = ap.parse_args()
    ...
    if str(getattr(bundle.model, "direct_recovery_value_output", "probability")) != "score":
        raise ValueError("v42 selection-conditional calibration requires model.direct_recovery_value_output=score")
    ...
    for i, path in enumerate(paths, 1):
        ...
        if i == 1 or i % 1000 == 0:
            print({"event": "v42_calibration_progress", "seen": i, "total": len(paths)}, flush=True)

    selected_rows: list[dict[str, float | int | bool]] = []
    ...
    for items in groups.values():
        ...
        recs = [
            ...
            for x in items
            if (not x["nominal"])
            and x["macro"] in macro_ids
            and x["feasible"]
            and x["hard"] <= args.max_hard
            and x["harm"] <= args.max_harm
        ...
        # Stable deterministic top-1: max score, then smallest candidate index.
        ...
        selected_rows.append(
            {
                ...
                "residual": float(selected[1] - selected[2]),
                "oracle_best_teacher_adv": float(oracle_best),
                "opportunity": bool(oracle_best >= args.positive_gain),
                "selected_positive": bool(selected[2] >= args.positive_gain),
            }
        )

    warnings: list[str] = []
    residuals = np.asarray([float(x["residual"]) for x in selected_rows], dtype=float)
    q = float("inf")
    if residuals.size:
        q = max(
            0.0,
            float(
                finite_sample_upper_quantile(
                    residuals,
                    args.delta,
                    numerical_margin=args.numerical_margin,
                    strict=True,
                )
            ),
        )
    else:
        warnings.append("no complete actionable calibration groups")
    ...
    pred_adv = np.asarray([float(x["pred_adv"]) for x in selected_rows], dtype=float)
    teacher_adv = np.asarray([float(x["teacher_adv"]) for x in selected_rows], dtype=float)
    oracle_best = np.asarray([float(x["oracle_best_teacher_adv"]) for x in selected_rows], dtype=float)
    lcb = pred_adv - q if np.isfinite(q) else np.full_like(pred_adv, -np.inf)
    challenge = lcb >= args.min_advantage_lcb
    selected_pos = teacher_adv >= args.positive_gain
    selected_nonpos = teacher_adv <= 0.0
    opportunity = oracle_best >= args.positive_gain
    captured = selected_pos & opportunity

    result = {
        "method": "top1_selection_conditional_additive_conformal_advantage_lcb",
        "selection_rule": "highest_predicted_actionable_advantage_then_smallest_candidate_index",
        "dataset": args.dataset,
        "checkpoint": args.checkpoint,
        "delta": args.delta,
        "direct_value_uncertainty_mode": "additive",
        "direct_value_additive_q": q,
        ...
        "min_advantage_lcb": args.min_advantage_lcb,
        "empirical_selected_coverage": _safe_rate(residuals <= q) if np.isfinite(q) else None,
        "oracle_opportunity_rate": _safe_rate(opportunity),
        "top1_selected_positive_rate": _safe_rate(selected_pos),
        "top1_opportunity_capture_rate": float(captured.sum() / max(1, opportunity.sum())) if opportunity.size else None,
        "selected_advantage_mae": float(np.mean(np.abs(pred_adv - teacher_adv))) if pred_adv.size else None,
        "all_pair_advantage_mae": float(np.mean(all_pair_errors)) if all_pair_errors else None,
        "challenge_rate": _safe_rate(challenge),
        "challenge_precision": _safe_rate(selected_pos[challenge]) if challenge.any() else None,
        "positive_challenge_recall": float((challenge & selected_pos).sum() / max(1, selected_pos.sum())) if selected_pos.size else None,
        "negative_challenge_rate": _safe_rate(challenge[selected_nonpos]) if selected_nonpos.any() else None,
        "selected_teacher_advantage_mean": float(teacher_adv.mean()) if teacher_adv.size else None,
        "selected_predicted_advantage_mean": float(pred_adv.mean()) if pred_adv.size else None,
        "skipped": skipped,
        "warnings": warnings,
    }
