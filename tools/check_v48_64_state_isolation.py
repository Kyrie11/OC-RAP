...
def main()->int:
    ap=argparse.ArgumentParser(description='v48.64 fail-closed OC-SARW frozen-Stage-I isolation audit')
    ...
    ap.add_argument('--active-set',default='true');ap.add_argument('--path-stop',default='true')
    ...
    feature_ok=schema==1 and source==EXPECTED_FEATURE_SOURCE
    flags_ok=(bool(da.get('direct_recovery_absolute_semantic_witness_correction',False)) and
              bool(da.get('direct_recovery_semantic_witness_active_set_alignment',True))==b(a.active_set) and
              bool(da.get('direct_recovery_semantic_witness_path_stop_alignment',True))==b(a.path_stop))
    valid=not removed and not shape and not changed and expected and feature_ok and flags_ok
    doc={'schema':'ocrap-v48.64-sarw-state-isolation-v1','valid':valid,'reference':str(a.reference.resolve(strict=False)),'adapted':str(a.adapted.resolve(strict=False)),
         ...
         'semantic_witness_feature_contract_valid':feature_ok,'active_set_alignment':b(a.active_set),'path_stop_alignment':b(a.path_stop),'factor_flags_valid':flags_ok,
         'stage_i_bitwise_identity':not changed and not removed and not shape,'test_roots_read':False}
    a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n')
    print(json.dumps({'event':'v48_64_sarw_state_isolation','valid':valid,'output':str(a.output)}));return 0 if valid else 30
