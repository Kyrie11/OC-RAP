...
EXPECTED_FEATURE_SOURCE='active_constraint_coverage_common_executable_recovery_witness'
...
def main()->int:
    ap=argparse.ArgumentParser(description='v48.66 fail-closed OC-ACRW frozen-Stage-I isolation audit')
    ...
    ap.add_argument('--route',default='false');ap.add_argument('--reentry',default='false');a=ap.parse_args()
    ra=load(a.reference); da=load(a.adapted); rs=ra.get('model_state',ra); ds=da.get('model_state',da)
    ...
    flags_ok=(flags['semantic_correction'] and flags['active_set_alignment'] and not flags['path_stop_alignment'] and not flags['classlocal_transport'] and flags['route_alignment']==b(a.route) and flags['reentry_alignment']==b(a.reentry))
    valid=bool(not removed and not shape and not changed and expected and feature_ok and flags_ok)
    raw_gain=[float(x) for x in new.detach().cpu().reshape(-1).tolist()] if isinstance(new,torch.Tensor) else None
    effective_gain=[min(2.0,max(0.0,x)) for x in raw_gain] if raw_gain is not None else None
    doc={'schema':'ocrap-v48.66-acrw-state-isolation-v1','valid':valid,
      ...
      'raw_semantic_witness_gain':raw_gain,'effective_clamped_semantic_witness_gain':effective_gain,
    ...
    print(json.dumps({'event':'v48_66_acrw_state_isolation','valid':valid,'output':str(a.output)}));return 0 if valid else 30
