...
def main():
 ...
 incomplete=sample(.5); incomplete['root_assignments']=np.array([0,0]); incomplete['future_metadata']=[{}]
 r_incomplete=nested_tail_switch_inverse_interval(incomplete)
 contradiction=sample(.5)
 r_contradiction=nested_tail_switch_inverse_interval(contradiction)
 synth=bool(r_inactive.exact_physical and abs(r_inactive.physical_lower-.7)<1e-5 and abs(r_inactive.physical_upper-.7)<1e-5 and r_active.physical_upper<=.60001 and (not r_mixed.lower_finite) and (not r_mixed.upper_finite) and r_mixed.mixed_structural_cell_fraction>0 and (not r_incomplete.lower_finite) and (not r_incomplete.upper_finite) and r_incomplete.incomplete_profile_cell_fraction>0 and (not r_contradiction.lower_finite) and (not r_contradiction.upper_finite) and r_contradiction.inverse_contradiction_cell_fraction>0)
 if not synth:errors.append('switch-aware inverse synthetic contract failed')
 doc={'schema':'ocrap-v48.81-runtime-code-contract-v1','engineering_version':'v48.81.2-OC-SITC-ENGFIX','valid':not errors,'attribution_ready':not errors,'errors':errors,'runtime_modules':runtime,'truth_contract_synthetic':{'valid':synth,'inactive_floor_exact':[r_inactive.physical_lower,r_inactive.physical_upper],'active_floor_interval':[r_active.physical_lower,r_active.physical_upper],'mixed_root_exposure_interval':[r_mixed.physical_lower,r_mixed.physical_upper],'mixed_root_exposure_fraction':r_mixed.mixed_structural_cell_fraction,'incomplete_profile_interval':[r_incomplete.physical_lower,r_incomplete.physical_upper],'incomplete_profile_fraction':r_incomplete.incomplete_profile_cell_fraction,'inverse_contradiction_interval':[r_contradiction.physical_lower,r_contradiction.physical_upper],'inverse_contradiction_fraction':r_contradiction.inverse_contradiction_cell_fraction},'source_contract':{'source_capacity_changed_vs_J78':False,'trainable_parameter':'direct_absolute_root_tail_source_scale[1]','root_tail_source':True,'tail_localization':True,'boundary_transport':False,'regime_id_input':False},'supervision_contract':{'truth_contract':'switch_inverse_interval_bounds','objective':'signed_margin_interval_huber','teacher_labels_changed':False,'teacher_future_input_to_model':False},'dataset_reconstruction':False,'test_roots_read':False}
