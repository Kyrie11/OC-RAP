...
def main():
    ...
    rows={}
    for rel,p in files:
        raw=p.read_bytes(); rows[rel]={'sha256':hashlib.sha256(raw).hexdigest(),'size':len(raw)}
    doc={'schema':'ocrap-v48.124.10.4-pcd-oracle-result-bundle-v1','pipeline_exit_code':int(a.exit_code),'complete_exit_zero':int(a.exit_code)==0,'num_files':len(rows),'files':rows}
    ...
    os.replace(tmp,out)
    print(json.dumps({'output':str(out),'manifest':str(man),'num_files':len(rows)+1,'pipeline_exit_code':int(a.exit_code)},indent=2))
