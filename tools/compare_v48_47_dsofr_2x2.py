#!/usr/bin/env python3
"""Compare v48.47 DS-OFR A/B/C/D on development/certificate data only.
...
def _v4847_witness(run: Path, variant: str) -> dict[str, Any]:
...
def _factor_contract(run: Path) -> dict[str, Any]:
    return _json(run / "V48_47_FACTOR_CONTRACT.json")
...
def _attribution_contract(arms: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for name,arm in arms.items():
        f=arm.get("factor_contract") or {}; dw,frontier=expected[name]
        if (f.get("arm")!=name or f.get("training_option_execution_semantics")!="observation_class"
            ...
            or bool(f.get("decision_weighted_observation"))!=dw
    ...
    return {"valid":valid,"reference_arm":"A","missing_reference_fields":missing,"mismatched_arms":sorted(mismatches),
            ...
            "meaning":"All arms share paper-consistent observation-class training/evaluation. B-A = decision-weighted observation calibration; C-A = direct OC-MERO recovery-frontier calibration; D-B-C+A = interaction, only when valid"}
...
def _arm(run: Path, positive_gain: float) -> dict[str, Any]:
    ...
    return {
        ...
        "witness_precision": _v4847_witness(run, "precision"),
        "witness_balanced": _v4847_witness(run, "balanced"),
