...
def main()->int:
 ...
 for v in ('balanced','precision'):
  ref=a.reference_run/'candidates'/v/'model_v48_trac_sr'/'best.pt';dst=a.sarw_run/'candidates'/v/'model_v48_trac_sr'/'best.pt';summ=a.sarw_run/'candidates'/v/'train_summary.json';state=a.sarw_run/'candidates'/v/'V48_64_STAGE_I_STATE_ISOLATION.json';pol=a.sarw_run/'candidates'/v/'POLICY_CONTRACT.env'
  miss=[str(p) for p in (ref,dst,summ,state,pol) if not p.is_file()]
  if miss:errors.append(f'{v}: missing {miss}');variants[v]={'valid':False,'missing':miss};continue
  sd,st=load(summ),load(state);pe,dups=env(pol);trainable=list(sd.get('trainable_param_prefixes') or []);refsha=sha(ref)
  ...
  variants[v]={'valid':ok,'checks':checks,'trainable_param_prefixes':trainable,'reference_sha256':refsha,'sarw_checkpoint':res(dst)};learn.append(res(dst))
