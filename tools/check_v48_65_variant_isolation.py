...
def main()->int:
 ap=argparse.ArgumentParser(description='v48.65 OC-CLRW balanced/precision provenance and factor isolation')
 ap.add_argument('--reference-run',type=Path,required=True);ap.add_argument('--reference-contract',type=Path,required=True);ap.add_argument('--clrw-run',type=Path,required=True);ap.add_argument('--output',type=Path,required=True)
 ap.add_argument('--active-set',default='true');ap.add_argument('--classlocal',default='true');ap.add_argument('--path-stop',default='false');a=ap.parse_args();errors=[];rc=load(a.reference_contract);snap=rc.get('reference_candidate_checkpoint_sha256') or {};variants={};learn=[]
 if not rc.get('valid'):errors.append('reference contract invalid')
 for v in ('balanced','precision'):
  base=a.clrw_run/'candidates'/v;ref=a.reference_run/'candidates'/v/'model_v48_trac_sr'/'best.pt';dst=base/'model_v48_trac_sr'/'best.pt';summ=base/'model_v48_trac_sr'/'train_summary.json';state=base/'V48_65_STAGE_I_STATE_ISOLATION.json';pol=base/'POLICY_CONTRACT.env';complete=base/'TRAINING_COMPLETE.json';ecp=base/'EVIDENCE_CORRECTION_COMPLETE.json'
  ...
  sd,st,tc,ec=load(summ),load(state),load(complete),load(ecp);pe,dups=env(pol);trainable=list(sd.get('trainable_param_prefixes') or []);refsha=sha(ref);dstsha=sha(dst);ck=sd.get('checkpoint')
  flags=st.get('factor_flags') or {}
  checks={'reference_snapshot_matches':str(snap.get(v,''))==refsha,'init_checkpoint_matches':res(sd.get('init_checkpoint',''))==res(ref),'output_checkpoint_matches':res(ck or '')==res(dst),
    ...
    'feature_contract':bool(st.get('semantic_witness_feature_contract_valid')),'factor_flags':bool(st.get('factor_flags_valid')) and flags.get('active_set_alignment')==b(a.active_set) and flags.get('path_stop_alignment')==b(a.path_stop) and flags.get('classlocal_transport')==b(a.classlocal),
  ...
  variants[v]={'valid':ok,'checks':checks,'trainable_param_prefixes':trainable,'reference_sha256':refsha,'clrw_checkpoint_sha256':dstsha,'clrw_checkpoint':res(dst),'train_summary':res(summ)};learn.append(res(dst))
 distinct=len(learn)==2 and len(set(learn))==2
 if not distinct:errors.append('balanced/precision CLRW checkpoints not distinct')
 valid=not errors and distinct and all(x.get('valid') for x in variants.values())
 doc={'schema':'ocrap-v48.65-clrw-variant-isolation-v1','valid':valid,'reference_run':res(a.reference_run),'clrw_run':res(a.clrw_run),'active_set_alignment':b(a.active_set),'path_stop_alignment':b(a.path_stop),'classlocal_transport':b(a.classlocal),'variants':variants,'distinct_clrw_checkpoint_paths':distinct,'errors':errors,'test_roots_read':False}
 a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_65_clrw_variant_isolation','valid':valid,'output':str(a.output)}));return 0 if valid else 30
