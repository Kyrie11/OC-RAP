...
def main():
 ...
 for p in (a.reference_contract,a.v72_complete,a.v72_comparison,a.runtime_contract,a.feasibility_audit,a.response_audit,a.truth_strata,a.comparison):
  if p.is_file():hashes[str(p)]=sha(p)
 valid=not errors;doc={'schema':'ocrap-v48.74-svbw-pipeline-complete-v1','valid':valid,'attribution_ready':valid,'algorithm_version':'v48.74-DCP-DRFC-BCDE-RIFA-OC-SVBW','engineering_version':'v48.74.0-OC-SVBW','errors':errors,'artifact_sha256':hashes,'arms':{'historical_M72':'static empirical acceleration hull + directional support','N73_ANCHORED_HULL':str(a.anchor_run),'O73_Main_OCSVBW':str(a.main_run)},'test_roots_read':False,'dataset_reconstruction':False};a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_74_svbw_pipeline_complete','valid':valid,'output':str(a.output)}));return 0 if valid else 30
