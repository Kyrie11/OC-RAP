#!/usr/bin/env python3
"""Compare v48.45 SOWR A/B/C/D on development/certificate data only.

The report intentionally does not read test roots.  It extracts the exact metrics
needed for the pre-registered SOWR causal readout and recomputes component-level
...
def _sowr_stage(run: Path, variant: str) -> dict[str, Any]:
    """Return head-recalibration diagnostics without touching test roots."""
    p = run / "candidates" / variant / "witness_stage" / "SOWR_COMPLETE.json"
    if not p.is_file():
        return {
            "enabled": False,
            "margin_witness_recalibration": False,
            "observation_kernel_recalibration": False,
        }
    doc = _json(p)
    initial = doc.get("initial_val") or {}
    best = doc.get("best_val") or {}
    delta = doc.get("delta_best_minus_initial") or {}
    return {
        "enabled": True,
        "margin_witness_recalibration": bool(doc.get("margin_witness_recalibration", False)),
        "observation_kernel_recalibration": bool(doc.get("observation_kernel_recalibration", False)),
