...
def main() -> int:
    ...
    for k in shared:
        if tuple(rs[k].shape) != tuple(ds[k].shape):
            shape.append(k)
        elif not torch.equal(rs[k].cpu(), ds[k].cpu()):
            changed.append(k)
    removed = sorted(set(rs) - set(ds)); added = sorted(set(ds) - set(rs)); w = ds.get(ALLOWED_KEY)
    new_ok = added == [ALLOWED_KEY] and isinstance(w, torch.Tensor) and w.ndim == 2 and w.shape[1] > 0
    ...
    expected = {
        'semantic_correction': True, 'active_set_alignment': True, 'path_stop_alignment': False,
        'classlocal_transport': False, 'route_alignment': True, 'reentry_alignment': True,
        'control_projection': True, 'boundary_transport': False, 'projection_fidelity': False,
        'active_constraint_typed_source': False, 'root_tail_source': True, 'tail_localization': True,
        'structured_tail_field': True, 'signed_tail_channels': bool(w is not None and w.shape[0] == 2),
    }
    tcfg = ((da.get('cfg') or {}).get('training') or {})
    truth = str(tcfg.get('direct_value_absolute_feasibility_truth_contract', 'legacy_full'))
    obj = str(tcfg.get('direct_value_absolute_feasibility_supervision_objective', 'binary_sign'))
    ...
    valid = (not removed and not shape and not changed and new_ok and feature_ok and flags == expected
             and truth == 'structural_interval_bounds' and obj == 'signed_margin_interval_huber' and idx_ok)
    doc = {
        'schema': 'ocrap-v48.82-sntf-state-isolation-v1', 'valid': valid,
        'reference': str(a.reference), 'adapted': str(a.adapted),
        ...
        'new_tensor_shape': list(w.shape) if isinstance(w, torch.Tensor) else None,
        'new_tensor_numel': int(w.numel()) if isinstance(w, torch.Tensor) else None,
        'raw_structured_tail_field_weight': w.detach().cpu().float().tolist() if isinstance(w, torch.Tensor) else None,
        'semantic_witness_feature_schema': schema, 'semantic_witness_feature_source': source,
        'semantic_witness_feature_contract_valid': feature_ok,
        'factor_flags': flags, 'factor_flags_valid': flags == expected,
        'absolute_feasibility_truth_contract': truth,
        'absolute_feasibility_supervision_objective': obj,
        'absolute_feasibility_truth_index': idx,
        'absolute_feasibility_truth_index_expected': str(a.truth_index.resolve()),
        'absolute_feasibility_truth_index_valid': idx_ok,
    ...
    print(json.dumps({'event': 'v48_82_state_isolation', 'valid': valid, 'output': str(a.output)}))
