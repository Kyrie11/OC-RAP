...
def sha(p):
 ...
 return h.hexdigest()
def check_run(run,proj,bound,errors,hashes):
 vi=run/'V48_67_VARIANT_ISOLATION.json';factor=run/'V48_67_FACTOR_CONTRACT.json';terminal=run/'dedicated_recalibration_status.json';top=[vi,factor,terminal]
 for p in top:
  if not p.is_file():errors.append(f'missing {p}')
 if vi.is_file():
  d=load(vi)
  if not d.get('valid'):errors.append(f'{run.name}: variant isolation invalid')
  if not (d.get('route_alignment') is True and d.get('reentry_alignment') is True and bool(d.get('control_projection'))==proj and bool(d.get('boundary_transport'))==bound):errors.append(f'{run.name}: flags mismatch')
 if factor.is_file():
  d=load(factor);req={'trainable_parameters':2,'threshold':.5,'threshold_search':False,'regime_id_input':False,'proposal_top_k':5,'proposal_expansion':False,'test_roots_read':False,'semantic_witness_feature_schema':3,'semantic_witness_feature_source':'projected_boundary_common_executable_recovery_witness','active_set_alignment':True,'path_stop_alignment':False,'classlocal_transport':False,'route_alignment':True,'reentry_alignment':True,'control_projection':proj,'boundary_transport':bound,'relative_score_intervention':False,'teacher_future_input':False}
  for k,v in req.items():
   if d.get(k)!=v:errors.append(f'{run.name}: factor {k} mismatch {d.get(k)!r}!={v!r}')
 for v in ('balanced','precision'):
  base=run/'candidates'/v;cal=base/'calibration';req=[base/'model_v48_trac_sr'/'best.pt',base/'model_v48_trac_sr'/'train_summary.json',base/'TRAINING_COMPLETE.json',base/'EVIDENCE_CORRECTION_COMPLETE.json',base/'V48_67_STAGE_I_STATE_ISOLATION.json',base/'POLICY_CONTRACT.env',cal/'METRIC_CALIBRATION_CONTRACT.json',cal/'dev_diagnostic_near_v48.proposal_rows.jsonl',cal/'dev_diagnostic_contact_v48.proposal_rows.jsonl',cal/'direct_value_risk_near_v48.proposal_rows.jsonl',cal/'direct_value_risk_contact_v48.proposal_rows.jsonl']
  ...
  if (base/'V48_67_STAGE_I_STATE_ISOLATION.json').is_file():
   st=load(base/'V48_67_STAGE_I_STATE_ISOLATION.json')
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--reference-contract',type=Path,required=True);ap.add_argument('--v66-complete',type=Path,required=True);ap.add_argument('--control-run',type=Path,required=True);ap.add_argument('--boundary-run',type=Path,required=True);ap.add_argument('--main-run',type=Path,required=True);ap.add_argument('--feasibility-audit',type=Path,required=True);ap.add_argument('--boundary-audit',type=Path,required=True);ap.add_argument('--comparison',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();errors=[];hashes={}
 ...
 if a.v66_complete.is_file():
  d=load(a.v66_complete)
  if not (d.get('valid') and d.get('attribution_ready') and d.get('engineering_version')=='v48.66.0-OC-ACRW' and not d.get('test_roots_read')):errors.append('V48.66 prerequisite invalid')
