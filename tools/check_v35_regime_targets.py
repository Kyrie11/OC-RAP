...
def main() -> int:
    ap = argparse.ArgumentParser(description="Check OC-RAP v35 tail-budget regime targets from closed-loop/audit JSON outputs.")
    ...
    ap.add_argument("--contact-nup-min", type=float, default=0.95)
    ap.add_argument("--contact-int-max", type=float, default=0.10)
    ap.add_argument("--offline-contact-nup-min", type=float, default=0.95)
    ap.add_argument("--offline-contact-int-max", type=float, default=0.15)
    args = ap.parse_args()

    root = args.run_dir
    files = {
        "safe": root / "closed_loop_safe_fast_v35.json",
        "near": root / "audit_near_contact_selected_topk_v35_v35.json",
        "contact": root / "audit_contact_selected_topk_v35_v35.json",
    }
    ...
    offline = root / "eval_contact_v35_v35.json"
    if offline.exists():
        d = load(offline)
