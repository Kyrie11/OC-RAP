...
ALLOWED_KEY = 'direct_absolute_root_tail_source_scale'
...
def main() -> int:
    ...
    new_ok = added == [ALLOWED_KEY] and isinstance(w, torch.Tensor) and w.ndim == 1 and w.numel() == 1
    ...
    valid = (not removed and not shape and not changed and new_ok and feature_ok and flags == expected
             and truth == 'switch_inverse_interval_bounds' and obj == 'signed_margin_interval_huber' and idx_ok)
    doc = {
        'schema': 'ocrap-v48.81-sitc-state-isolation-v1', 'valid': valid,
        ...
        'raw_root_tail_source_scale': w.detach().cpu().float().tolist() if isinstance(w, torch.Tensor) else None,
    ...
    print(json.dumps({'event': 'v48_79_state_isolation', 'valid': valid, 'output': str(a.output)}))
