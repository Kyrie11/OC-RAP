...
import torch
ALLOWED_KEY='direct_absolute_semantic_witness_gain'; EXPECTED_FEATURE_SCHEMA=4; EXPECTED_FEATURE_SOURCE='robust_trust_projected_recovery_witness'
def load(p):
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--reference',type=Path,required=True);ap.add_argument('--adapted',type=Path,required=True);ap.add_argument('--fidelity',default='false');ap.add_argument('--occupancy',default='false');ap.add_argument('--output',type=Path,required=True);a=ap.parse_args()
 ra=load(a.reference);da=load(a.adapted);rs=ra.get('model_state',ra);ds=da.get('model_state',da);shared=sorted(set(rs)&set(ds));changed=[];shape=[]
 for k in shared:
  if tuple(rs[k].shape)!=tuple(ds[k].shape):shape.append(k)
  elif not torch.equal(rs[k].cpu(),ds[k].cpu()):changed.append(k)
 removed=sorted(set(rs)-set(ds));added=sorted(set(ds)-set(rs));new=ds.get(ALLOWED_KEY);expected=added==[ALLOWED_KEY] and isinstance(new,torch.Tensor) and tuple(new.shape)==(2,)
