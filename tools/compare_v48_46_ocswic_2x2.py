#!/usr/bin/env python3
"""Compare v48.46 OC-SWIC A/B/C/D on development/certificate data only.

The report intentionally does not read test roots.  It extracts the exact metrics
needed for the pre-registered OC-SWIC causal readout and recomputes component-level
...
def _v4846_witness(run: Path, variant: str) -> dict[str, Any]:
    root=run / "candidates" / variant
    out={"enabled":False,"stages":[]}
    for name in ("v48_46_witness_obs","v48_46_witness_margin"):
        p=root/name/"V48_46_WITNESS_COMPLETE.json"
        if p.is_file():
...
def _factor_contract(run: Path) -> dict[str, Any]:
    return _json(run / "V48_46_FACTOR_CONTRACT.json")
...
def _attribution_contract(arms: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    expected={
      "A":("global",False),"B":("observation_class",False),
      "C":("global",True),"D":("observation_class",True),
    }
    factor_errors=[]
    for name,arm in arms.items():
        f=arm.get("factor_contract") or {}; sem,seq=expected[name]
        if f.get("arm")!=name or f.get("training_option_execution_semantics")!=sem or f.get("evaluation_option_execution_semantics")!="observation_class" or bool(f.get("sequential_obs_margin_witness"))!=seq or bool(f.get("root_logit_recalibration")):
    ...
    return {"valid":valid,"reference_arm":"A","missing_reference_fields":missing,"mismatched_arms":sorted(mismatches),
            ...
            "meaning":"All arms share paper-consistent observation-class evaluation. B-A = observation-class-aligned training supervision; C-A = staged obs->margin witness; D-B-C+A = interaction, only when valid"}
...
def _arm(run: Path, positive_gain: float) -> dict[str, Any]:
    ...
    return {
        ...
        "witness_precision": _v4846_witness(run, "precision"),
        "witness_balanced": _v4846_witness(run, "balanced"),
        "near": {
...
def main() -> int:
    ...
    report = {
        "schema": "v48.46-ocswic-2x2-comparison-v1",
