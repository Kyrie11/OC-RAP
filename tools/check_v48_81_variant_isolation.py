...
def main() -> int:
    ...
    ap.add_argument('--sitc-run', type=Path, required=True)
    ...
    for v in ('balanced', 'precision'):
        ...
        state = base / 'V48_81_STAGE_I_STATE_ISOLATION.json'
        ...
        checks = {
            ...
            'truth_contract_checkpoint': str(st.get('absolute_feasibility_truth_contract')) == 'switch_inverse_interval_bounds',
            'objective_checkpoint': str(st.get('absolute_feasibility_supervision_objective')) == 'signed_margin_interval_huber',
            'truth_index_checkpoint': res(st.get('absolute_feasibility_truth_index', '')) == expected_idx,
            'truth_contract_summary': str(tcfg.get('direct_value_absolute_feasibility_truth_contract')) == 'switch_inverse_interval_bounds',
            'objective_summary': str(tcfg.get('direct_value_absolute_feasibility_supervision_objective')) == 'signed_margin_interval_huber',
            'truth_index_summary': res(tcfg.get('direct_value_absolute_feasibility_truth_index', '')) == expected_idx,
            'truth_contract_policy': pe.get('ABSOLUTE_FEASIBILITY_TRUTH_CONTRACT') == 'switch_inverse_interval_bounds',
            'objective_policy': pe.get('ABSOLUTE_FEASIBILITY_SUPERVISION_OBJECTIVE') == 'signed_margin_interval_huber',
        ...
        variants[v] = {'valid': ok, 'checks': checks, 'reference_sha256': refsha, 'sitc_checkpoint_sha256': dstsha, 'root_tail_source_scale': st.get('raw_root_tail_source_scale')}
        paths.append(res(dst))
    distinct = len(paths) == 2 and len(set(paths)) == 2
    if not distinct: errors.append('balanced/precision SITC checkpoints not distinct')
    valid = not errors and all(z.get('valid') for z in variants.values()) and distinct
    doc = {'schema': 'ocrap-v48.81-sitc-variant-isolation-v1', 'valid': valid,
           'reference_run': res(a.reference_run), 'sitc_run': res(a.sitc_run),
           'absolute_feasibility_truth_contract': 'switch_inverse_interval_bounds',
