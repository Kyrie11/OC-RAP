#!/usr/bin/env python3
"""Compare v48.51 DCP-DRFC-BCDE A/B/C/D on development/certificate data only.
...
def _attribution_contract(arms: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for name,arm in arms.items():
        f=arm.get("factor_contract") or {}; bc_frontier,bc_adv=expected[name]
        if (f.get("arm")!=name or f.get("training_option_execution_semantics")!="observation_class"
            ...
            or bool(f.get("native_certificate_preservation")) is not True
            or bool(f.get("recovery_frontier_calibration")) is not True
            or bool(f.get("native_margin_complete_preservation")) is not False
            or bool(f.get("native_advantage_preservation")) is not True
            or bool(f.get("decision_equivalent_frontier")) is not False
            or bool(f.get("native_exact_advantage_preservation")) is not False
            or bool(f.get("boundary_complete_frontier"))!=bc_frontier
            or bool(f.get("native_boundary_complete_advantage_preservation"))!=bc_adv
            or f.get("boundary_complete_deadband_source")!="native_positive_gain"
            or f.get("new_tuned_thresholds") is not False
    ...
    return {"valid":valid,"reference_arm":"A","missing_reference_fields":missing,"mismatched_arms":sorted(mismatches),
            ...
            "meaning":"All arms share the validated v48.50-A / v48.49-C NCP+DRFC+smooth-NAP reference with MC-NCP and exact-only NAP disabled. B-A = boundary-complete frontier sign/order decomposition; C-A = boundary-complete material-sign/smooth-deadband NAP; D-B-C+A = interaction, only when valid."}
...
def main() -> int:
    ...
    report = {
        "schema": "v48.51-dcp-drfc-bcde-2x2-comparison-v1",
