#!/usr/bin/env python3
"""Select a v45 OC-RAVE checkpoint only after verified risk and offline-use gates."""
...
def infer_offline(base: Path) -> Path:
    candidates = [Path(str(base) + "_offline"), Path(str(base) + "_v45_offline"), base.parent / (base.name + "_rava_offline")]
    return next((p for p in candidates if p.exists()), candidates[0])


def find_eval(offline: Path, bucket: str) -> dict[str, Any] | None:
    for name in (f"eval_{bucket}_v45_v45.json", f"eval_{bucket}_v45.json"):
        r = method_result(offline / name)
        if r is not None:
            return r
    matches = sorted(offline.glob(f"eval_{bucket}_*v45*.json"))
...
def evaluate(base: Path, offline: Path | None = None) -> tuple[list[str], float, dict[str, Any]]:
    ...
    for bucket in ("near", "contact"):
        c = load(base / "calibration" / f"direct_value_risk_{bucket}_v45.json")
        if not c:
            failures.append(f"{bucket}: missing v45 risk calibration")
            continue
        ...
        diag[f"{bucket}_cal"] = {
            ...
            "harmful_group_exposure_ucb90": harm_ucb, "pred_teacher_advantage_correlation": corr,
    ...
    if any(r is None for r in results.values()):
        failures.append("missing one or more v45 offline evaluations")
