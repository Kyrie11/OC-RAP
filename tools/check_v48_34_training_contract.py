...
def main() -> int:
    ap = argparse.ArgumentParser(description="Fail-closed v48.34 eligible-set policy training contract audit")
    ...
    ap.add_argument("--expect-prior-mode", default="barrier_gated_slack")
    ap.add_argument("--expect-context-source", default="relative")
    ...
    ap.add_argument("--expect-opportunity-threshold", type=float, default=0.65)
    ap.add_argument("--expect-harm-threshold", type=float, default=0.30)
    ...
    doc = {
        "event": "v48_34_training_contract_audit",
        "run": str(args.run),
