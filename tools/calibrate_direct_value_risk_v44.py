#!/usr/bin/env python3
"""Fit/verify v44 OC-RAVA opportunity-gated selective recovery certificates.
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Fit/verify v44 OC-RAVA opportunity-gated risk certificate.")
    ...
    for (scene, time_index), items in raw_groups.items():
        ...
        for r in recs:
            ...
            pairs.append({
                ...
                "opportunity": float(r["opportunity"]), "teacher_adv": teacher_adv,
            })
    ...
    result = {
        "method": "oc_rava_observation_only_opportunity_gated_selective_certificate",
        "bucket": args.bucket,
    ...
    if args.rows_output is not None:
        ...
        with args.rows_output.open("w") as f:
            for row in all_top1:
                out = dict(row)
                out["challenged"] = bool(float(row["pred_adv"]) >= score_thr)
