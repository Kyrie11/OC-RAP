...
def main() -> int:
    ...
    ap.add_argument('--pistc-run', type=Path, required=True)
    ...
    for v in ('balanced', 'precision'):
        ...
        state = base / 'V48_80_STAGE_I_STATE_ISOLATION.json'
        ...
        checks = {
            ...
            'truth_contract_checkpoint': str(st.get('absolute_feasibility_truth_contract')) == 'structural_interval_bounds',
            ...
            'truth_contract_summary': str(tcfg.get('direct_value_absolute_feasibility_truth_contract')) == 'structural_interval_bounds',
            ...
            'truth_contract_policy': pe.get('ABSOLUTE_FEASIBILITY_TRUTH_CONTRACT') == 'structural_interval_bounds',
        ...
        variants[v] = {'valid': ok, 'checks': checks, 'reference_sha256': refsha, 'pistc_checkpoint_sha256': dstsha, 'root_tail_source_scale': st.get('raw_root_tail_source_scale')}
        paths.append(res(dst))
    distinct = len(paths) == 2 and len(set(paths)) == 2
    if not distinct: errors.append('balanced/precision PISTC checkpoints not distinct')
    valid = not errors and all(z.get('valid') for z in variants.values()) and distinct
    doc = {'schema': 'ocrap-v48.80-pistc-variant-isolation-v1', 'valid': valid,
           'reference_run': res(a.reference_run), 'pistc_run': res(a.pistc_run),
