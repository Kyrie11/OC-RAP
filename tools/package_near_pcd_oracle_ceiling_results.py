...
def main() -> None:
    ...
    doc={'schema':'ocrap-v48.124.10.4-pcd-oracle-result-bundle-v1','root':str(root),'pipeline_exit_code':int(a.exit_code),'complete_exit_zero':int(a.exit_code)==0,'num_files':len(entries),'files':entries}
    ...
    os.replace(tmp,out)
    print(json.dumps({'output':str(out),'manifest':str(man),'num_files':len(entries)+1,'pipeline_exit_code':int(a.exit_code)},indent=2))
