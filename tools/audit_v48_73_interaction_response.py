...
def summarize(base,new):
 ...
 return out
def main():
 ap=argparse.ArgumentParser(description='v48.73 temporal interaction-response reachability row-level audit');ap.add_argument('--m72',type=Path,required=True);ap.add_argument('--n73',type=Path,required=True);ap.add_argument('--o73',type=Path,required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();doc={'schema':'ocrap-v48.73-irrw-interaction-response-audit-v1','test_roots_read':False,'dataset_reconstruction':False,'comparisons':{}}
 for v in VARIANTS:
  doc['comparisons'][v]={}
  for k in KINDS:
 ...
 a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_73_irrw_interaction_response_audit','output':str(a.output)}))
