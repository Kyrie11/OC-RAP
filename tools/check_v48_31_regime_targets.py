...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Check v48.31 Near/Contact physical targets and paired frontier diagnostics."
    )
    ...
    for metric, op, threshold in (
        ...
        ("near_contact_exposure_episode_count", "<=", 0.0),
        ("near_contact_longest_exposure_run_s", "<=", 0.0),
        ("critical_ttc_longest_exposure_run_s", "<=", 0.0),
        ("clearance_recovery_gain_m", ">=", 0.0),
        ("ttc_recovery_gain_s", ">=", 0.0),
        ("terminal_clearance_m", ">=", 0.0),
        ("terminal_ttc_s", ">=", 0.0),
    ...
    for metric, op, threshold in (
        ...
        ("recontact_event", "<=", -0.02),
        ("recontact_episode_count", "<=", 0.0),
        ("overlap_duration_s", "<=", 0.0),
        ("longest_overlap_run_s", "<=", 0.0),
        ("post_contact_clearance_m_max", ">=", 0.0),
        ("post_contact_terminal_clearance_m", ">=", 0.0),
        ("post_contact_clearance_gain_m", ">=", 0.0),
        ("post_contact_free_space_auc_m_s", ">=", 0.0),
        ("post_contact_free_space_auc_normalized_m", ">=", 0.0),
        ("post_contact_clearance_deficit_auc_m_s", "<=", 0.0),
        ("post_contact_escape_event", ">=", 0.02),
        ("time_to_post_contact_escape_s", "<=", 0.0),
        ("new_stable_stop_event", ">=", 0.02),
        ("new_stable_stop_quality_event", ">=", 0.02),
        ("time_to_stable_stop_quality_s", "<=", 0.0),
    ):
