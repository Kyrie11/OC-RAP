#!/usr/bin/env python3
"""Select a v46 OC-RACE checkpoint only after verified risk and offline-use gates."""
...
def infer_offline(base: Path) -> Path:
    candidates = [Path(str(base) + "_offline"), Path(str(base) + "_v46_offline"), base.parent / (base.name + "_rava_offline")]
    return next((p for p in candidates if p.exists()), candidates[0])


def find_eval(offline: Path, bucket: str) -> dict[str, Any] | None:
    for name in (f"eval_{bucket}_v46_v46.json", f"eval_{bucket}_v46.json"):
        r = method_result(offline / name)
        if r is not None:
            return r
    matches = sorted(offline.glob(f"eval_{bucket}_*v46*.json"))
...
def evaluate(base: Path, offline: Path | None = None) -> tuple[list[str], float, dict[str, Any]]:
    ...
    for bucket in ("near", "contact"):
        c = load(base / "calibration" / f"direct_value_risk_{bucket}_v46.json")
        if not c:
            failures.append(f"{bucket}: missing v46 risk calibration")
            continue
        ...
        diag[f"{bucket}_cal"] = {
            "contract_mode": c.get("contract_mode"), "active_valid": valid, "score_threshold": score_thr, "opportunity_threshold": opp_thr,
    ...
    if any(r is None for r in results.values()):
        failures.append("missing one or more v46 offline evaluations")
