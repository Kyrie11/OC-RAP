#!/usr/bin/env python3
"""Compare the v48.52 PSA A/B axis on development/certificate data only.

A is either a fail-closed validated reuse of v48.51-B or a fresh q-proxy BC-FC
reference. B changes only teacher-side Physical Sign Alignment. The report never
...
def _factor_contract(run: Path) -> dict[str, Any]:
    p52 = run / "V48_52_FACTOR_CONTRACT.json"
    if p52.is_file():
        return _json(p52)
    return _json(run / "V48_51_FACTOR_CONTRACT.json")
...
def _attribution_contract(arms: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    factor_errors=[]
    a=arms["A"].get("factor_contract") or {}; b=arms["B"].get("factor_contract") or {}

    def common_ok(f):
        return (
            f.get("training_option_execution_semantics")=="observation_class"
            and f.get("evaluation_option_execution_semantics")=="observation_class"
            and bool(f.get("native_certificate_preservation")) is True
            and bool(f.get("recovery_frontier_calibration")) is True
            and bool(f.get("native_margin_complete_preservation")) is False
            and bool(f.get("native_advantage_preservation")) is True
            and bool(f.get("decision_equivalent_frontier")) is False
            and bool(f.get("boundary_complete_frontier")) is True
            and bool(f.get("native_exact_advantage_preservation")) is False
            and bool(f.get("native_boundary_complete_advantage_preservation")) is False
            and bool(f.get("decision_weighted_observation")) is False
            and bool(f.get("root_logit_recalibration")) is False
            and f.get("strategy_regime_conditioning") is False
            and f.get("test_roots_read") is False
            and int(f.get("proposal_top_k",-1))==5
        )

    if not common_ok(a): factor_errors.append("A")
    if not common_ok(b): factor_errors.append("B")
    a_v=str(a.get("version", ""))
    a_is_historical=(a_v=="v48.51-DCP-DRFC-BCDE" and a.get("arm")=="B")
    a_is_fresh=(a_v=="v48.52-DCP-DRFC-BCDE-PSA" and a.get("arm")=="A" and a.get("physical_teacher_sign_alignment") is False)
    if not (a_is_historical or a_is_fresh): factor_errors.append("A_reference_identity")
    if not (b.get("version")=="v48.52-DCP-DRFC-BCDE-PSA" and b.get("arm")=="B" and b.get("physical_teacher_sign_alignment") is True):
        factor_errors.append("B_psa_identity")
    if a_is_historical and bool(a.get("physical_teacher_sign_alignment", False)) is not False:
        factor_errors.append("A_historical_psa_should_be_false")
    if b.get("teacher_sign_coordinate") != "q_selected_mstar_physical_drs_exact_pcd":
        factor_errors.append("B_teacher_sign_coordinate")
    if b.get("frontier_order_coordinate") != "smooth_boundary_drs_smooth_pcd":
        factor_errors.append("B_order_coordinate")

    valid=(not missing) and not mismatches and not factor_errors and all(a_.get("pipeline_valid") is True and a_.get("authoritative_exit_code") in (0,20) for a_ in arms.values())
    return {"valid":valid,"reference_arm":"A","reference_source":"v48.51-B-reuse" if a_is_historical else "fresh-v48.52-A",
            "missing_reference_fields":missing,"mismatched_arms":sorted(mismatches),
            "factor_contract_errors":sorted(set(factor_errors)),"identities":identities,
            "meaning":"A is the validated BC-FC + smooth-NAP q-proxy reference (historical v48.51-B or byte/protocol-compatible fresh v48.52-A). B-A isolates Physical Sign Alignment: teacher q selects the legal observation-consistent recovery action, while m_star>=0 defines physical teacher root success. No regime-conditioned policy is introduced."}
...
def main() -> int:
    ...
    report = {
        "schema": "v48.52-dcp-drfc-bcde-psa-ab-comparison-v1",
