...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--reference',type=Path,required=True);ap.add_argument('--adapted',type=Path,required=True);ap.add_argument('--tail-localization',required=True);ap.add_argument('--output',type=Path,required=True);a=ap.parse_args();tail=b(a.tail_localization)
    ...
    expected={'semantic_correction':True,'active_set_alignment':True,'path_stop_alignment':False,'classlocal_transport':False,'route_alignment':True,'reentry_alignment':True,'control_projection':True,'boundary_transport':False,'projection_fidelity':False,'active_constraint_typed_source':False,'root_tail_source':True,'tail_localization':tail}
    cfg=da.get('cfg') or {};tcfg=cfg.get('training') or {};truth=str(tcfg.get('direct_value_absolute_feasibility_truth_contract','legacy_full'));obj=str(tcfg.get('direct_value_absolute_feasibility_supervision_objective','binary_sign'))
    valid=(not removed and not shape and not changed and new_ok and feature_ok and flags==expected and truth=='censor_exact_0p5' and obj=='signed_margin_huber')
    doc={'schema':'ocrap-v48.78-rtsi-state-isolation-v1','valid':valid,'reference':str(a.reference),'adapted':str(a.adapted),'reference_sha256':sha(a.reference),'adapted_sha256':sha(a.adapted),'shared_state_tensors':len(shared),'changed_shared_state_keys':changed,'added_state_keys':added,'removed_state_keys':removed,'shape_mismatch_keys':shape,'new_tensor_shape':list(w.shape) if isinstance(w,torch.Tensor) else None,'new_tensor_numel':int(w.numel()) if isinstance(w,torch.Tensor) else None,'raw_root_tail_source_scale':w.detach().cpu().float().tolist() if isinstance(w,torch.Tensor) else None,'semantic_witness_feature_schema':schema,'semantic_witness_feature_source':source,'semantic_witness_feature_contract_valid':feature_ok,'factor_flags':flags,'factor_flags_valid':flags==expected,'absolute_feasibility_truth_contract':truth,'absolute_feasibility_supervision_objective':obj,'stage_i_bitwise_identity':not changed and not removed and not shape,'tail_localization':tail,'dataset_reconstruction':False,'test_roots_read':False}
    a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n');print(json.dumps({'event':'v48_78_state_isolation','valid':valid,'output':str(a.output)}));return 0 if valid else 30
