...
import torch
ALLOWED_KEY='direct_absolute_semantic_witness_gain'; EXPECTED_FEATURE_SCHEMA=3; EXPECTED_FEATURE_SOURCE='projected_boundary_common_executable_recovery_witness'
def load(p):
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--reference',type=Path,required=True);ap.add_argument('--adapted',type=Path,required=True);ap.add_argument('--projection',default='false');ap.add_argument('--boundary',default='false');ap.add_argument('--output',type=Path,required=True);a=ap.parse_args()
 ...
 feature_ok=schema==3 and source==EXPECTED_FEATURE_SOURCE;flags_ok=flags=={'semantic_correction':True,'active_set_alignment':True,'path_stop_alignment':False,'classlocal_transport':False,'route_alignment':True,'reentry_alignment':True,'control_projection':b(a.projection),'boundary_transport':b(a.boundary)}
