...
# Exact scientific-execution artifact from the completed V48.124.8
# contact-anchored run.  This pins the code that actually produced the
# evidence, rather than requiring a later worktree (which may contain
# submission/external-baseline changes) to remain byte-identical forever.
FROZEN_V481248_FULL_RUN_ENGINEERING_VERSION = "v48.124.8-OC-FMSA-CONTACT-ANCHOR-ENGFIX"
FROZEN_V481248_FULL_RUN_RUNTIME_SHA256 = "16a869c6d790f209964dd8db03337deb739fe7e75937ba0f3f04dcfb14ea71a5"
FROZEN_V481248_SOURCE_ZIP_SHA256 = "21fac1eb5024d3b7e925f9537bfadebb5f356abe95b4e13e3937f9c7a4de85cc"
...
def full_run_runtime_contract_ok(path: Path, doc: dict[str, Any]) -> bool:
    ...
    V48.124.8 may adjudicate either the exact retained V48.124.5 population
    ...
    artifact_sha = sha(path) if path.is_file() else ""
    retained_v481245 = (
        full_run_engineering == FROZEN_FULL_RUN_ENGINEERING_VERSION
        and artifact_sha == FROZEN_FULL_RUN_RUNTIME_SHA256
    )
    retained_v481248 = (
        full_run_engineering == FROZEN_V481248_FULL_RUN_ENGINEERING_VERSION
        and artifact_sha == FROZEN_V481248_FULL_RUN_RUNTIME_SHA256
    )
    ...
    base_contract = bool(
        ...
        and (retained_v481245 or retained_v481248 or fresh_current)
    ...
    if not base_contract:
        return False
    if retained_v481248 or fresh_current:
        return bool(
            sc_full.get("contact_endpoint_anchor_contract")
            == "same_target_observed_simulator_contact_anchor_at_rollout_step_0_before_policy_action"
            and sc_full.get("counterfactual_contact_surrogate_not_sufficient_for_post_contact_gate") is True
            and sc_full.get("contact_anchor_construction") == "exact_a0_pretreatment_prelude_v1"
            and sc_full.get("contact_anchor_manifest_scene_disjoint") is True
            and sc_full.get("contact_anchor_state_fingerprint_required") is True
        )
    return True
...
def main() -> int:
    ...
    ap.add_argument("--contact-anchor-manifest", type=Path, required=False)
    ...
    try:
        ...
        full_engineering = str(full_run_runtime.get("engineering_version") or "")
        if full_engineering == FROZEN_FULL_RUN_ENGINEERING_VERSION:
            for rel, want in FROZEN_FULL_RUN_CORE_SHA256.items():
                if (rows.get(rel) or {}).get("sha256") != want:
                    errors.append(f"full_run_core_sha:{rel}")
        elif full_engineering == FROZEN_V481248_FULL_RUN_ENGINEERING_VERSION:
            # The completed V48.124.8 run is an immutable scientific-execution
            # snapshot.  Its exact runtime-contract SHA already commits to every
            # recorded active source hash.  A later worktree may legitimately
            # diverge for external-baseline/submission engineering and must not
            # retroactively invalidate the completed run.
            if sha(a.full_run_runtime) != FROZEN_V481248_FULL_RUN_RUNTIME_SHA256:
                errors.append("v481248_full_run_runtime_sha")
        elif full_engineering == ENGINEERING_VERSION:
            # A genuinely fresh current-build run must still be produced by the
            # same checkout that performs adjudication.
            for rel, row in rows.items():
                path = Path(str((row or {}).get("path") or ""))
                want = str((row or {}).get("sha256") or "")
                if not path.is_file() or not want or sha(path) != want:
                    errors.append(f"fresh_full_run_runtime_sha:{rel}")
    except Exception as exc:
    ...
    contact_anchor_manifest: dict[str, Any] = {}
    if a.contact_anchor_manifest is not None:
        try:
            contact_anchor_manifest = load(a.contact_anchor_manifest)
            artifacts["contact_anchor_manifest"] = artifact_record(a.contact_anchor_manifest)
            if not (contact_anchor_manifest.get("valid") and contact_anchor_manifest.get("schema") == "ocrap-contact-anchor-manifest-v1"):
                errors.append("contact_anchor_manifest_contract")
        except Exception as exc:
            errors.append(f"contact_anchor_manifest_parse:{type(exc).__name__}")
    try:
        for regime in ("safe", "near", "contact"):
            for variant in ("nominal", "balanced", "precision"):
    ...
    provenance = {
        ...
        "full_population_runtime": {
            ...
            "historical_frozen_core_sha256": FROZEN_FULL_RUN_CORE_SHA256 if str(full_run_runtime.get("engineering_version") or "") == FROZEN_FULL_RUN_ENGINEERING_VERSION else None,
            "retained_v481248_runtime_sha256": FROZEN_V481248_FULL_RUN_RUNTIME_SHA256 if str(full_run_runtime.get("engineering_version") or "") == FROZEN_V481248_FULL_RUN_ENGINEERING_VERSION else None,
            "retained_v481248_source_zip_sha256": FROZEN_V481248_SOURCE_ZIP_SHA256 if str(full_run_runtime.get("engineering_version") or "") == FROZEN_V481248_FULL_RUN_ENGINEERING_VERSION else None,
            "current_worktree_bytes_required": str(full_run_runtime.get("engineering_version") or "") == ENGINEERING_VERSION,
        },
        "contact_anchor_manifest": {
            "artifact": artifact_record(a.contact_anchor_manifest) if a.contact_anchor_manifest is not None and a.contact_anchor_manifest.is_file() else None,
            "schema": contact_anchor_manifest.get("schema"),
            "valid": contact_anchor_manifest.get("valid"),
            "selection_policy": contact_anchor_manifest.get("selection_policy"),
            "pre_treatment_policy": contact_anchor_manifest.get("pre_treatment_policy"),
            "num_selected_anchors": contact_anchor_manifest.get("num_selected_anchors"),
            "num_selected_scenes": contact_anchor_manifest.get("num_selected_scenes"),
        },
        "womd_sources": {
            regime: {
                variant: {
