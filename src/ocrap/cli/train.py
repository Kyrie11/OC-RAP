...
def _epoch(model: OCRAPModel, loader: DataLoader, cfg: dict, device: torch.device, optimizer: torch.optim.Optimizer | None = None) -> dict[str, float]:
    ...
    for batch in loader:
        ...
        if training:
            ...
            total.backward()
            grad_clip = float((cfg.get("training", {}) or {}).get("grad_clip", 5.0))
            if grad_clip > 0:
    ...
    return {k: float(v / max(n, 1)) for k, v in totals.items()} | {"num_samples": int(n)}
...
def train(dataset: str, output: str, cfg: dict) -> dict:
    ...
    # Infer label geometry from the first sample; this keeps the model aligned
    # with datasets built with num_recovery_options=12, 24, etc.
    first = train_ds[0]
    ...
    for ep in range(1, epochs + 1):
        tr = _epoch(model, train_loader, cfg, device, opt)
        with torch.no_grad():
            va = _epoch(model, val_loader, cfg, device, None)
        ...
        if va.get("loss", float("inf")) <= best_val:
            ...
            torch.save({
                ...
                "epoch": ep,
                "val_loss": best_val,
                "note": "OC-RAP compact neural checkpoint: predicts root probabilities, recovery margins, and observation compatibility from scene-prefix features.",
            }, best_path)
    result = {
