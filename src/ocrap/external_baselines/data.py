...
def use_teacher_branch_context(cfg: dict[str, Any]) -> bool:
    ...
    The legacy default remains ``True`` so old checkpoints retain their original
    input contract; new GameFormer configs explicitly disable it.
    """
    ...
    return bool(mcfg.get("use_teacher_branch_context", True))
...
def _history_arrays(d: dict[str, Any], cfg: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """GameFormer-style ego/neighbor history and candidate-prefix trajectory.

    The uploaded GameFormer source encodes ego/neighbor history with AgentEncoder
    LSTMs and decodes multi-modal future trajectories.  OC-RAP does not store the
    native GameFormer npz schema, so we expose equivalent tensors from the OC-RAP
    sample: agent_history for actor sequences and prefix_states for the ego
    candidate trajectory that acts as the imitation target.
    """
    ...
    ego_state = np.asarray(d.get("ego_state", np.zeros((GAMEFORMER_STATE_DIM,))), dtype=np.float32).reshape(-1)
    ...
    if hist.ndim == 3 and hist.shape[0] > 0:
        ...
        hpad = np.zeros((H, hist.shape[1], hist.shape[2]), dtype=np.float32)
        vpad = np.zeros((H, hist.shape[1]), dtype=bool)
        hpad[-hsel.shape[0] :] = hsel
        vpad[-vsel.shape[0] :] = vsel
        if hpad.shape[1] > 0:
            ego_raw = hpad[:, 0, :GAMEFORMER_STATE_DIM]
            ego_hist[:, : ego_raw.shape[1]] = ego_raw
        for a in range(min(A, max(hpad.shape[1] - 1, 0))):
            raw = hpad[:, a + 1, :GAMEFORMER_STATE_DIM]
            neigh_hist[a, :, : raw.shape[1]] = raw
            neigh_valid[a] = vpad[:, a + 1]
    else:
        m = min(GAMEFORMER_STATE_DIM, ego_state.size)
        ego_hist[:, :m] = ego_state[:m]

    prefix_xy, prefix_valid = _resample_xy(_ego_prefix_xy(d), T)
    return (
        np.nan_to_num(ego_hist, nan=0.0, posinf=0.0, neginf=0.0),
        np.nan_to_num(neigh_hist, nan=0.0, posinf=0.0, neginf=0.0),
        neigh_valid.astype(bool),
        prefix_xy,
        prefix_valid.astype(bool),
    )
...
def _target_index(samples: list[dict[str, Any]], baseline: str, cfg: dict[str, Any]) -> int:
    ...
    if baseline in {"nominal", "nominal_replay", "log_replay", "route_bc", "route_bc_lite", "waymax_bc", "waymax_bc_lite", "wayformer_bc", "wayformer_style_bc", "route_bc_wayformer"}:
    ...
    elif baseline in {"betop", "betop_lite", "betopnet", "betopnet_lite"}:
        topo_values = []
        for d in samples:
            arr = _actor_topology_arrays(d, cfg)[1]
            topo_values.append(float(arr.max() if arr.size else 0.0))
        topo_hit = np.asarray(topo_values, dtype=np.float32)
        score = utility - float(bcfg.get("hard_weight", 12.0)) * hard - float(bcfg.get("harm_weight", 2.0)) * harm + float(bcfg.get("oracle_weight", 0.6)) * r_orc + float(bcfg.get("topology_weight", 0.5)) * topo_hit
