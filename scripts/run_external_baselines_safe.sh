...
if [[ "${CL_WOMD,,}" == auto ]]; then
  CL_WOMD="$(runtime_resolve_bucket_womd_spec "$CL_BUCKET_DATASET" "$CL_BUCKET_SPLIT" "$WOMD_ROOT" "$WOMD_NUM_SHARDS" "${CL_WOMD_ROLE:-validation}")"
else
...
if runtime_bool_true "$RUN_LEGACY_SAFE"; then
  SPECS+=(
...
SAFE_NONLEARNING_SPECS="pdm_closed=configs/external_baselines/pdm_closed.yaml,pdm_hybrid=configs/external_baselines/pdm_hybrid.yaml,idm=configs/external_baselines/idm.yaml"
if runtime_bool_true "$DO_TRAIN"; then
  if ! runtime_bool_true "$FORCE_REREGISTER" && python tools/check_external_nonlearning_registration.py \
      --root "$CHECKPOINT_ROOT" --dataset "$TRAIN_SAFE" --val-dataset "$VAL_SAFE" --specs "$SAFE_NONLEARNING_SPECS" >/dev/null 2>&1; then
    echo "[REUSE] Safe non-learning baseline registrations are compatible"
  else
    python -u tools/register_external_nonlearning_baselines.py \
      --dataset "$TRAIN_SAFE" --val-dataset "$VAL_SAFE" \
      --specs "$SAFE_NONLEARNING_SPECS" --output-root "$CHECKPOINT_ROOT" \
...
prepare_or_offline_method() {
  ...
  if [[ "$kind" == learned ]]; then
    ...
    if runtime_bool_true "$FORCE_RETRAIN_SAFE" || ! checkpoint_valid "$ckpt" "$expected_impl" "$config"; then
      if ! runtime_bool_true "$DO_TRAIN"; then
      ...
      checkpoint_valid "$ckpt" "$expected_impl" "$config" || { echo "Training produced an invalid checkpoint: $ckpt" >&2; return 2; }
    else
...
run_queue_dynamic() {
  ...
  for ((i=0; i<MAX_PARALLEL && next<${#items[@]}; i++)); do
    launch_one "${items[$next]}" "${GPU_SLOTS[$i]}"; next=$((next+1))
  done
  while ((active>0)); do
...
run_queue_fixed() {
  ...
  for ((base=0; base<${#items[@]}; base+=MAX_PARALLEL)); do
    ...
    for ((j=0; j<MAX_PARALLEL && base+j<${#items[@]}; j++)); do
      idx=$((base+j)); "$runner" "${items[$idx]}" "${GPU_SLOTS[$j]}" &
...
if runtime_bool_true "$RUN_NOMINAL_CONTROL"; then SUMMARY_METHODS+=(nominal_replay); fi
