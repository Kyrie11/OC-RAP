...
# v26 is a selector-level Guarded-NARC/BMRC correction on top of the v22 model.  By default it
...
export RUN=${RUN:-runs/ocrap_v26_gated_challenge}
...
make_sel() {
  ...
  case "$tag" in
    ...
    v26) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=true; pcd_rescue=true ;;
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v26_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v26_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v26_${tag}.log"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v26.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v26.log"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v26.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v26 =====")
for p in sorted(root.glob('eval_*_v26_*.json')):
...
for p in sorted(root.glob('*v26*.json')):
