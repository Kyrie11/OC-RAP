#!/usr/bin/env bash
# V48.80 OC-PISTC: Partially-Identified Structural Truth Contract.
...
REFERENCE_A="${V4880_REFERENCE_A:-$BASE_OUT/ocrap_v48_56_dcp_drfc_bcde_drac_ablation_A}"
J78_RUN="${V4880_J78:-$BASE_OUT/ocrap_v48_78_dcp_drfc_bcde_rifa_rtsi_main}"
V79_COMPARE="${V4880_V79_COMPARE:-$BASE_OUT/OC-RAP-v48.79-DCP-DRFC-BCDE-RIFA-OC-PSTC-comparison.json}"
V79_COMPLETE="${V4880_V79_COMPLETE:-$BASE_OUT/OC-RAP-v48.79-PIPELINE_COMPLETE.json}"
L_RUN="$BASE_OUT/ocrap_v48_80_dcp_drfc_bcde_rifa_pistc_main"
REF_AUDIT="$BASE_OUT/OC-RAP-v48.80-reference-reuse-contract.json"
RUNTIME_AUDIT="$BASE_OUT/OC-RAP-v48.80-runtime-code-contract.json"
TRAIN_TRUTH_INDEX="$BASE_OUT/OC-RAP-v48.80-train-dev-physical-interval-truth-index.jsonl"
TRAIN_TRUTH_SUMMARY="$BASE_OUT/OC-RAP-v48.80-train-dev-physical-interval-truth-index-summary.json"
EVAL_TRUTH_INDEX="$BASE_OUT/OC-RAP-v48.80-dev-certificate-physical-interval-truth-index.jsonl"
EVAL_TRUTH_SUMMARY="$BASE_OUT/OC-RAP-v48.80-dev-certificate-physical-interval-truth-index-summary.json"
AUDIT="$BASE_OUT/OC-RAP-v48.80-OC-PISTC-audit.json"
COMPARE="$BASE_OUT/OC-RAP-v48.80-DCP-DRFC-BCDE-RIFA-OC-PISTC-comparison.json"
PIPELINE_COMPLETE="$BASE_OUT/OC-RAP-v48.80-PIPELINE_COMPLETE.json"
...
TENSOR_CACHE_DIR="${V4880_TENSOR_CACHE_DIR:-$BASE_OUT/.ocrap_v48_78_tensor_cache}"
mkdir -p "$BASE_OUT" "$TENSOR_CACHE_DIR"
rm -rf "$L_RUN"
rm -f "$REF_AUDIT" "$RUNTIME_AUDIT" "$TRAIN_TRUTH_INDEX" "$TRAIN_TRUTH_SUMMARY" "$EVAL_TRUTH_INDEX" "$EVAL_TRUTH_SUMMARY" "$AUDIT" "$COMPARE" "$PIPELINE_COMPLETE" "$L_RUN.zip" "$BASE_OUT/OC-RAP-v48.80-OC-PISTC-audits.zip"

python tools/check_v48_58_reference_reuse.py --reference-run "$REFERENCE_A" --output "$REF_AUDIT"
python tools/check_v48_80_runtime_code_contract.py --repo "$REPO" --output "$RUNTIME_AUDIT"
python - "$V79_COMPLETE" "$V79_COMPARE" "$REFERENCE_A" "$J78_RUN" <<'PY'
import json,pathlib,sys
complete,compare=map(pathlib.Path,sys.argv[1:3]);runs=[pathlib.Path(x) for x in sys.argv[3:]]
for p in (complete,compare):
    if not p.is_file():raise SystemExit(f'missing V48.79 prerequisite: {p}')
m=json.loads(complete.read_text());p=json.loads(compare.read_text());d=p.get('preregistered_decision') or {}
if not (m.get('valid') and m.get('attribution_ready') and m.get('engineering_version')=='v48.79.0-OC-PSTC' and not m.get('test_roots_read')):raise SystemExit(f'V48.79 pipeline invalid: {m}')
if not (p.get('valid') and p.get('attribution_ready') and d.get('status')=='UNDERPOWERED_TRUTH_ADJUDICATION' and d.get('physical_structural_truth_confound_go') is False and d.get('next_branch')=='report_physical_identifiability_power_limit_no_algorithm_promotion_and_no_dataset_reconstruction'):raise SystemExit(f'V48.79 branch mismatch: {d}')
for r in runs:
...
python tools/build_v48_80_interval_truth_index.py \
  ...
  --alpha 0.20 --beta 0.20 --top-m 8 --workers "${V4880_TRUTH_INDEX_WORKERS:-8}"
...
mkdir -p "$L_RUN/candidates" "$L_RUN/logs"
train_one() {
  local v="$1"; local gpu="$2"; local src="$REFERENCE_A/candidates/$v"; local dst="$L_RUN/candidates/$v"
  mkdir -p "$dst"
  ...
  ABSOLUTE_FEASIBILITY_WEIGHT=1.0 ABSOLUTE_FEASIBILITY_TRUTH_CONTRACT=structural_interval_bounds ABSOLUTE_FEASIBILITY_TRUTH_INDEX="$TRAIN_TRUTH_INDEX" ABSOLUTE_FEASIBILITY_SUPERVISION_OBJECTIVE=signed_margin_interval_huber \
  ...
  EVIDENCE_ADAPT_EPOCHS="${V4880_PISTC_EPOCHS:-20}" EVIDENCE_ADAPT_PATIENCE="${V4880_PISTC_PATIENCE:-5}" EVIDENCE_ADAPT_LR="${V4880_PISTC_LR:-0.001}" MAX_EVIDENCE_CALIBRATOR_PARAMS=1 \
  ...
  OCRAP_ALGORITHM_VERSION="v48.80-DCP-DRFC-BCDE-RIFA-OC-PISTC-L80_INTERVAL_TAIL_PROBE" \
  bash scripts/adapt_ocrap_v48_36_ocaf_single_stage.sh >"$L_RUN/logs/pistc_L80_${v}.log" 2>&1
  grep -qx 'ABSOLUTE_FEASIBILITY_TRUTH_CONTRACT=structural_interval_bounds' "$dst/POLICY_CONTRACT.env" || exit 30
  ...
  python tools/check_v48_80_state_isolation.py --reference "$src/model_v48_trac_sr/best.pt" --adapted "$dst/model_v48_trac_sr/best.pt" --truth-index "$TRAIN_TRUTH_INDEX" --output "$dst/V48_80_STAGE_I_STATE_ISOLATION.json"
}
set +e; train_one balanced "$GPU0" & p0=$!; train_one precision "$GPU1" & p1=$!; wait "$p0"; r0=$?; wait "$p1"; r1=$?; set -e
[[ "$r0" == 0 && "$r1" == 0 ]] || { echo "V48.80 L80 training failed balanced=$r0 precision=$r1" >&2; exit 30; }
python tools/check_v48_80_variant_isolation.py --reference-run "$REFERENCE_A" --reference-contract "$REF_AUDIT" --pistc-run "$L_RUN" --truth-index "$TRAIN_TRUTH_INDEX" --output "$L_RUN/V48_80_VARIANT_ISOLATION.json"
python - "$L_RUN/V48_80_FACTOR_CONTRACT.json" "$TRAIN_TRUTH_SUMMARY" <<'PY'
import json,pathlib,sys,time
p=pathlib.Path(sys.argv[1]); summary=json.loads(pathlib.Path(sys.argv[2]).read_text())
d={'event':'v48_80_factor_contract','version':'v48.80-DCP-DRFC-BCDE-RIFA-OC-PISTC','engineering_version':'v48.80.0-OC-PISTC','arm':'L80_INTERVAL_TAIL_PROBE','stage_i':'bitwise v48.56-A checkpoint','stage_ii':'same J78 nested deployability-tail zero-translation root source','scientific_intervention':'supervision truth contract only: use conservative partially-identified physical R_dep intervals induced by structural floors/overrides; hidden branch overrides remain unsupervised','root_tail_source':True,'tail_localization':True,'source_capacity_changed_vs_J78':False,'trainable_state':'direct_absolute_root_tail_source_scale[1]','expected_trainable_parameters':1,'option_translation_zero_mean':True,'option_id_input':False,'regime_id_input':False,'classlocal_transport':False,'active_constraint_typed_source':False,'projection_fidelity':False,'boundary_transport':False,'absolute_feasibility_truth_contract':'structural_interval_bounds','absolute_feasibility_supervision_objective':'signed_margin_interval_huber','huber_beta':1.0,'truth_index_summary':summary,'teacher_files_modified':False,'teacher_labels_changed':False,'dataset_reconstruction':False,'control_projection':True,'route_alignment':True,'reentry_alignment':True,'threshold':0.5,'threshold_search':False,'proposal_top_k':5,'root_retraining':False,'relative_score_intervention':False,'teacher_future_input_to_model':False,'test_roots_read':False,'persistent_tensor_cache':True,'tensor_cache_reuses_v48_78_feature_key':True,'save_every_epoch':False,'save_latest':False,'created_unix':time.time()}
p.write_text(json.dumps(d,indent=2,sort_keys=True)+'\n')
PY

set +e
OUTPUTDIR="$L_RUN" GPU0="$GPU0" GPU1="$GPU1" V4836_ATTEMPT_ID="v48.80-L80-$(date +%s)" OCRAP_IMPLEMENTATION_VERSION="v48.80.0-OC-PISTC-L80" \
...
bash scripts/calibrate_v48_36_shared_certificate_pool.sh >"$L_RUN/logs/v48_80_L80_certificate_controller.log" 2>&1
rc=$?; set -e; case "$rc" in 0|20) echo "L80 calibration valid evidence RC=$rc";;*) echo "L80 calibration engineering failure RC=$rc" >&2; exit 30;; esac
...
python tools/audit_v48_80_pistc.py --j78 "$J78_RUN" --l80 "$L_RUN" --truth-index "$EVAL_TRUTH_INDEX" --truth-summary "$EVAL_TRUTH_SUMMARY" --output "$AUDIT"
python tools/compare_v48_80_pistc.py --audit "$AUDIT" --v79-comparison "$V79_COMPARE" --output "$COMPARE"

python - "$REF_AUDIT" "$RUNTIME_AUDIT" "$L_RUN" "$TRAIN_TRUTH_SUMMARY" "$EVAL_TRUTH_SUMMARY" "$AUDIT" "$COMPARE" "$PIPELINE_COMPLETE" <<'PY'
import json,pathlib,sys
...
for v in ('balanced','precision'):
 for rel in (f'candidates/{v}/V48_80_STAGE_I_STATE_ISOLATION.json',f'candidates/{v}/model_v48_trac_sr/best.pt'):
  if not (krun/rel).is_file(): errors.append(f'missing {krun/rel}')
for rel in ('V48_80_VARIANT_ISOLATION.json','V48_80_FACTOR_CONTRACT.json','dedicated_recalibration_status.json'):
 if not (krun/rel).is_file(): errors.append(f'missing {krun/rel}')
doc={'schema':'ocrap-v48.80-pistc-pipeline-complete-v1','algorithm_version':'v48.80-DCP-DRFC-BCDE-RIFA-OC-PISTC','engineering_version':'v48.80.0-OC-PISTC','valid':not errors,'attribution_ready':not errors,'errors':errors,'arms':{'L80_INTERVAL_TAIL_PROBE':str(krun),'historical_J78':'historical'},'checkpoint_packaging_required':True,'truth_contract':'structural_interval_bounds','teacher_labels_changed':False,'source_capacity_changed_vs_J78':False,'persistent_tensor_cache':True,'save_every_epoch':False,'save_latest':False,'dataset_reconstruction':False,'test_roots_read':False}
out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(doc,indent=2,sort_keys=True)+'\n'); print(json.dumps({'event':'v48_80_pipeline_complete','valid':not errors,'output':str(out)})); raise SystemExit(0 if not errors else 30)
PY

cd "$BASE_OUT"
b="$(basename "$L_RUN")"; rm -f "$b.zip"; zip -qr "$b.zip" "$b"
zip -qj OC-RAP-v48.80-OC-PISTC-audits.zip "$REF_AUDIT" "$RUNTIME_AUDIT" "$TRAIN_TRUTH_INDEX" "$TRAIN_TRUTH_SUMMARY" "$EVAL_TRUTH_INDEX" "$EVAL_TRUTH_SUMMARY" "$AUDIT" "$COMPARE" "$PIPELINE_COMPLETE" "$V79_COMPLETE" "$V79_COMPARE" "$L_RUN/V48_80_VARIANT_ISOLATION.json" "$L_RUN/V48_80_FACTOR_CONTRACT.json" "$L_RUN/candidates/balanced/V48_80_STAGE_I_STATE_ISOLATION.json" "$L_RUN/candidates/precision/V48_80_STAGE_I_STATE_ISOLATION.json"
echo "v48.80 complete. Upload $b.zip + OC-RAP-v48.80-OC-PISTC-audits.zip; keep both best.pt files in the run zip."
