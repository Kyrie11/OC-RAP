#!/usr/bin/env bash
# V48.87 OC-BARR: Observation-Consistent Bilinear Action-Root Response.
# Preregistered after V48.86 CRSC STOP. Two new arms isolate representation from supervision:
#   U87_BILINEAR_RESPONSE_INTERVAL: bilinear frozen-root x executable-action response + S86 interval objective.
#   V87/Main_BILINEAR_SELECTIVE_RESPONSE: same capacity + T86 selective structural ordering.
# No regime router, no broad encoder/root retrain, no boundary transport, no dataset reconstruction.
...
REFERENCE_A="${V4887_REFERENCE_A:-$BASE_OUT/ocrap_v48_56_dcp_drfc_bcde_drac_ablation_A}"
L80_RUN="${V4887_L80:-$BASE_OUT/ocrap_v48_80_dcp_drfc_bcde_rifa_pistc_main}"
S86_RUN="${V4887_S86:-$BASE_OUT/ocrap_v48_86_dcp_drfc_bcde_rifa_crsc_response}"
T86_RUN="${V4887_T86:-$BASE_OUT/ocrap_v48_86_dcp_drfc_bcde_rifa_crsc_main}"
V86_COMPARE="${V4887_V86_COMPARE:-$BASE_OUT/OC-RAP-v48.86-DCP-DRFC-BCDE-RIFA-OC-CRSC-comparison.json}"
U_RUN="$BASE_OUT/ocrap_v48_87_dcp_drfc_bcde_rifa_barr_response";V_RUN="$BASE_OUT/ocrap_v48_87_dcp_drfc_bcde_rifa_barr_main"
RUNTIME="$BASE_OUT/OC-RAP-v48.87-runtime-code-contract.json";REF="$BASE_OUT/OC-RAP-v48.87-reference-reuse-contract.json";AUDIT="$BASE_OUT/OC-RAP-v48.87-OC-BARR-audit.json";COMPARE="$BASE_OUT/OC-RAP-v48.87-DCP-DRFC-BCDE-RIFA-OC-BARR-comparison.json";COMPLETE="$BASE_OUT/OC-RAP-v48.87-PIPELINE_COMPLETE.json"
CACHE="${V4887_TENSOR_CACHE_DIR:-$BASE_OUT/.ocrap_v48_78_tensor_cache}"
mkdir -p "$BASE_OUT" "$CACHE";rm -rf "$U_RUN" "$V_RUN";rm -f "$RUNTIME" "$REF" "$AUDIT" "$COMPARE" "$COMPLETE"
python tools/check_v48_58_reference_reuse.py --reference-run "$REFERENCE_A" --output "$REF"
python tools/check_v48_87_runtime_code_contract.py --repo "$REPO" --output "$RUNTIME"
python - "$V86_COMPARE" "$L80_RUN" "$S86_RUN" "$T86_RUN" <<'PY'
import json,pathlib,sys
p=json.loads(pathlib.Path(sys.argv[1]).read_text());d=p.get('preregistered_decision') or {}
if not(p.get('valid') and d.get('status')=='COUNTERFACTUAL_RECOVERY_SUPERVISION_STOP' and not d.get('counterfactual_response_supervision_go') and 'observation_action_interaction' in str(d.get('next_branch',''))):raise SystemExit('V48.86 STOP/narrow-interaction prerequisite missing')
for x in sys.argv[2:]:
...
train_one(){
 ...
 RUN="$dst" MODEL_DIR="$dst/model_v48_trac_sr" VARIANT="$v" TRAIN_GPU="$gpu" INIT_CKPT="$src/model_v48_trac_sr/best.pt" TRAIN_MIX="$TRAIN_NEAR,$TRAIN_CONTACT" VAL_MIX="$DEV_NEAR,$DEV_CONTACT" GROUP_INDEX="$REFERENCE_A/evidence_adapt_teacher_pcd_index.jsonl" VAL_GROUP_INDEX="$REFERENCE_A/evidence_adapt_dev_teacher_pcd_index.jsonl" EVIDENCE_TRAINABLE_PREFIXES_OVERRIDE=direct_absolute_action_response_adapter STRICT_INIT_ALLOWED_MISSING_PREFIXES=direct_absolute_action_response_adapter ABSOLUTE_SEMANTIC_WITNESS_CORRECTION=true SEMANTIC_WITNESS_ACTIVE_SET_ALIGNMENT=true SEMANTIC_WITNESS_ROUTE_ALIGNMENT=true SEMANTIC_WITNESS_REENTRY_ALIGNMENT=true SEMANTIC_WITNESS_CONTROL_PROJECTION=true SEMANTIC_WITNESS_BOUNDARY_TRANSPORT=false SEMANTIC_WITNESS_PROJECTION_FIDELITY=false SEMANTIC_WITNESS_ACTIVE_CONSTRAINT_TYPED_SOURCE=false SEMANTIC_WITNESS_ROOT_TAIL_SOURCE=false SEMANTIC_WITNESS_TAIL_LOCALIZATION=false SEMANTIC_WITNESS_STRUCTURED_TAIL_FIELD=false SEMANTIC_WITNESS_SIGNED_TAIL_CHANNELS=false SEMANTIC_WITNESS_COUNTERFACTUAL_TAIL_RESPONSE=false SEMANTIC_WITNESS_ACTION_RESPONSE_ADAPTER=true SEMANTIC_WITNESS_ACTION_RESPONSE_STATE_CONDITIONING=false SEMANTIC_WITNESS_ACTION_ROOT_BILINEAR_INTERACTION=true ABSOLUTE_FEASIBILITY_WEIGHT=1.0 ABSOLUTE_FEASIBILITY_TRUTH_CONTRACT=structural_interval_bounds ABSOLUTE_FEASIBILITY_TRUTH_INDEX="$TRAIN_IDX" ACTION_RESPONSE_TRUTH_INDEX="$RESP_IDX" ABSOLUTE_FEASIBILITY_SUPERVISION_OBJECTIVE="$objective" BEST_METRIC="$best_metric" BEST_METRIC_MIN_DELTA=0.00001 EVIDENCE_ADAPT_EPOCHS="${V4887_EPOCHS:-20}" EVIDENCE_ADAPT_PATIENCE="${V4887_PATIENCE:-5}" EVIDENCE_ADAPT_LR="${V4887_LR:-0.001}" MAX_EVIDENCE_CALIBRATOR_PARAMS=100000 PERSISTENT_TENSOR_CACHE=true PERSISTENT_TENSOR_CACHE_DIR="$CACHE" PERSISTENT_TENSOR_CACHE_BUILD_WORKERS="${PERSISTENT_TENSOR_CACHE_BUILD_WORKERS:-8}" SAVE_EVERY_EPOCH=false SAVE_LATEST=false OCRAP_ALGORITHM_VERSION="v48.87-OC-BARR-$arm" bash scripts/adapt_ocrap_v48_36_ocaf_single_stage.sh >"$run/logs/${arm}_${v}.log" 2>&1
 python tools/check_v48_87_state_isolation.py --reference "$src/model_v48_trac_sr/best.pt" --adapted "$dst/model_v48_trac_sr/best.pt" --truth-index "$TRAIN_IDX" --response-index "$RESP_IDX" --objective "$objective" --output "$dst/V48_87_STAGE_I_STATE_ISOLATION.json"
}
mkdir -p "$U_RUN/candidates" "$V_RUN/candidates"
for v in balanced precision;do
 set +e;train_one U87_BILINEAR_RESPONSE_INTERVAL "$v" "$GPU0" counterfactual_response_interval_huber direct_absolute_counterfactual_response_interval_huber "$U_RUN" & p0=$!;train_one V87_BILINEAR_SELECTIVE_RESPONSE "$v" "$GPU1" counterfactual_selective_response direct_absolute_counterfactual_selective_response_loss "$V_RUN" & p1=$!;wait "$p0";r0=$?;wait "$p1";r1=$?;set -e;[[ $r0 == 0 && $r1 == 0 ]] || exit 30
done
cal(){ local run="$1" tag="$2";set +e;OUTPUTDIR="$run" GPU0="$GPU0" GPU1="$GPU1" V4836_ATTEMPT_ID="v48.87-$tag-$(date +%s)" OCRAP_IMPLEMENTATION_VERSION="v48.87.0-OC-BARR-$tag" CAL_SAFE="$CAL_SAFE" CERT_NEAR="$CERT_NEAR" CERT_CONTACT="$CERT_CONTACT" DEV_NEAR="$DEV_NEAR" DEV_CONTACT="$DEV_CONTACT" ABSOLUTE_FEASIBILITY_MODE=learned ABSOLUTE_FEASIBILITY_THRESHOLD=0.5 OPTION_EXECUTION_SEMANTICS=observation_class HARM_LABEL_MODE=component_veto OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit PROPOSAL_TOP_K=5 bash scripts/calibrate_v48_36_shared_certificate_pool.sh >"$run/logs/v48_87_${tag}_certificate_controller.log" 2>&1;rc=$?;set -e;case $rc in 0|20);;*)exit 30;;esac;}
cal "$U_RUN" U87;cal "$V_RUN" V87
python tools/audit_v48_87_barr.py --l80 "$L80_RUN" --s86 "$S86_RUN" --t86 "$T86_RUN" --u87 "$U_RUN" --v87 "$V_RUN" --truth-index "$EVAL_IDX" --truth-summary "$EVAL_SUM" --output "$AUDIT"
python tools/compare_v48_87_barr.py --audit "$AUDIT" --v86-comparison "$V86_COMPARE" --u-run "$U_RUN" --v-run "$V_RUN" --output "$COMPARE"
python tools/check_v48_87_pipeline_complete.py --u-run "$U_RUN" --v-run "$V_RUN" --runtime "$RUNTIME" --reference "$REF" --audit "$AUDIT" --comparison "$COMPARE" --response-summary "$RESP_SUM" --output "$COMPLETE"
cd "$BASE_OUT";for r in "$U_RUN" "$V_RUN";do b=$(basename "$r");rm -f "$b.zip";zip -qr "$b.zip" "$b";done
zip -qj OC-RAP-v48.87-OC-BARR-audits.zip "$REF" "$RUNTIME" "$RESP_SUM" "$EVAL_SUM" "$AUDIT" "$COMPARE" "$COMPLETE"
echo "V48.87 complete: upload $(basename "$U_RUN").zip + $(basename "$V_RUN").zip + OC-RAP-v48.87-OC-BARR-audits.zip"
