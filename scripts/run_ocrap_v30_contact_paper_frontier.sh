...
# v30 is a contact paper-frontier correction on top of the v22 model.  By default it
...
export RUN=${RUN:-runs/ocrap_v30_contact_paper_frontier}
...
make_sel() {
  ...
  case "$tag" in
    ...
    v30|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=true; pcd_rescue=true; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    --set selection.pcd_rescue_macro_allowlist_by_bucket.contact=${CONTACT_FRONTIER_MACROS:-brake,yield,merge,stabilize}
    --set selection.pcd_rescue_macro_allowlist_by_bucket.test_contact=${CONTACT_FRONTIER_MACROS:-brake,yield,merge,stabilize}
    ...
    --set selection.pcd_rescue_min_pred_pcd_by_bucket.contact=${CONTACT_FRONTIER_MIN_PCD:-0.02}
    --set selection.pcd_rescue_min_pred_pcd_by_bucket.test_contact=${CONTACT_FRONTIER_MIN_PCD:-0.02}
    ...
    --set selection.pcd_rescue_min_pcd_gain_by_bucket.contact=${CONTACT_FRONTIER_MIN_PCD_GAIN:--1.0}
    --set selection.pcd_rescue_min_pcd_gain_by_bucket.test_contact=${CONTACT_FRONTIER_MIN_PCD_GAIN:--1.0}
    ...
    --set selection.pcd_rescue_min_pred_drs_by_bucket.contact=${CONTACT_FRONTIER_MIN_DRS:-0.45}
    --set selection.pcd_rescue_min_pred_drs_by_bucket.test_contact=${CONTACT_FRONTIER_MIN_DRS:-0.45}
    ...
    --set selection.pcd_rescue_min_pred_r_dep_by_bucket.contact=${CONTACT_FRONTIER_MIN_R_DEP:--2.20}
    --set selection.pcd_rescue_min_pred_r_dep_by_bucket.test_contact=${CONTACT_FRONTIER_MIN_R_DEP:--2.20}
    ...
    --set selection.rescue_challenge_min_pred_drs_by_bucket.contact=${CONTACT_CHALLENGE_MIN_DRS:-0.45}
    --set selection.rescue_challenge_min_pred_drs_by_bucket.test_contact=${CONTACT_CHALLENGE_MIN_DRS:-0.45}
    --set selection.rescue_challenge_min_pred_pcd_by_bucket.contact=${CONTACT_CHALLENGE_MIN_PCD:-0.02}
    --set selection.rescue_challenge_min_pred_pcd_by_bucket.test_contact=${CONTACT_CHALLENGE_MIN_PCD:-0.02}
  ...
  if [[ "$is_v27" == "true" ]]; then
    COMMON_SEL+=(
      ...
      --set selection.rescue_challenge_min_pcd_gain_by_bucket.contact=${CONTACT_CHALLENGE_MIN_PCD_GAIN:--1.000}
      --set selection.rescue_challenge_min_pcd_gain_by_bucket.test_contact=${CONTACT_CHALLENGE_MIN_PCD_GAIN:--1.000}
      ...
      --set selection.rescue_challenge_min_improvement_axes_by_bucket.contact=${CONTACT_CHALLENGE_MIN_AXES:-0}
      --set selection.rescue_challenge_min_improvement_axes_by_bucket.test_contact=${CONTACT_CHALLENGE_MIN_AXES:-0}
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v30_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v30_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v30_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v30_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v30.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v30.log"
  assert_json "$RUN/closed_loop_safe_fast_v30.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v30.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v30 =====")
for p in sorted(root.glob('eval_*_v30_*.json')):
...
for p in sorted(root.glob('*v30*.json')):
...
run_eval v30 near_contact 1 &
wait
run_eval scalar contact 0 &
run_eval v30 contact 1 &
wait

# Stress audits. Two GPUs run baseline and v24 concurrently.
run_audit scalar near_contact 0 32 384 &
run_audit v30 near_contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait
run_audit scalar contact 0 32 384 &
run_audit v30 contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait

if [[ "${RUN_NEAR_CHALLENGE_ABLATION:-0}" == "1" ]]; then
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_eval v30 near_contact 0
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_audit v30 near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384}
fi

if [[ "${RUN_DEEP_CONTACT:-0}" == "1" ]]; then
  run_audit v30 contact 0 ${DEEP_CONTACT_TARGETS:-48} ${DEEP_CONTACT_LABELS:-640}
fi
