...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_16_anchor_dedicated_4816}"
...
run_variant(){
  ...
  EVIDENCE_ADAPT_LR="${EVIDENCE_ADAPT_LR:-0.00015}" EVIDENCE_CALIBRATOR_HIDDEN="${EVIDENCE_CALIBRATOR_HIDDEN:-4}" \
  EVIDENCE_CALIBRATOR_SCALE="${EVIDENCE_CALIBRATOR_SCALE:-0.20}" \
  ORDINAL_EVIDENCE_CLASS_BALANCED_WEIGHT="${ORDINAL_EVIDENCE_CLASS_BALANCED_WEIGHT:-1.00}" \
  ...
  ORDINAL_EVIDENCE_TARGET_PROBABILITY="${ORDINAL_EVIDENCE_TARGET_PROBABILITY:-0.60}" \
  EVIDENCE_CALIBRATOR_ANCHOR_WEIGHT="${EVIDENCE_CALIBRATOR_ANCHOR_WEIGHT:-0.10}" \
  ORDINAL_EVIDENCE_HARD_HARM_WEIGHT="${ORDINAL_EVIDENCE_HARD_HARM_WEIGHT:-0.50}" \
  ORDINAL_EVIDENCE_HARD_BENEFIT_WEIGHT="${ORDINAL_EVIDENCE_HARD_BENEFIT_WEIGHT:-0.75}" \
    bash scripts/adapt_ocrap_v48_16_anchor_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?; set -e
  if [[ "$rc" != 0 ]]; then
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  printf '{"event":"v48_16_pipeline_failed","balanced":%s,"precision":%s}\n' "$s0" "$s1" > "$OUTPUTDIR/PIPELINE_FAILED.json"
  exit 30
fi
variants=""
...
doc={'event':'v48_16_anchor_controller_complete','created_unix':time.time(),'source_run':str(source),'protocol_root':str(protocol),'variants':variants,'certificate_exit_code':rc,'gate_evaluated':rc in (0,20),'gate_passed':(root/'NEXT_COMMANDS.txt').is_file(),'pipeline_valid':rc in (0,20),'test_roots_read':False}
(root/'V48_16_COMPLETE.json').write_text(json.dumps(doc,indent=2)+'\n')
PY
exit "$rc"
