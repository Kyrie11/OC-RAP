...
# v25 is a selector-level BMRC+NARC correction on top of the v22 model.  By default it
...
export RUN=${RUN:-runs/ocrap_v25_rescue_challenge}
...
make_sel() {
  ...
  case "$tag" in
    ...
    v25) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=true; pcd_rescue=true ;;
  ...
  COMMON_SEL=(
    ...
    # v25 NARC: a certified brake/PCD rescue may challenge nominal even when
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v25_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v25_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v25_${tag}.log"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v25.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v25.log"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v25.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v25 =====")
for p in sorted(root.glob('eval_*_v25_*.json')):
...
for p in sorted(root.glob('*v25*.json')):
