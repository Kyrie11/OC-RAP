...
OUT="${OUT:?OUT is required; point it to the v48.30 or repair-source dedicated run}"
...
SHADOW_AUDIT_TOP_K="${SHADOW_AUDIT_TOP_K:-3}"
...
python tools/audit_v48_30_shadow_provenance.py \
...
(root/'DIAGNOSTIC_ONLY_NO_PAPER.json').write_text(json.dumps({
 'event':'v48_30_dev_shadow_closed_loop','created_unix':time.time(),
...
run_variant() {
  ...
  python tools/check_v48_30_shadow_runtime_contract.py \
...
doc={'event':'v48_30_dev_shadow_complete','created_unix':time.time(),
