...
# v46 OC-RACE evaluation. The frozen OC-MERO backbone remains unchanged.
...
export RUN=${RUN:-runs/ocrap_v46_race_eval}
export BASE_RUN=${BASE_RUN:-runs/ocrap_v46_race_regime_balanced}
if [[ -z "${CKPT:-}" ]]; then
  CKPT="$BASE_RUN/model_v46_race/best.pt"
fi
if [[ -z "${CAL:-}" ]]; then
  CAL="$BASE_RUN/calibration/calibration_mix_v46.json"
fi
if [[ -z "${GAMMA:-}" ]]; then
  GAMMA="$BASE_RUN/calibration/gamma_rec_by_bucket_v46.json"
fi
...
# v46 reads both an opportunity threshold and a observation-conditioned score
...
s=float(d.get('direct_value_threshold', float('inf')))
o=float(d.get('direct_value_opportunity_threshold', float('inf')))
if not d.get('valid_for_active_contract', False) or not math.isfinite(s) or not math.isfinite(o):
    raise SystemExit(f'invalid OC-RACE certificate: {p}')
name={2:'brake',3:'yield',5:'merge',6:'pull_over',7:'stabilize'}
macros=','.join(name[x] for x in d.get('supported_macro_ids', []) if x in name)
if not macros:
    raise SystemExit(f'OC-RACE certificate has no fit-supported macro family: {p}')
print(s, o, macros)
PYQ
}
read -r _NEAR_SCORE _NEAR_OPP _NEAR_MACROS < <(read_direct_rule "$BASE_RUN/calibration/direct_value_risk_near_v46.json")
read -r _CONTACT_SCORE _CONTACT_OPP _CONTACT_MACROS < <(read_direct_rule "$BASE_RUN/calibration/direct_value_risk_contact_v46.json")
NEAR_DIRECT_THRESHOLD=${NEAR_DIRECT_THRESHOLD:-$_NEAR_SCORE}
NEAR_DIRECT_OPPORTUNITY_THRESHOLD=${NEAR_DIRECT_OPPORTUNITY_THRESHOLD:-$_NEAR_OPP}
NEAR_DIRECT_MACROS=${NEAR_DIRECT_MACROS:-$_NEAR_MACROS}
CONTACT_DIRECT_THRESHOLD=${CONTACT_DIRECT_THRESHOLD:-$_CONTACT_SCORE}
CONTACT_DIRECT_OPPORTUNITY_THRESHOLD=${CONTACT_DIRECT_OPPORTUNITY_THRESHOLD:-$_CONTACT_OPP}
CONTACT_DIRECT_MACROS=${CONTACT_DIRECT_MACROS:-$_CONTACT_MACROS}
export NEAR_DIRECT_THRESHOLD CONTACT_DIRECT_THRESHOLD NEAR_DIRECT_OPPORTUNITY_THRESHOLD CONTACT_DIRECT_OPPORTUNITY_THRESHOLD
export NEAR_DIRECT_MACROS CONTACT_DIRECT_MACROS
echo "direct-value OC-RACE rules: near(score=$NEAR_DIRECT_THRESHOLD,opp=$NEAR_DIRECT_OPPORTUNITY_THRESHOLD,macros=$NEAR_DIRECT_MACROS) contact(score=$CONTACT_DIRECT_THRESHOLD,opp=$CONTACT_DIRECT_OPPORTUNITY_THRESHOLD,macros=$CONTACT_DIRECT_MACROS)"

make_sel() {
  local tag="$1"  # scalar | v46
  ...
  case "$tag" in
    ...
    v46|v40|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=${CONTACT_BRAKE_RESCUE:-false}; pcd_rescue=true; brake_tail=${CONTACT_BRAKE_TAIL:-false}; brake_tail_bypass_pcd=${CONTACT_TAIL_BYPASS_PCD_GAIN:-false}; direct_value=${RUN_DIRECT_VALUE:-true}; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    # v46 OC-RACE: a held-out-verified top-1 value certificate may augment
    ...
    --set selection.direct_value_macro_allowlist_by_bucket.near_contact=${NEAR_DIRECT_MACROS:-merge,stabilize}
    --set selection.direct_value_macro_allowlist_by_bucket.test_near_contact=${NEAR_DIRECT_MACROS:-merge,stabilize}
    --set selection.direct_value_macro_allowlist_by_bucket.contact=${CONTACT_DIRECT_MACROS:-merge,stabilize}
    --set selection.direct_value_macro_allowlist_by_bucket.test_contact=${CONTACT_DIRECT_MACROS:-merge,stabilize}
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v46_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v46_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee -a "$RUN/audit_${b}_selected_topk_v46_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v46_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v46.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    --set closed_loop.resume="$CL_RESUME" \
    --set closed_loop.resume_allow_legacy_partial=true \
    ...
    | tee -a "$RUN/closed_loop_safe_fast_v46.log"
  assert_json "$RUN/closed_loop_safe_fast_v46.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v46.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v46 =====")
for p in sorted(root.glob('eval_*_v46_*.json')):
...
for p in sorted(root.glob('*v46*.json')):
...
if [[ "$RUN_OFFLINE_EVAL" == "1" ]]; then
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    run_eval scalar safe 0 & run_eval v46 safe 1 & wait
    run_eval scalar near_contact 0 & run_eval v46 near_contact 1 & wait
    run_eval scalar contact 0 & run_eval v46 contact 1 & wait
  else
    if [[ "${OFFLINE_SINGLE_GPU:-0}" == "1" ]]; then
      g=${GPU_SAFE:-0}
      run_eval v46 safe "$g"
      run_eval v46 near_contact "$g"
      run_eval v46 contact "$g"
    else
      run_eval v46 safe ${GPU_SAFE:-0} & run_eval v46 near_contact ${GPU_NEAR:-1} & wait
      run_eval v46 contact ${GPU_CONTACT:-0}
    fi
  fi
fi
  ...
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" && "${PAIR_BUCKETS_ON_TWO_GPUS:-1}" == "1" ]]; then
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      (run_audit scalar near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v46 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_NEAR=$!
    fi
    if [[ "$RUN_CONTACT_AUDIT" == "1" ]]; then
      (run_audit scalar contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v46 contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_CONTACT=$!
    fi
    [[ -z "${P_NEAR:-}" ]] || wait "$P_NEAR"
    [[ -z "${P_CONTACT:-}" ]] || wait "$P_CONTACT"
  elif [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    [[ "$RUN_NEAR_AUDIT" != "1" ]] || { run_audit scalar near_contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v46 near_contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
    [[ "$RUN_CONTACT_AUDIT" != "1" ]] || { run_audit scalar contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v46 contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
  else
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      run_audit v46 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} &
      P_NEAR=$!
