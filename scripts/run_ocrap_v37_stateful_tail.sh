...
# v37 is a selector-only stateful-budget residual-tail correction on top of the v33 model. It keeps
...
export RUN=${RUN:-runs/ocrap_v37_stateful_tail_eval}
...
make_sel() {
  ...
  case "$tag" in
    ...
    v37|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=${CONTACT_BRAKE_RESCUE:-false}; pcd_rescue=true; brake_tail=${CONTACT_BRAKE_TAIL:-true}; brake_tail_bypass_pcd=${CONTACT_TAIL_BYPASS_PCD_GAIN:-true}; is_v27=true ;;
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v37_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v37_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v37_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v37_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v37.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v37.log"
  assert_json "$RUN/closed_loop_safe_fast_v37.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v37.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v37 =====")
for p in sorted(root.glob('eval_*_v37_*.json')):
...
for p in sorted(root.glob('*v37*.json')):
...
run_eval v37 near_contact 1 &
wait
run_eval scalar contact 0 &
run_eval v37 contact 1 &
wait

# Stress audits. Two GPUs run clean scalar baseline and v37 concurrently.
run_audit scalar near_contact 0 32 384 &
run_audit v37 near_contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait
run_audit scalar contact 0 32 384 &
run_audit v37 contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait

if [[ "${RUN_NEAR_CHALLENGE_ABLATION:-0}" == "1" ]]; then
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_eval v37 near_contact 0
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_audit v37 near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384}
fi

if [[ "${RUN_DEEP_CONTACT:-0}" == "1" ]]; then
  run_audit v37 contact 0 ${DEEP_CONTACT_TARGETS:-48} ${DEEP_CONTACT_LABELS:-640}
fi
