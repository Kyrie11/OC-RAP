...
export RUN=${RUN:-runs/ocrap_v22_semantic_drs}
export V22_RUN=${V22_RUN:-runs/ocrap_v22_semantic_drs}
export CKPT=${CKPT:-$V22_RUN/model_v22/best.pt}
export CAL=${CAL:-$V22_RUN/calibration/calibration_mix_v22.json}
export GAMMA=${GAMMA:-$V22_RUN/calibration/gamma_rec_by_bucket_v22.json}
mkdir -p "$RUN"
...
make_sel() {
  ...
  COMMON_SEL=(
    ...
    # v22 semantic intervention firewall.  Coverage/perturbation candidates may
    # exist in the candidate set, but only recovery-eligible macro families can
    # be executed as non-nominal interventions.  This prevents scalar admission
    # from silently selecting perturb_nominal in contact or safe rollouts.
    ...
    --set selection.intervention_macro_blocklist_by_bucket.safe_v2=keep,brake,yield,lane_shift,merge,pull_over,stabilize,perturb_nominal
    ...
    --set selection.intervention_macro_blocklist_by_bucket.test_safe_v2=keep,brake,yield,lane_shift,merge,pull_over,stabilize,perturb_nominal
    ...
    # v22 macro-gated Pareto plus protective-macro recovery certificate.
    ...
    # v22 narrow protective channel. v19 proved macro gating prevents the v18
    ...
    # v22: protective brake/stabilize are judged by a deployability-vector score
    ...
    --set selection.protective_macro_score_min_gain_by_bucket.contact=0.020
    --set selection.protective_macro_score_min_gain_by_bucket.test_contact=0.020
    ...
    --set selection.protective_macro_min_drs_by_bucket.contact=0.78
    --set selection.protective_macro_min_drs_by_bucket.test_contact=0.78
    --set selection.protective_macro_min_rec_lcb_by_bucket.contact=-0.55
    --set selection.protective_macro_min_rec_lcb_by_bucket.test_contact=-0.55
    ...
    --set selection.protective_macro_min_drs_gain_by_bucket.contact=0.00
    --set selection.protective_macro_min_drs_gain_by_bucket.test_contact=0.00
    ...
    --set selection.protective_macro_max_drs_drop_by_bucket.contact=0.00
    --set selection.protective_macro_max_drs_drop_by_bucket.test_contact=0.00
    --set selection.protective_macro_max_rec_lcb_drop_by_bucket.contact=0.12
    --set selection.protective_macro_max_rec_lcb_drop_by_bucket.test_contact=0.12
    --set selection.protective_macro_max_gap_by_bucket.contact=1.45
    --set selection.protective_macro_max_gap_by_bucket.test_contact=1.45
    --set selection.protective_macro_max_gap_increase_by_bucket.contact=0.20
    --set selection.protective_macro_max_gap_increase_by_bucket.test_contact=0.20
    ...
    --set selection.protective_macro_bonus_by_bucket.contact=0.12
    --set selection.protective_macro_bonus_by_bucket.test_contact=0.12
    ...
    # v22 only allows semantic recovery macros to use the recovery-feasibility pool
    ...
    --set selection.intervention_budget_rate_by_bucket.contact=0.04
    ...
    --set selection.intervention_budget_penalty_by_bucket.contact=20.0
    --set selection.intervention_budget_hard_by_bucket.contact=true
    --set selection.intervention_budget_hard_by_bucket.test_contact=true
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v22_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v22_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v22_${tag}.log"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v22.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v22.log"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v22.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v22 =====")
for p in sorted(root.glob('eval_*_v22_*.json')):
...
for p in sorted(root.glob('*v22*.json')):
