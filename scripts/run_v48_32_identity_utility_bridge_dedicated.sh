...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_32_identity_utility_bridge_dedicated_4832}"
...
rm -f "$OUTPUTDIR"/ADAPTATION_FAILED_*.json "$OUTPUTDIR/PIPELINE_FAILED.json" \
      "$OUTPUTDIR/V48_32_COMPLETE.json" "$OUTPUTDIR/NEXT_COMMANDS.txt" \
...
failed={'event':'v48_32_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_32_identity_utility_bridge_controller_complete','created_unix':time.time(),
     ...
     'certificate_exit_code':30 if stage=='certificate' else None,'pipeline_exit_code':30,
     'gate_evaluated':stage=='certificate','gate_passed':False,'next_commands_generated':False,
     'pipeline_valid':False,'failure_stage':stage,'test_roots_read':False}
(root/'V48_32_COMPLETE.json').write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
blocked={'event':'v48_32_next_commands_blocked','created_unix':time.time(),
         'generated':False,'reason':'pipeline_failure','failure_stage':stage,
         'pipeline_exit_code':30,'gate_evaluated':stage=='certificate','test_roots_read':False}
...
run_variant() {
  ...
  bash scripts/adapt_ocrap_v48_32_identity_utility_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_32.json" --version v48.32-IDENTITY-UTILITY-BRIDGE || true
  exit 30
fi
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" \
  OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit bash scripts/calibrate_v48_32_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
...
doc={'event':'v48_32_identity_utility_bridge_controller_complete','created_unix':time.time(),
     ...
     'gate_evaluated':True,'gate_passed':(rc==0 and next_exists),
