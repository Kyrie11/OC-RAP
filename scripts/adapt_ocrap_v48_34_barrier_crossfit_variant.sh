...
ENABLE_SUPPORT_RELIABILITY="${V4834_ENABLE_SUPPORT_RELIABILITY:-1}"
IDENTITY_TRAIN_ALL="${V4834_IDENTITY_TRAIN_ALL:-1}"
COUPLE_ADMISSION_PRIOR="${V4834_COUPLE_ADMISSION_PRIOR:-1}"
ADAPTIVE_IDENTITY_MARGIN="${V4834_ADAPTIVE_IDENTITY_MARGIN:-0}"
ENABLE_FINAL_CALIBRATION="${V4834_ENABLE_FINAL_CALIBRATION:-0}"
FACTOR_CACHE_RUN="${V4834_FACTOR_CACHE_RUN:-}"
...
doc={'event':'v48_34_variant_stage_failed','created_unix':time.time(),'variant':sys.argv[2],
...
factor_cache_contract_args=(
  ...
  --setting "calibrator_context=${EVIDENCE_CALIBRATOR_CONTEXT:-false}"
  --setting "calibrator_context_source=${EVIDENCE_CALIBRATOR_CONTEXT_SOURCE:-relative}"
...
if [[ -n "$FACTOR_CACHE_RUN" ]]; then
  ...
  python tools/materialize_v48_34_factor_cache.py \
  ...
  EVIDENCE_CALIBRATOR_CONTEXT=false EVIDENCE_CALIBRATOR_CONTEXT_SOURCE=relative \
  EVIDENCE_ADMISSION_HEAD=false \
  EVIDENCE_COMPONENT_COUNT="${EVIDENCE_COMPONENT_COUNT:-5}" \
  EVIDENCE_ADMISSION_PRIOR_MODE=barrier_gated_slack EVIDENCE_ADMISSION_PRIOR_DETACH=true \
  ...
  EVIDENCE_ADAPT_EPOCHS="${FACTOR_EPOCHS:-20}" EVIDENCE_ADAPT_PATIENCE="${FACTOR_PATIENCE:-6}" \
    bash scripts/adapt_ocrap_v48_34_barrier_crossfit_single_stage.sh
...
EVIDENCE_ADMISSION_PRIOR_MODE="${EVIDENCE_ADMISSION_PRIOR_MODE:-barrier_gated_slack}" EVIDENCE_ADMISSION_PRIOR_DETACH="$prior_detach" \
EVIDENCE_SLACK_TEMPERATURE="${EVIDENCE_SLACK_TEMPERATURE:-0.025}" \
EVIDENCE_SLACK_PENALTY="${EVIDENCE_SLACK_PENALTY:-1.5}" \
...
for p in (source,factor,identity,final,support,run/'STAGE_TRANSFER_INTEGRITY.json'):
    if not p.is_file(): raise SystemExit(f'missing v48.34 artifact: {p}')
transfer=json.load(open(run/'STAGE_TRANSFER_INTEGRITY.json'))
doc={
 'event':'v48_34_barrier_crossfit_complete','created_unix':time.time(),
 ...
 'checkpoint_metric':'direct_contract_lexicographic','exact_eligibility_metric':True,'selection_semantics':'rank_topk_then_filter_then_evidence_rerank',
