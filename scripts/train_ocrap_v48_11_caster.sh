...
# v48.11 OC-TRAC-CASTER
# Stage T: recovery-only set tournament learns conditional option preference.
# Stage E: regime-specific ordered evidence learns benefit/dead/harm for the
...
{
  "version": "v48.11-CASTER",
  "policy_semantics": "recovery_set_tournament_then_no_fallback_evidence",
...
env "${COMMON_ENV[@]}" \
  ...
  ORDINAL_EVIDENCE_HARM_CLASS_WEIGHT="${ORDERED_EVIDENCE_HARM_WEIGHT:-2.50}" \
  ORDINAL_EVIDENCE_DEAD_CLASS_WEIGHT="${ORDERED_EVIDENCE_DEAD_WEIGHT:-0.45}" \
  ORDINAL_EVIDENCE_BENEFIT_CLASS_WEIGHT="${ORDERED_EVIDENCE_BENEFIT_WEIGHT:-1.25}" \
  bash scripts/train_ocrap_v48_trac_sr.sh
...
doc={
  'event':'v48_11_caster_staged_training_complete','created_unix':time.time(),
