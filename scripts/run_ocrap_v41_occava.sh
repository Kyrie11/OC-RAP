...
# v41 OC-CAVA evaluation. OC-MERO/shared-option logic remains the admission certificate;
# the new uncertainty-aware direct value head only ranks already-certified candidates
# and can challenge nominal through a lower-confidence advantage bound.
export RUN=${RUN:-runs/ocrap_v41_occava_balanced_eval}
export BASE_RUN=${BASE_RUN:-runs/ocrap_v41_occava_balanced}
export CKPT=${CKPT:-$BASE_RUN/model_v41_occava/best.pt}
export CAL=${CAL:-$BASE_RUN/calibration/calibration_mix_v41.json}
export GAMMA=${GAMMA:-$BASE_RUN/calibration/gamma_rec_by_bucket_v41.json}
mkdir -p "$RUN"
...
# Prefer the scene-time simultaneous conformal LCB learned on validation data.
# Explicit environment variables still override the calibrated values.
read_direct_q() {
  local path="$1"; local fallback="$2"
  python - "$path" "$fallback" <<'PYQ'
import json, math, sys
p, fallback = sys.argv[1], float(sys.argv[2])
try:
    q = float(json.load(open(p))["direct_value_additive_q"])
    print(q if math.isfinite(q) else fallback)
except Exception:
    print(fallback)
PYQ
}
export NEAR_DIRECT_ADDITIVE_Q=${NEAR_DIRECT_ADDITIVE_Q:-$(read_direct_q "$BASE_RUN/calibration/direct_value_advantage_near_v41.json" 0.08)}
export CONTACT_DIRECT_ADDITIVE_Q=${CONTACT_DIRECT_ADDITIVE_Q:-$(read_direct_q "$BASE_RUN/calibration/direct_value_advantage_contact_v41.json" 0.08)}
echo "direct-value additive q: near=$NEAR_DIRECT_ADDITIVE_Q contact=$CONTACT_DIRECT_ADDITIVE_Q"

make_sel() {
  local tag="$1"  # scalar | v41
  ...
  case "$tag" in
    ...
    v41|v40|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=${CONTACT_BRAKE_RESCUE:-false}; pcd_rescue=true; brake_tail=${CONTACT_BRAKE_TAIL:-true}; brake_tail_bypass_pcd=${CONTACT_TAIL_BYPASS_PCD_GAIN:-false}; direct_value=${RUN_DIRECT_VALUE:-true}; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    # v41 OC-CAVA: direct value is only a preference certificate. A candidate
    # must still be admitted by an OC-MERO/shared-option certificate.
    ...
    --set selection.direct_value_macro_allowlist_by_bucket.near_contact=${NEAR_DIRECT_MACROS:-brake,yield,merge,stabilize}
    --set selection.direct_value_macro_allowlist_by_bucket.test_near_contact=${NEAR_DIRECT_MACROS:-brake,yield,merge,stabilize}
    --set selection.direct_value_macro_allowlist_by_bucket.contact=${CONTACT_DIRECT_MACROS:-brake,yield,merge,stabilize}
    --set selection.direct_value_macro_allowlist_by_bucket.test_contact=${CONTACT_DIRECT_MACROS:-brake,yield,merge,stabilize}
    ...
    --set selection.direct_value_uncertainty_mode_by_bucket.near_contact=additive
    --set selection.direct_value_additive_q_by_bucket.near_contact=${NEAR_DIRECT_ADDITIVE_Q:-0.08}
    --set selection.direct_value_uncertainty_mode_by_bucket.test_near_contact=additive
    --set selection.direct_value_additive_q_by_bucket.test_near_contact=${NEAR_DIRECT_ADDITIVE_Q:-0.08}
    --set selection.direct_value_uncertainty_mode_by_bucket.contact=additive
    --set selection.direct_value_additive_q_by_bucket.contact=${CONTACT_DIRECT_ADDITIVE_Q:-0.08}
    --set selection.direct_value_uncertainty_mode_by_bucket.test_contact=additive
    --set selection.direct_value_additive_q_by_bucket.test_contact=${CONTACT_DIRECT_ADDITIVE_Q:-0.08}
    --set selection.direct_value_min_advantage_lcb_by_bucket.near_contact=${NEAR_DIRECT_MIN_ADV:-0.045}
    --set selection.direct_value_min_advantage_lcb_by_bucket.test_near_contact=${NEAR_DIRECT_MIN_ADV:-0.045}
    --set selection.direct_value_min_advantage_lcb_by_bucket.contact=${CONTACT_DIRECT_MIN_ADV:-0.035}
    --set selection.direct_value_min_advantage_lcb_by_bucket.test_contact=${CONTACT_DIRECT_MIN_ADV:-0.035}
    --set selection.direct_value_min_candidate_value_by_bucket.near_contact=${NEAR_DIRECT_MIN_VALUE:-0.48}
    --set selection.direct_value_min_candidate_value_by_bucket.test_near_contact=${NEAR_DIRECT_MIN_VALUE:-0.48}
    --set selection.direct_value_min_candidate_value_by_bucket.contact=${CONTACT_DIRECT_MIN_VALUE:-0.44}
    --set selection.direct_value_min_candidate_value_by_bucket.test_contact=${CONTACT_DIRECT_MIN_VALUE:-0.44}
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v41_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v41_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee -a "$RUN/audit_${b}_selected_topk_v41_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v41_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v41.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    --set closed_loop.resume="$CL_RESUME" \
    --set closed_loop.resume_allow_legacy_partial=true \
    ...
    | tee -a "$RUN/closed_loop_safe_fast_v41.log"
  assert_json "$RUN/closed_loop_safe_fast_v41.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v41.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v41 =====")
for p in sorted(root.glob('eval_*_v41_*.json')):
...
for p in sorted(root.glob('*v41*.json')):
...
if [[ "$RUN_OFFLINE_EVAL" == "1" ]]; then
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    run_eval scalar safe 0 & run_eval v41 safe 1 & wait
    run_eval scalar near_contact 0 & run_eval v41 near_contact 1 & wait
    run_eval scalar contact 0 & run_eval v41 contact 1 & wait
  else
    run_eval v41 safe 0 & run_eval v41 near_contact 1 & wait
    run_eval v41 contact 0
  fi
fi
if [[ "$RUN_AUDITS" == "1" ]]; then
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    run_audit scalar near_contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} &
    run_audit v41 near_contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait
    run_audit scalar contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} &
    run_audit v41 contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait
  else
    run_audit v41 near_contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} &
    run_audit v41 contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait
  fi
fi
