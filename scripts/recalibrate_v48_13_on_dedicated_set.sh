...
recalibrate_variant(){
  ...
  [[ -f "$ckpt" ]] || return 0
  local proposal_args=() conditional_args=() policy_args=()
  if [[ -f "$src/POLICY_CONTRACT.env" ]]; then
    set -a
    # shellcheck disable=SC1090
    source "$src/POLICY_CONTRACT.env"
    set +a
  fi
  case "${CONDITIONAL_RECOVERY_RANKING:-false}" in 1|true|TRUE|yes|YES) conditional_args=(--conditional-recovery-ranking) ;; esac
  case "${POLICY_FIRST_NO_FALLBACK:-false}" in 1|true|TRUE|yes|YES) policy_args=(--policy-first-no-fallback) ;; esac
  case "${EVIDENCE_RERANK_TOP_K:-false}" in
    1|true|TRUE|yes|YES) proposal_args=(--proposal-top-k "${PROPOSAL_TOP_K:-3}" --evidence-rerank-top-k) ;;
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    --dataset "$CAL_NEAR" --checkpoint "$ckpt" --bucket near --risk-source "${RISK_SOURCE:-ordinal_evidence}" "${conditional_args[@]}" "${policy_args[@]}" "${proposal_args[@]}" \
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    --dataset "$CAL_CONTACT" --checkpoint "$ckpt" --bucket contact --risk-source "${RISK_SOURCE:-ordinal_evidence}" "${conditional_args[@]}" "${policy_args[@]}" "${proposal_args[@]}" \
...
echo "v48.13 dedicated recalibration complete. No model was retrained and no test root was read."
