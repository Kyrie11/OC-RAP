...
ROOT="${ABLATION_ROOT:-runs/ocrap_v48_31_contract_slack_rank_ablations_4831}"
...
run_task() {
  local group="$1" variant="$2" gpu="$3" support="$4" joint="$5"
  ...
  JOINT_EPOCHS="${JOINT_EPOCHS:-12}" JOINT_PATIENCE="${JOINT_PATIENCE:-5}" \
  EVIDENCE_ADAPT_LR="${EVIDENCE_ADAPT_LR:-0.00015}" JOINT_LR="${JOINT_LR:-0.00005}" \
  V4831_ENABLE_SUPPORT_RELIABILITY="$support" V4831_ENABLE_JOINT_STAGE="$joint" \
  ...
  COMPONENT_HARM_PROXY_TOLERANCE="$COMPONENT_HARM_PROXY_TOLERANCE" \
    bash scripts/adapt_ocrap_v48_31_contract_slack_rank_variant.sh >"$out/logs/adapt.log" 2>&1
  local adapt_rc=$?
  set -e
  [[ "$adapt_rc" == 0 ]] || { printf '{"complete":false,"stage":"adaptation","raw_exit_code":%s}\n' "$adapt_rc" >"$out/TASK_FAILED.json"; return 30; }

  local checker_extra=() model_extra=()
  if [[ "$joint" != 1 ]]; then checker_extra+=(--allow-no-joint); fi
  if [[ "$support" != 1 ]]; then
    checker_extra+=(--reliability-override "1,1,1,1,1")
    model_extra+=(--reliability-override "1,1,1,1,1")
  fi
  python tools/check_v48_31_training_contract.py --run "$run" \
    --output "$run/TRAINING_CONTRACT.json" "${checker_extra[@]}"
  python tools/check_v48_31_model_contract.py \
    ...
    --expect-admission-prior-mode safety_slack --expect-slack-temperature 0.025 --expect-slack-penalty 1.0 \
    "${model_extra[@]}"
...
doc={'complete':True,'version':'v48.31-CONTRACT-SLACK-RANK','group':group,'variant':variant,
     'support_reliability_enabled':support,'joint_refinement_enabled':joint,
...
support_flags=(0 1 0 1)
joint_flags=(0 0 1 1)
: >"$ROOT/TASK_GPU_ASSIGNMENT.txt"
failures=0
# Four waves, one task per A30. This avoids the CPU/I/O and VRAM contention of
# v48.30's four simultaneous jobs per device while preserving two-way parallelism.
for i in "${!groups[@]}"; do
  group="${groups[$i]}"; support="${support_flags[$i]}"; joint="${joint_flags[$i]}"
  echo "${group}_balanced:gpu${GPU0}" >>"$ROOT/TASK_GPU_ASSIGNMENT.txt"
  echo "${group}_precision:gpu${GPU1}" >>"$ROOT/TASK_GPU_ASSIGNMENT.txt"
  run_task "$group" balanced "$GPU0" "$support" "$joint" & p0=$!
  run_task "$group" precision "$GPU1" "$support" "$joint" & p1=$!
  set +e
  wait "$p0"; r0=$?
  wait "$p1"; r1=$?
  set -e
...
groups=['A_contract_natural_no_reliability_no_joint','B_add_support_reliability_no_joint','C_add_joint_refinement_no_reliability','D_full_contract_slack_rank']
...
doc={'complete':not missing and failures==0,'version':'v48.31-CONTRACT-SLACK-RANK',
     'execution':'four waves; one Balanced task on GPU0 and one Precision task on GPU1 per wave',
     'max_concurrent_tasks':2,'expected_tasks':expected,'missing_tasks':missing,'failed_waits':failures,
     'created_unix':time.time(),'test_roots_read':False}
