...
[[ -s "$OUT/V48_32_1_COMPLETE.json" ]] || { echo "missing $OUT/V48_32_1_COMPLETE.json" >&2; exit 30; }
python - "$OUT/V48_32_1_COMPLETE.json" <<'PY_STATUS'
import json,sys
...
if not (d.get('pipeline_valid') and d.get('gate_evaluated') and d.get('gate_passed') and d.get('certificate_exit_code')==0 and d.get('next_commands_generated')):
    raise SystemExit('v48.32.1 run is not stress-authorized')
PY_STATUS
