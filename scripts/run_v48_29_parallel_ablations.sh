...
ROOT="${ABLATION_ROOT:-runs/ocrap_v48_29_veto_rank_physics_ablations_4829}"
...
[[ "$TASKS_PER_GPU" -ge 4 ]] || { echo "v48.29 needs TASKS_PER_GPU>=4 for all-eight concurrency" >&2; exit 30; }
...
doc={'complete':False,'version':'v48.29-VETO-RANK-PHYSICS-BRIDGE','stage':sys.argv[2],
...
run_task() {
  ...
  local component_count=5 component_scale=6.0 safe_list=0 frontier=0 hard_negative=0 prior_mode=benefit_only
  case "$group" in
    A_risk_centered_reference)
      prior_mode=risk_centered; hard_negative=0 ;;
    B_veto_decoupled)
      prior_mode=benefit_only; hard_negative=0 ;;
    C_add_safe_hard_negative)
      prior_mode=benefit_only; hard_negative=1.0 ;;
    D_add_frontier_to_hard_negative)
      prior_mode=benefit_only; hard_negative=1.0; frontier=0.25 ;;
  ...
  ADMISSION_FRONTIER_PAIRWISE_WEIGHT="$frontier" \
    bash scripts/adapt_ocrap_v48_29_veto_rank_variant.sh >"$out/logs/adapt.log" 2>&1
  local adapt_rc=$?
  set -e
  ...
  GPU0="$gpu" GPU1="$gpu" VARIANTS="$variant" OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit \
    bash scripts/calibrate_v48_29_certificate_pool.sh >"$out/logs/certificate.log" 2>&1
  local cert_rc=$?
  set -e
  [[ "$cert_rc" == 0 || "$cert_rc" == 20 ]] || { printf '{"complete":false,"stage":"certificate","raw_exit_code":%s}\n' "$cert_rc" >"$out/TASK_FAILED.json"; return 30; }

  python - "$out" "$group" "$variant" "$cert_rc" "$component_count" "$component_scale" "$safe_list" "$frontier" "$hard_negative" "$prior_mode" <<'PY'
import hashlib,json,pathlib,sys,time
out=pathlib.Path(sys.argv[1]); group=sys.argv[2]; variant=sys.argv[3]; rc=int(sys.argv[4])
component_count=int(sys.argv[5]); component_scale=float(sys.argv[6]); safe_list=float(sys.argv[7]); frontier=float(sys.argv[8]); hard_negative=float(sys.argv[9]); prior_mode=sys.argv[10]
...
doc={'complete':True,'version':'v48.29-VETO-RANK-PHYSICS-BRIDGE','group':group,'variant':variant,
...
groups=(A_risk_centered_reference B_veto_decoupled C_add_safe_hard_negative D_add_frontier_to_hard_negative)
...
doc={'complete':not missing and failures==0,'version':'v48.29-VETO-RANK-PHYSICS-BRIDGE',
