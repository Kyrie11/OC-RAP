...
: "${CL_WOMD_ROLE:=$PRIMARY_WOMD_ROLE}"
: "${CALIB_WOMD_ROLE:=$PRIMARY_WOMD_ROLE}"
...
common=(
  ...
  CL_WOMD_ROLE="$CL_WOMD_ROLE"
  CALIB_WOMD_ROLE="$CALIB_WOMD_ROLE"
  PRIMARY_WOMD_ROLE="$PRIMARY_WOMD_ROLE"
  FORCE_REREGISTER="$FORCE_RETRAIN_ALL"
)
...
run_regime() {
  ...
  if [[ "$enabled" != 1 ]]; then
    write_phase "$regime" skipped 0 "$started" "$(runtime_iso_now)"; return 0
  fi
  ...
  ended="$(runtime_iso_now)"; status=complete; ((rc==0)) || status=failed
  ...
  if ((rc!=0)); then
    ...
    runtime_bool_true "$CONTINUE_AFTER_REGIME_FAILURE" || return "$rc"
  fi
  return 0
}
