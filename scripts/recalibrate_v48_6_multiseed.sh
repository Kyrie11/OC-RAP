...
SOURCE_RUN="${SOURCE_RUN:?Set SOURCE_RUN to a completed v48.6 training run}"
...
python tools/audit_v48_6_completion.py \
  --root "$SOURCE_RUN" \
  --output "$SOURCE_RUN/source_completion_audit_v48_6.json"
...
for raw_seed in "${seed_array[@]}"; do
  ...
  calibrate_variant() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket near --risk-source direct_delta \
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket contact --risk-source direct_delta \
...
python tools/summarize_v48_6_multiseed.py \
  --root "$MULTISEED_ROOT" \
  --output "$MULTISEED_ROOT/multiseed_summary_v48_6.json"
printf '{"complete":true,"seeds":"%s","risk_source":"direct_delta"}\n' "$SEEDS" \
