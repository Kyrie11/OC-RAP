...
export RUN=${RUN:-runs/ocrap_v20_protective_macro}
...
make_sel() {
  ...
  COMMON_SEL=(
    ...
    # v20 macro-gated Pareto plus protective-macro recovery certificate.
    ...
    --set selection.relative_recovery_macro_allowlist_by_bucket.contact=brake,stabilize,yield,merge
    --set selection.relative_recovery_macro_allowlist_by_bucket.test_contact=brake,stabilize,yield,merge
    ...
    # v20 narrow protective channel. v19 proved macro gating prevents the v18
    ...
    --set selection.protective_macro_min_drs_by_bucket.contact=0.68
    --set selection.protective_macro_min_drs_by_bucket.test_contact=0.68
    --set selection.protective_macro_min_gap_reduction_by_bucket.contact=0.06
    --set selection.protective_macro_min_gap_reduction_by_bucket.test_contact=0.06
    ...
    --set selection.protective_macro_min_drs_gain_by_bucket.contact=-1.0
    --set selection.protective_macro_min_drs_gain_by_bucket.test_contact=-1.0
    ...
    --set selection.protective_macro_max_drs_drop_by_bucket.contact=0.055
    --set selection.protective_macro_max_drs_drop_by_bucket.test_contact=0.055
    ...
    --set selection.protective_macro_max_gap_by_bucket.contact=1.55
    --set selection.protective_macro_max_gap_by_bucket.test_contact=1.55
    --set selection.protective_macro_max_gap_increase_by_bucket.contact=0.05
    --set selection.protective_macro_max_gap_increase_by_bucket.test_contact=0.05
    ...
    --set selection.protective_macro_bonus_by_bucket.contact=0.20
    --set selection.protective_macro_bonus_by_bucket.test_contact=0.20
    ...
    # v20 only allows semantic recovery macros to use the recovery-feasibility pool
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v20_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v20_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v20_${tag}.log"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v20.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v20.log"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v20.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v20 =====")
for p in sorted(root.glob('eval_*_v20_*.json')):
...
for p in sorted(root.glob('*v20*.json')):
