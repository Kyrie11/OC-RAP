...
# v42 OCSAVA evaluation. OC-MERO/shared-option logic remains the admission certificate;
...
export RUN=${RUN:-runs/ocrap_v42_ocsava_pairwise_balanced_eval}
export BASE_RUN=${BASE_RUN:-runs/ocrap_v42_ocsava_pairwise_balanced}
export CKPT=${CKPT:-$BASE_RUN/model_v42_ocsava/best.pt}
export CAL=${CAL:-$BASE_RUN/calibration/calibration_mix_v42.json}
export GAMMA=${GAMMA:-$BASE_RUN/calibration/gamma_rec_by_bucket_v42.json}
mkdir -p "$RUN"
...
# Prefer the selection-conditional top-1 conformal LCB learned on validation data.
...
export NEAR_DIRECT_ADDITIVE_Q=${NEAR_DIRECT_ADDITIVE_Q:-$(read_direct_q "$BASE_RUN/calibration/direct_value_advantage_near_v42.json" 0.08)}
export CONTACT_DIRECT_ADDITIVE_Q=${CONTACT_DIRECT_ADDITIVE_Q:-$(read_direct_q "$BASE_RUN/calibration/direct_value_advantage_contact_v42.json" 0.08)}
echo "direct-value additive q: near=$NEAR_DIRECT_ADDITIVE_Q contact=$CONTACT_DIRECT_ADDITIVE_Q"

make_sel() {
  local tag="$1"  # scalar | v42
  ...
  case "$tag" in
    ...
    v42|v40|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=${CONTACT_BRAKE_RESCUE:-false}; pcd_rescue=true; brake_tail=${CONTACT_BRAKE_TAIL:-false}; brake_tail_bypass_pcd=${CONTACT_TAIL_BYPASS_PCD_GAIN:-false}; direct_value=${RUN_DIRECT_VALUE:-true}; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    # v42 OCSAVA: direct value is only a preference certificate. A candidate
    ...
    --set selection.direct_value_min_advantage_lcb_by_bucket.near_contact=${NEAR_DIRECT_MIN_ADV:-0.010}
    --set selection.direct_value_min_advantage_lcb_by_bucket.test_near_contact=${NEAR_DIRECT_MIN_ADV:-0.010}
    --set selection.direct_value_min_advantage_lcb_by_bucket.contact=${CONTACT_DIRECT_MIN_ADV:-0.010}
    --set selection.direct_value_min_advantage_lcb_by_bucket.test_contact=${CONTACT_DIRECT_MIN_ADV:-0.010}
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v42_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v42_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee -a "$RUN/audit_${b}_selected_topk_v42_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v42_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v42.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    --set closed_loop.resume="$CL_RESUME" \
    --set closed_loop.resume_allow_legacy_partial=true \
    ...
    | tee -a "$RUN/closed_loop_safe_fast_v42.log"
  assert_json "$RUN/closed_loop_safe_fast_v42.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v42.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v42 =====")
for p in sorted(root.glob('eval_*_v42_*.json')):
...
for p in sorted(root.glob('*v42*.json')):
...
if [[ "$RUN_OFFLINE_EVAL" == "1" ]]; then
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    run_eval scalar safe 0 & run_eval v42 safe 1 & wait
    run_eval scalar near_contact 0 & run_eval v42 near_contact 1 & wait
    run_eval scalar contact 0 & run_eval v42 contact 1 & wait
  else
    run_eval v42 safe 0 & run_eval v42 near_contact 1 & wait
    run_eval v42 contact 0
  fi
fi
  ...
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" && "${PAIR_BUCKETS_ON_TWO_GPUS:-1}" == "1" ]]; then
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      (run_audit scalar near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v42 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_NEAR=$!
    fi
    if [[ "$RUN_CONTACT_AUDIT" == "1" ]]; then
      (run_audit scalar contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v42 contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_CONTACT=$!
    fi
    [[ -z "${P_NEAR:-}" ]] || wait "$P_NEAR"
    [[ -z "${P_CONTACT:-}" ]] || wait "$P_CONTACT"
  elif [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    [[ "$RUN_NEAR_AUDIT" != "1" ]] || { run_audit scalar near_contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v42 near_contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
    [[ "$RUN_CONTACT_AUDIT" != "1" ]] || { run_audit scalar contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v42 contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
  else
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      run_audit v42 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} &
      P_NEAR=$!
