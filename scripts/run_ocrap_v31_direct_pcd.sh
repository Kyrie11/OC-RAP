...
# v31 is a direct teacher-PCD contact correction on top of the v31 model.  It tests
# whether retraining the deployability heads fixes the v30 reverse learned-PCD
# signal. Override CKPT/CAL/GAMMA if you place the model elsewhere.
export RUN=${RUN:-runs/ocrap_v31_direct_pcd_eval}
export BASE_RUN=${BASE_RUN:-runs/ocrap_v31_direct_pcd}
export CKPT=${CKPT:-$BASE_RUN/model_v31_direct_pcd/best.pt}
export CAL=${CAL:-$BASE_RUN/calibration/calibration_mix_v31.json}
export GAMMA=${GAMMA:-$BASE_RUN/calibration/gamma_rec_by_bucket_v31.json}
mkdir -p "$RUN"
...
make_sel() {
  ...
  case "$tag" in
    ...
    v31|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=true; pcd_rescue=true; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    --set selection.pcd_rescue_min_pred_pcd_by_bucket.contact=${CONTACT_FRONTIER_MIN_PCD:-0.30}
    --set selection.pcd_rescue_min_pred_pcd_by_bucket.test_contact=${CONTACT_FRONTIER_MIN_PCD:-0.30}
    ...
    --set selection.pcd_rescue_min_pred_drs_by_bucket.contact=${CONTACT_FRONTIER_MIN_DRS:-0.65}
    --set selection.pcd_rescue_min_pred_drs_by_bucket.test_contact=${CONTACT_FRONTIER_MIN_DRS:-0.65}
    ...
    --set selection.rescue_challenge_min_pred_drs_by_bucket.contact=${CONTACT_CHALLENGE_MIN_DRS:-0.65}
    --set selection.rescue_challenge_min_pred_drs_by_bucket.test_contact=${CONTACT_CHALLENGE_MIN_DRS:-0.65}
  ...
  if [[ "$is_v27" == "true" ]]; then
    COMMON_SEL+=(
      ...
      --set selection.rescue_challenge_max_used_by_bucket.contact=${CONTACT_CHALLENGE_MAX_USED:-6}
      --set selection.rescue_challenge_max_used_by_bucket.test_contact=${CONTACT_CHALLENGE_MAX_USED:-6}
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v31_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v31_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v31_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v31_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v31.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v31.log"
  assert_json "$RUN/closed_loop_safe_fast_v31.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v31.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v31 =====")
for p in sorted(root.glob('eval_*_v31_*.json')):
...
for p in sorted(root.glob('*v31*.json')):
...
run_eval v31 near_contact 1 &
wait
run_eval scalar contact 0 &
run_eval v31 contact 1 &
wait

# Stress audits. Two GPUs run baseline and v24 concurrently.
run_audit scalar near_contact 0 32 384 &
run_audit v31 near_contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait
run_audit scalar contact 0 32 384 &
run_audit v31 contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait

if [[ "${RUN_NEAR_CHALLENGE_ABLATION:-0}" == "1" ]]; then
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_eval v31 near_contact 0
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_audit v31 near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384}
fi

if [[ "${RUN_DEEP_CONTACT:-0}" == "1" ]]; then
  run_audit v31 contact 0 ${DEEP_CONTACT_TARGETS:-48} ${DEEP_CONTACT_LABELS:-640}
fi
