...
ROOT="${ABLATION_ROOT:-runs/ocrap_v48_28_provenance_margin_ablations_4828}"
...
[[ "$TASKS_PER_GPU" -ge 4 ]] || { echo "v48.28 needs TASKS_PER_GPU>=4 for all-eight concurrency" >&2; exit 30; }
...
doc={'complete':False,'version':'v48.28-PROVENANCE-MARGIN-BRIDGE','stage':sys.argv[2],
...
run_task() {
  ...
  local component_count=5 component_scale=6.0 safe_list=0 frontier=0
  case "$group" in
    A_three_factor_wide_range)
      component_count=3; component_scale=6.0 ;;
    B_five_factor_old_range)
      component_count=5; component_scale=2.0 ;;
    C_five_factor_wide_range_regression)
      component_count=5; component_scale=6.0 ;;
    D_add_listwise_frontier)
      component_count=5; component_scale=6.0; safe_list=0.25; frontier=0.25 ;;
  ...
  ADMISSION_FRONTIER_PAIRWISE_WEIGHT="$frontier" \
    bash scripts/adapt_ocrap_v48_28_provenance_margin_variant.sh >"$out/logs/adapt.log" 2>&1
  local adapt_rc=$?
  set -e
  ...
  GPU0="$gpu" GPU1="$gpu" VARIANTS="$variant" OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit \
    bash scripts/calibrate_v48_28_certificate_pool.sh >"$out/logs/certificate.log" 2>&1
  local cert_rc=$?
  set -e
  [[ "$cert_rc" == 0 || "$cert_rc" == 20 ]] || { printf '{"complete":false,"stage":"certificate","raw_exit_code":%s}\n' "$cert_rc" >"$out/TASK_FAILED.json"; return 30; }

  python - "$out" "$group" "$variant" "$cert_rc" "$component_count" "$component_scale" "$safe_list" "$frontier" <<'PY'
import hashlib,json,pathlib,sys,time
out=pathlib.Path(sys.argv[1]); group=sys.argv[2]; variant=sys.argv[3]; rc=int(sys.argv[4])
component_count=int(sys.argv[5]); component_scale=float(sys.argv[6]); safe_list=float(sys.argv[7]); frontier=float(sys.argv[8])
...
doc={'complete':True,'version':'v48.28-PROVENANCE-MARGIN-BRIDGE','group':group,'variant':variant,
...
groups=(A_three_factor_wide_range B_five_factor_old_range C_five_factor_wide_range_regression D_add_listwise_frontier)
...
doc={'complete':not missing and failures==0,'version':'v48.28-PROVENANCE-MARGIN-BRIDGE',
