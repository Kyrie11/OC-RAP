...
ENABLE_SUPPORT_RELIABILITY="${V4832_ENABLE_SUPPORT_RELIABILITY:-1}"
IDENTITY_TRAIN_ALL="${V4832_IDENTITY_TRAIN_ALL:-1}"
COUPLE_ADMISSION_PRIOR="${V4832_COUPLE_ADMISSION_PRIOR:-1}"
ADAPTIVE_IDENTITY_MARGIN="${V4832_ADAPTIVE_IDENTITY_MARGIN:-1}"
ENABLE_FINAL_CALIBRATION="${V4832_ENABLE_FINAL_CALIBRATION:-1}"
FACTOR_CACHE_RUN="${V4832_FACTOR_CACHE_RUN:-}"
...
factor_cache_contract_args=(
  ...
  --setting "proposal_top_k=${PROPOSAL_TOP_K:-3}"
  ...
  --setting "calibrator_hidden=${EVIDENCE_CALIBRATOR_HIDDEN:-16}"
  ...
  --setting "positive_macro_balance_power=${FACTOR_POSITIVE_MACRO_BALANCE_POWER:-0.25}"
...
copy_stage_artifacts() {
  ...
  rm -rf "$dst"
  mkdir -p "$dst"
  for item in model_v48_trac_sr STAGE_ARCHITECTURE.json POLICY_CONTRACT.env TRAINING_COMPLETE.json EVIDENCE_CORRECTION_COMPLETE.json FACTOR_CACHE_CONTRACT.json; do
    [[ -e "$src/$item" ]] || { echo "missing reusable stage artifact $src/$item" >&2; return 30; }
...
if [[ -n "$FACTOR_CACHE_RUN" ]]; then
  python tools/manage_v48_32_factor_cache.py --mode verify \
    "${factor_cache_contract_args[@]}" \
    --contract "$FACTOR_CACHE_RUN/FACTOR_CACHE_CONTRACT.json" \
    --output "$FINAL_RUN/FACTOR_CACHE_VALIDATION.json"
  copy_stage_artifacts "$FACTOR_CACHE_RUN" "$FACTOR_RUN"
else
  ...
  EVIDENCE_ADMISSION_PRIOR_MODE=safety_slack EVIDENCE_ADMISSION_PRIOR_DETACH=true \
  ...
  EVIDENCE_ADAPT_EPOCHS="${FACTOR_EPOCHS:-20}" EVIDENCE_ADAPT_PATIENCE="${FACTOR_PATIENCE:-6}" \
    bash scripts/adapt_ocrap_v48_32_identity_utility_single_stage.sh
...
EVIDENCE_ADMISSION_PRIOR_MODE=safety_slack EVIDENCE_ADMISSION_PRIOR_DETACH="$prior_detach" \
...
ORDINAL_EVIDENCE_SAFE_BENEFIT_TARGET=false \
ORDINAL_EVIDENCE_BENEFIT_LISTWISE_WEIGHT="${IDENTITY_BENEFIT_LISTWISE_WEIGHT:-0.50}" \
ORDINAL_EVIDENCE_COMPONENT_TAIL_WEIGHT="${IDENTITY_COMPONENT_TAIL_WEIGHT:-0.35}" \
ORDINAL_EVIDENCE_COMPONENT_MARGIN_REGRESSION_WEIGHT="${IDENTITY_COMPONENT_MARGIN_REGRESSION_WEIGHT:-0.50}" \
...
ORDINAL_EVIDENCE_SAFE_UTILITY_LISTWISE_WEIGHT="${IDENTITY_SAFE_UTILITY_LISTWISE_WEIGHT:-0.75}" \
...
ORDINAL_EVIDENCE_FRONTIER_PAIRWISE_WEIGHT="${IDENTITY_FRONTIER_PAIRWISE_WEIGHT:-0.35}" \
ORDINAL_EVIDENCE_FRONTIER_PAIRWISE_MARGIN="${IDENTITY_FRONTIER_MARGIN:-0.04}" \
ORDINAL_EVIDENCE_ADMISSION_WEIGHT="${IDENTITY_ADMISSION_BINARY_WEIGHT:-0.05}" \
...
EVIDENCE_CALIBRATOR_ANCHOR_WEIGHT="${IDENTITY_CALIBRATOR_ANCHOR_WEIGHT:-0.12}" \
SELECTIVE_RISK_WEIGHT=0 SELECTIVE_COVERAGE_WEIGHT=0 \
OPPORTUNITY_ADMISSION_WEIGHT=0 HARM_ADMISSION_WEIGHT=0 \
GROUP_BATCH_STRATIFIED=false GROUP_BATCHING_REPLACEMENT=false \
POSITIVE_GROUP_BOOST="${IDENTITY_POSITIVE_GROUP_BOOST:-3.0}" POSITIVE_MACRO_BALANCE_POWER=0.0 \
EVIDENCE_ADAPT_LR="${IDENTITY_LR:-0.00010}" \
BEST_METRIC=direct_contract_safe_rank_risk EVALUATE_INITIAL_CHECKPOINT=true \
EVIDENCE_ADAPT_EPOCHS="${IDENTITY_EPOCHS:-18}" EVIDENCE_ADAPT_PATIENCE="${IDENTITY_PATIENCE:-6}" \
  bash scripts/adapt_ocrap_v48_32_identity_utility_single_stage.sh
...
if [[ "$ENABLE_FINAL_CALIBRATION" == 1 ]]; then
  ...
  ORDINAL_EVIDENCE_SAFE_UTILITY_LISTWISE_WEIGHT="${FINAL_SAFE_UTILITY_LISTWISE_WEIGHT:-0.50}" \
...
python - "$FINAL_RUN" "$SOURCE_CKPT" "$FACTOR_CKPT" "$IDENTITY_CKPT" "$FINAL_CKPT" "$SUPPORT_JSON" "$ENABLE_SUPPORT_RELIABILITY" "$IDENTITY_TRAIN_ALL" "$COUPLE_ADMISSION_PRIOR" "$ADAPTIVE_IDENTITY_MARGIN" "$ENABLE_FINAL_CALIBRATION" <<'PY'
...
coupled=sys.argv[9] == '1'; adaptive=sys.argv[10] == '1'; final_enabled=sys.argv[11] == '1'
for p in (source,factor,identity,final,support,run/'STAGE_TRANSFER_INTEGRITY.json'):
    if not p.is_file(): raise SystemExit(f'missing v48.32 artifact: {p}')
transfer=json.load(open(run/'STAGE_TRANSFER_INTEGRITY.json'))
doc={
 'event':'v48_32_identity_utility_bridge_complete','created_unix':time.time(),
 ...
 'stage3_population':'natural_without_replacement' if final_enabled else 'disabled_ablation',
 ...
 'continuous_unified_semantics':'raw benefit minus support-weighted positive worst safety slack',
 'independent_measured_hard_veto':True,
 'checkpoint_metric':'direct_contract_safe_rank_risk','exact_eligibility_metric':True,
