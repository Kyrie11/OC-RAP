...
ROOT="${ABLATION_ROOT:-runs/ocrap_v48_20_unison_ablations}"
...
doc={'complete':False,'version':'v48.20-UNISON-BRIDGE','stage':stage,
...
if [[ "$rebuild_index" != 1 && -f "$GROUP_INDEX" && -f "$GROUP_SUMMARY" ]]; then
  ...
  python tools/check_v48_19_target_support.py "${index_contract_args[@]}" --mode contract \
    --output "$ROOT/UNISON_INDEX_CONTRACT.json" >"$ROOT/logs/check_teacher_index_contract.log" 2>&1
  contract_rc=$?
  set -e
...
contract_rc=$?
set -e
if [[ "$contract_rc" != 0 ]]; then fail_controller teacher_index_contract "$contract_rc" "$ROOT/UNISON_INDEX_CONTRACT.json"; exit 30; fi
set +e
python tools/check_v48_19_target_support.py "${index_contract_args[@]}" --mode all \
  --output "$ROOT/UNISON_TARGET_SUPPORT.json" >"$ROOT/logs/check_unison_target_support.log" 2>&1
target_rc=$?
set -e
if [[ "$target_rc" != 0 ]]; then fail_controller target_support "$target_rc" "$ROOT/UNISON_TARGET_SUPPORT.json"; exit 30; fi
...
run_task() {
  ...
  local component_heads=true setwise=0.0 component_tail=0.75 batch_balance=true global_balance=true class_balance=0.75
  case "$group" in
  ...
  BEST_METRIC=direct_unison_selection_risk \
    bash scripts/adapt_ocrap_v48_20_unison_variant.sh >"$out/logs/adapt.log" 2>&1
  local adapt_rc=$?
  set -e
  if [[ "$adapt_rc" != 0 ]]; then
  ...
  GPU0="$gpu" GPU1="$gpu" VARIANTS="$variant" CERTIFICATE_GRID_SIZE="${CERTIFICATE_GRID_SIZE:-31}" \
    bash scripts/calibrate_v48_20_certificate_pool.sh >"$out/logs/calibrate.log" 2>&1
  local cert_rc=$?
  set -e
  ...
  set +e
  python - "$out" "$group" "$variant" "$cert_rc" "$component_heads" "$setwise" "$component_tail" "$batch_balance" "$global_balance" <<'PY_TASK'
import hashlib,json,pathlib,sys,time
out=pathlib.Path(sys.argv[1]); group,variant=sys.argv[2:4]; rc=int(sys.argv[4])
component_heads=sys.argv[5].lower()=='true'; setwise=float(sys.argv[6]); component_tail=float(sys.argv[7])
batch_balance=sys.argv[8].lower()=='true'; global_balance=sys.argv[9].lower()=='true'
...
doc={'complete':True,'version':'v48.20-UNISON-BRIDGE','group':group,'variant':variant,
     'component_heads':component_heads,'setwise_admission_weight':setwise,
     'component_tail_weight':component_tail,'batch_balance':batch_balance,'global_balance':global_balance,
...
groups=(A_candidate_tail_only B_safe_set_aggregate_harm C_component_safe_set_no_balance D_full_unison)
run_wave() {
  local variant="$1"
...
set -e
python tools/summarize_v48_14_ablations.py --root "$ROOT" --output "$ROOT/ablation_summary_v48_20.json" --version v48.20-UNISON-BRIDGE || true
set +e
python - "$ROOT" "$failures" <<'PY'
import json,pathlib,sys,time
root=pathlib.Path(sys.argv[1]); failures=int(sys.argv[2]); groups=['A_candidate_tail_only','B_safe_set_aggregate_harm','C_component_safe_set_no_balance','D_full_unison']
...
doc={'complete':not missing and failures==0,'version':'v48.20-UNISON-BRIDGE','max_concurrent_tasks':4,
