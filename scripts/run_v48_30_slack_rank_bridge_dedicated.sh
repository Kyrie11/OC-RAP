...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_30_slack_rank_bridge_dedicated_4830}"
...
failed={'event':'v48_30_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_30_slack_rank_bridge_controller_complete','created_unix':time.time(),
...
(root/'V48_30_COMPLETE.json').write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
PY
}
...
run_variant() {
  ...
  ADMISSION_SAFE_UTILITY_REGRESSION_WEIGHT="${ADMISSION_SAFE_UTILITY_REGRESSION_WEIGHT:-0.75}" \
  ADMISSION_SAFE_HARD_NEGATIVE_WEIGHT="${ADMISSION_SAFE_HARD_NEGATIVE_WEIGHT:-1.0}" \
  ADMISSION_SAFE_HARD_NEGATIVE_MARGIN="${ADMISSION_SAFE_HARD_NEGATIVE_MARGIN:-0.05}" \
  ADMISSION_SAFE_UTILITY_LISTWISE_WEIGHT=0 ADMISSION_FRONTIER_PAIRWISE_WEIGHT=0 \
  FACTOR_COMPONENT_MARGIN_REGRESSION_WEIGHT="${FACTOR_COMPONENT_MARGIN_REGRESSION_WEIGHT:-0.50}" \
  ADMISSION_SETWISE_WEIGHT="${ADMISSION_SETWISE_WEIGHT:-0.10}" ADMISSION_BINARY_WEIGHT="${ADMISSION_BINARY_WEIGHT:-0.10}" \
  bash scripts/adapt_ocrap_v48_30_slack_rank_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_30.json" --version v48.30-SLACK-RANK-BRIDGE || true
  exit 30
fi
...
# v48.30 fail-closed preflight: training and downstream inference must
...
for variant in balanced precision; do
  ...
  set +e
  python tools/check_v48_30_model_contract.py \
    --checkpoint "$ckpt" \
  ...
  set +e
  python tools/check_v48_30_training_contract.py \
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" \
  OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit bash scripts/calibrate_v48_30_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
