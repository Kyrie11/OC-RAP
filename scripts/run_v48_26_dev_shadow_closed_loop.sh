...
OUT="${OUT:?OUT is required; point it to the v48.26 dedicated run}"
...
(root/'DIAGNOSTIC_ONLY_NO_PAPER.json').write_text(json.dumps({
 'event':'v48_26_dev_shadow_closed_loop','created_unix':time.time(),
...
run_variant() {
  ...
  set +e
  python tools/check_v48_26_regime_targets.py "$run" \
...
doc={'event':'v48_26_dev_shadow_complete','created_unix':time.time(),
