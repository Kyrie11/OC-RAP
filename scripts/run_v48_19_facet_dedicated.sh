...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_19_facet_dedicated_4819}"
...
failed={'event':'v48_19_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_19_facet_bridge_controller_complete','created_unix':time.time(),
...
(root/'V48_19_COMPLETE.json').write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
PY
}
...
if [[ "$rebuild_index" != 1 && -f "$GROUP_INDEX" && -f "$GROUP_SUMMARY" ]]; then
  ...
  python tools/check_v48_19_target_support.py "${index_contract_args[@]}" \
    --mode contract --output "$OUTPUTDIR/FACET_INDEX_CONTRACT.json" \
...
# Re-audit after a rebuild so FACET_INDEX_CONTRACT.json always describes the
...
set -e
if [[ "$contract_rc" != 0 ]]; then
  write_pipeline_failure teacher_index_contract "$contract_rc" "$OUTPUTDIR/FACET_INDEX_CONTRACT.json"
  exit 30
fi

set +e
python tools/check_v48_19_target_support.py "${index_contract_args[@]}" \
  --mode all --output "$OUTPUTDIR/FACET_TARGET_SUPPORT.json" \
  >"$OUTPUTDIR/logs/check_facet_target_support.log" 2>&1
target_rc=$?
set -e
if [[ "$target_rc" != 0 ]]; then
  write_pipeline_failure target_support "$target_rc" "$OUTPUTDIR/FACET_TARGET_SUPPORT.json"
  exit 30
fi
...
run_variant() {
  ...
  EVIDENCE_ADAPT_LR="${EVIDENCE_ADAPT_LR:-0.00030}" \
    bash scripts/adapt_ocrap_v48_19_facet_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_19.json" --version v48.19-FACET-BRIDGE || true
  exit 30
fi
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" \
  bash scripts/calibrate_v48_19_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
