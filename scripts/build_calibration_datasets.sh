...
OUTPUT_ROOT="${OUTPUT_ROOT:-/data0/senzeyu2/dataset/OCRAP_calibration_build}"
...
for spec in \
  ...
  "$PYTHON_BIN" tools/filter_dataset_scenes.py --overwrite --input "$OUTPUT_ROOT/$raw" --output "$OUTPUT_ROOT/$final" \
...
python tools/check_scene_overlap.py \
  "${audit_existing[@]}" \
