...
FACTOR_CACHE_RUN="${V48321_FACTOR_CACHE_RUN:-${V4832_FACTOR_CACHE_RUN:-}}"
...
doc={'event':'v48_32_1_variant_stage_failed','created_unix':time.time(),'variant':sys.argv[2],
...
if [[ -n "$FACTOR_CACHE_RUN" ]]; then
  ...
  python tools/materialize_v48_32_1_factor_cache.py \
  ...
  EVIDENCE_ADAPT_EPOCHS="${FACTOR_EPOCHS:-20}" EVIDENCE_ADAPT_PATIENCE="${FACTOR_PATIENCE:-6}" \
    bash scripts/adapt_ocrap_v48_32_1_identity_utility_single_stage.sh
...
for p in (source,factor,identity,final,support,run/'STAGE_TRANSFER_INTEGRITY.json'):
    if not p.is_file(): raise SystemExit(f'missing v48.32.1 artifact: {p}')
transfer=json.load(open(run/'STAGE_TRANSFER_INTEGRITY.json'))
doc={
 'event':'v48_32_1_identity_utility_bridge_complete','created_unix':time.time(),
