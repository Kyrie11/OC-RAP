...
export RUN=${RUN:-runs/ocrap_v34_tail_challenge}
export MODEL_DIR=${MODEL_DIR:-$RUN/model_v34_tail_challenge}
...
# v34 optional fine-tune: start from v33 brake-tail and mildly reinforce
# brake/yield component/rank losses.  The primary v34 change is selector-side
# tail challenge bypass; this fine-tune is optional and should be run only after
# selector-only v34 confirms the remaining contact misses are tail certificate cases.
CUDA_VISIBLE_DEVICES="$TRAIN_GPU" PYTHONUNBUFFERED=1 python -u -m ocrap.cli train \
  ...
  --set training.group_batch_hard_boost=${GROUP_HARD_BOOST:-12.0} \
  ...
  --set loss_weights.teacher_pcd_direct=${TEACHER_PCD_DIRECT_WEIGHT:-26.0} \
  ...
  --set training.teacher_pcd_direct_ranking_weight=${TEACHER_PCD_DIRECT_RANK_WEIGHT:-7.0} \
  ...
  --set training.teacher_pcd_direct_nominal_max_pred_pcd=${TEACHER_PCD_DIRECT_NOMINAL_MAX_PRED:-0.42} \
  ...
  --set training.teacher_pcd_direct_positive_rank_all_weight=${TEACHER_PCD_DIRECT_POS_RANK_ALL_WEIGHT:-2.0} \
  --set training.teacher_pcd_direct_positive_floor_weight=${TEACHER_PCD_DIRECT_POS_FLOOR_WEIGHT:-2.5} \
  --set training.teacher_pcd_direct_positive_min_pred_r_dep=${TEACHER_PCD_DIRECT_POS_MIN_R_DEP:-0.08} \
  --set training.teacher_pcd_direct_positive_max_pred_gap=${TEACHER_PCD_DIRECT_POS_MAX_GAP:-0.18} \
  --set training.teacher_pcd_direct_positive_min_pred_drs=${TEACHER_PCD_DIRECT_POS_MIN_DRS:-0.90} \
  ...
  2>&1 | tee "$MODEL_DIR/train_v34_tail_challenge.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$VAL_MIX" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_mix_v34.json" \
  --set calibration.required_min_for_delta=100 \
  2>&1 | tee "$CAL_DIR/calibrate_mix_v34.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_safe" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_safe_v34.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_safe_v34.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_near_contact" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_near_v34.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_near_v34.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_contact" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_contact_v34.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_contact_v34.log"

python tools/write_gamma_by_bucket.py \
  --safe "$CAL_DIR/calibration_safe_v34.json" \
  --near "$CAL_DIR/calibration_near_v34.json" \
  --contact "$CAL_DIR/calibration_contact_v34.json" \
  --delta 0.05 \
  --output "$CAL_DIR/gamma_rec_by_bucket_v34.json" \
  2>&1 | tee "$CAL_DIR/write_gamma_v34.log"

printf '\n[v34] model: %s\n[v34] calibration: %s\n[v34] gamma map: %s\n' "$MODEL_DIR/best.pt" "$CAL_DIR/calibration_mix_v34.json" "$CAL_DIR/gamma_rec_by_bucket_v34.json"
