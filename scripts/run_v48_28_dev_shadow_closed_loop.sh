...
OUT="${OUT:?OUT is required; point it to the v48.28 or repair-source dedicated run}"
...
python tools/audit_v48_28_shadow_provenance.py \
...
(root/'DIAGNOSTIC_ONLY_NO_PAPER.json').write_text(json.dumps({
 'event':'v48_28_dev_shadow_closed_loop','created_unix':time.time(),
...
run_variant() {
  ...
  set +e
  python tools/check_v48_28_regime_targets.py "$run" \
...
doc={'event':'v48_28_dev_shadow_complete','created_unix':time.time(),
