...
# v33 is a brake-tail residual contact correction on top of the v33 model.  It tests
# whether a narrow residual brake-tail certificate fixes the remaining v32
# contact miss without reopening merge/stabilize false positives. Override CKPT/CAL/GAMMA if you place the model elsewhere.
export RUN=${RUN:-runs/ocrap_v33_brake_tail_eval}
...
make_sel() {
  ...
  case "$tag" in
    ...
    v33|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=true; pcd_rescue=true; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    # v33 residual brake-tail certificate: captures contact cases where paper-best
    ...
    --set selection.brake_tail_rescue_certificate_by_bucket.near_contact=false
    --set selection.brake_tail_rescue_certificate_by_bucket.test_near_contact=false
    --set selection.brake_tail_rescue_certificate_by_bucket.contact=${CONTACT_BRAKE_TAIL:-true}
    --set selection.brake_tail_rescue_certificate_by_bucket.test_contact=${CONTACT_BRAKE_TAIL:-true}
    ...
    --set selection.brake_tail_min_candidate_gap_by_bucket.contact=${CONTACT_TAIL_MIN_GAP:-0.10}
    --set selection.brake_tail_min_candidate_gap_by_bucket.test_contact=${CONTACT_TAIL_MIN_GAP:-0.10}
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v33_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v33_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v33_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v33_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v33.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v33.log"
  assert_json "$RUN/closed_loop_safe_fast_v33.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v33.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v33 =====")
for p in sorted(root.glob('eval_*_v33_*.json')):
...
for p in sorted(root.glob('*v33*.json')):
...
run_eval v33 near_contact 1 &
wait
run_eval scalar contact 0 &
run_eval v33 contact 1 &
wait

# Stress audits. Two GPUs run baseline and v24 concurrently.
run_audit scalar near_contact 0 32 384 &
run_audit v33 near_contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait
run_audit scalar contact 0 32 384 &
run_audit v33 contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait

if [[ "${RUN_NEAR_CHALLENGE_ABLATION:-0}" == "1" ]]; then
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_eval v33 near_contact 0
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_audit v33 near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384}
fi

if [[ "${RUN_DEEP_CONTACT:-0}" == "1" ]]; then
  run_audit v33 contact 0 ${DEEP_CONTACT_TARGETS:-48} ${DEEP_CONTACT_LABELS:-640}
fi
