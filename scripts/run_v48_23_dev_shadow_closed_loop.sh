...
OUT="${OUT:?OUT is required; point it to the v48.23 dedicated run}"
...
(root/'DIAGNOSTIC_ONLY_NO_PAPER.json').write_text(json.dumps({
 'event':'v48_23_dev_shadow_closed_loop','created_unix':time.time(),
...
run_variant() {
  ...
  set +e
  python tools/check_v48_23_regime_targets.py "$run" \
...
doc={'event':'v48_23_dev_shadow_complete','created_unix':time.time(),
