...
# v48.13 OC-TRAC-TERRA
# Stage P: recovery tournament learns a small teacher-acceptable proposal set.
# Stage E: regime-specific ordered evidence is trained on every frozen proposal member,
#          uses same-group counterfactual comparisons, and reranks only inside the proposal.
...
EVIDENCE_RERANK_TOP_K="${EVIDENCE_RERANK_TOP_K:-true}"
case "$EVIDENCE_RERANK_TOP_K" in
  1|true|TRUE|yes|YES) POLICY_FIRST_CONTRACT=false ;;
  *) POLICY_FIRST_CONTRACT=true ;;
esac
...
{
  "version": "v48.13-TERRA",
  "policy_semantics": "topk_recovery_proposal_then_intragroup_ordinal_evidence_rerank",
  "set_tournament_hidden": $TOURNAMENT_HIDDEN,
  "set_tournament_heads": $TOURNAMENT_HEADS,
  "stage_p_trainable": ["direct_preference_set_ranker"],
...
env "${COMMON_ENV[@]}" \
  ...
  PREFERENCE_CONDITIONAL_PAIRWISE_WEIGHT="${PREFERENCE_CONDITIONAL_PAIRWISE_WEIGHT:-0.0}" \
  PREFERENCE_PROPOSAL_TOPK_WEIGHT="${PREFERENCE_PROPOSAL_TOPK_WEIGHT:-1.25}" \
  PREFERENCE_PROPOSAL_TOPK="$PROPOSAL_TOP_K" \
  PREFERENCE_PROPOSAL_MARGIN="${PREFERENCE_PROPOSAL_MARGIN:-0.020}" \
...
for bucket in near contact; do
  ...
  CUDA_VISIBLE_DEVICES="$TRAIN_GPU" python -u tools/calibrate_policy_risk_v48.py \
    ...
    --risk-source heads --conditional-recovery-ranking --proposal-top-k "$PROPOSAL_TOP_K" \
...
env "${COMMON_ENV[@]}" \
  ...
  BEST_METRIC=direct_certificate_risk_fold_robust SKIP_POST_TRAIN_CALIBRATION=1 \
  POLICY_METRIC_RISK_SOURCE=ordinal_evidence \
  POLICY_METRIC_PROPOSAL_TOP_K="$PROPOSAL_TOP_K" \
  POLICY_METRIC_EVIDENCE_RERANK_TOP_K="$EVIDENCE_RERANK_TOP_K" \
  POINT_WEIGHT=0 VALUE_LISTWISE_WEIGHT=0 CENTERED_WEIGHT=0 ADVANTAGE_WEIGHT=0 \
  SETWISE_W=0 PREFERENCE_WEIGHT=0 PREFERENCE_REGRET_WEIGHT=0 \
  PREFERENCE_LISTWISE_WEIGHT=0 PREFERENCE_GAP_WEIGHT=0 PREFERENCE_SET_WEIGHT=0 \
  ...
  ORDINAL_EVIDENCE_ORDERED_NLL_TOP1_WEIGHT="${ORDERED_EVIDENCE_TOP1_WEIGHT:-0.50}" \
  ORDINAL_EVIDENCE_ORDERED_NLL_ALL_WEIGHT="${ORDERED_EVIDENCE_ALL_WEIGHT:-0.10}" \
  ORDINAL_EVIDENCE_PROPOSAL_TOPK_WEIGHT="${ORDERED_EVIDENCE_PROPOSAL_TOPK_WEIGHT:-3.00}" \
  ORDINAL_EVIDENCE_PROPOSAL_TOPK="$PROPOSAL_TOP_K" \
  ORDINAL_EVIDENCE_PROPOSAL_RANK_DECAY="${ORDERED_EVIDENCE_PROPOSAL_RANK_DECAY:-0.75}" \
  ORDINAL_EVIDENCE_INTRAGROUP_BENEFIT_WEIGHT="${ORDERED_EVIDENCE_INTRAGROUP_BENEFIT_WEIGHT:-0.80}" \
  ORDINAL_EVIDENCE_INTRAGROUP_HARM_WEIGHT="${ORDERED_EVIDENCE_INTRAGROUP_HARM_WEIGHT:-1.80}" \
  ORDINAL_EVIDENCE_INTRAGROUP_MARGIN="${ORDERED_EVIDENCE_INTRAGROUP_MARGIN:-0.25}" \
  ...
  ORDINAL_EVIDENCE_PAIRWISE_BENEFIT_WEIGHT="${ORDERED_EVIDENCE_PAIRWISE_BENEFIT_WEIGHT:-0.0}" \
  ORDINAL_EVIDENCE_PAIRWISE_HARM_WEIGHT="${ORDERED_EVIDENCE_PAIRWISE_HARM_WEIGHT:-0.0}" \
  ORDINAL_EVIDENCE_PAIRWISE_MARGIN="${ORDERED_EVIDENCE_PAIRWISE_MARGIN:-0.25}" \
  bash scripts/train_ocrap_v48_trac_sr.sh

python - "$RUN" "$PREF_CKPT" "$FINAL_MODEL/best.pt" "$PROPOSAL_TOP_K" "$POLICY_FIRST_CONTRACT" "$EVIDENCE_RERANK_TOP_K" <<'PY'
import hashlib,json,pathlib,sys,time
run,pref,final=map(pathlib.Path,sys.argv[1:4])
for p in (pref,final):
    if not p.is_file(): raise SystemExit(f'missing staged checkpoint: {p}')
doc={
  'event':'v48_13_terra_staged_training_complete','created_unix':time.time(),
  ...
  'policy_first_no_fallback':str(sys.argv[5]).lower()=='true',
  'evidence_rerank_top_k':str(sys.argv[6]).lower()=='true',
  'proposal_top_k':int(sys.argv[4]),
}
...
POLICY_FIRST_NO_FALLBACK=$POLICY_FIRST_CONTRACT
PROPOSAL_TOP_K=$PROPOSAL_TOP_K
EVIDENCE_RERANK_TOP_K=$EVIDENCE_RERANK_TOP_K
MACRO_CONSTRAINT_MODE=${MACRO_CONSTRAINT_MODE:-opportunity_normalized}
