...
EVIDENCE_ADMISSION_PRIOR_MODE=benefit_only \
...
EVIDENCE_ADAPT_PATIENCE="${FACTOR_PATIENCE:-${EVIDENCE_ADAPT_PATIENCE:-6}}" \
  bash scripts/adapt_ocrap_v48_29_veto_rank_single_stage.sh
...
EVIDENCE_ADMISSION_PRIOR_MODE="${EVIDENCE_ADMISSION_PRIOR_MODE:-benefit_only}" \
...
ORDINAL_EVIDENCE_ADMISSION_WEIGHT="${ADMISSION_BINARY_WEIGHT:-0.25}" \
SETWISE_W="${ADMISSION_SETWISE_WEIGHT:-0.25}" \
SELECTIVE_RISK_WEIGHT=0 SELECTIVE_COVERAGE_WEIGHT=0 \
OPPORTUNITY_ADMISSION_WEIGHT=0 HARM_ADMISSION_WEIGHT=0 \
EVIDENCE_ADMISSION_BOUNDED=true \
BEST_METRIC=direct_integrity_selection_risk EVALUATE_INITIAL_CHECKPOINT=false \
...
python tools/check_v48_29_factor_transfer.py \
...
doc={
 'event':'v48_29_two_stage_veto_rank_complete','created_unix':time.time(),
 ...
 'admission_prior_mode':'benefit_only',
 'safe_rank_objective':'best_safe_vs_nominal_and_hardest_non_safe',
