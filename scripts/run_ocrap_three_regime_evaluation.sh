...
: "${MODEL_RUN:?MODEL_RUN is required}"
...
: "${OCRAP_SELECTOR:=}"
: "${CONTACT_REQUIRE_ABSOLUTE_ADMISSION_FOR_INTERVENTION:=$REQUIRE_ABSOLUTE_ADMISSION_FOR_INTERVENTION}"
: "${CONTACT_OCRAP_SELECTOR:=$OCRAP_SELECTOR}"
: "${RESUME_FORCE:=false}"
: "${RESUME:=true}"
...
run_one() {
  ...
  env RUN_DIR="$OUT/$regime" OUTPUT="$OUT/$regime/closed_loop_ocrap.json" \
    ...
    REQUIRE_ABSOLUTE_ADMISSION_FOR_INTERVENTION="$([[ "$regime" == contact ]] && printf %s "$CONTACT_REQUIRE_ABSOLUTE_ADMISSION_FOR_INTERVENTION" || printf %s "$REQUIRE_ABSOLUTE_ADMISSION_FOR_INTERVENTION")" \
    OCRAP_SELECTOR="$([[ "$regime" == contact ]] && printf %s "$CONTACT_OCRAP_SELECTOR" || printf %s "$OCRAP_SELECTOR")" \
...
run_one_status() {
  ...
  started="$(runtime_iso_now)"
  if [[ "$enabled" != 1 ]]; then
    write_phase "$regime" skipped 0 "$started" "$(runtime_iso_now)"
    return 0
  fi
  ...
  write_phase "$regime" "$status" "$rc" "$started" "$(runtime_iso_now)"
  return "$rc"
}
