...
OUT="${OUT:?OUT is required; point it to the v48.29 or repair-source dedicated run}"
...
python tools/audit_v48_29_shadow_provenance.py \
...
(root/'DIAGNOSTIC_ONLY_NO_PAPER.json').write_text(json.dumps({
 'event':'v48_29_dev_shadow_closed_loop','created_unix':time.time(),
...
run_variant() {
  ...
  python tools/check_v48_29_shadow_runtime_contract.py \
  ...
  set +e
  python tools/check_v48_29_regime_targets.py "$run" \
...
doc={'event':'v48_29_dev_shadow_complete','created_unix':time.time(),
