...
export RUN=${RUN:-runs/ocrap_v32_pcd_rank2}
export MODEL_DIR=${MODEL_DIR:-$RUN/model_v32_pcd_rank2}
...
# v32 contact brake/yield PCD-rank2: start from v28 residual-DDC, then fine-tune
...
CUDA_VISIBLE_DEVICES="$TRAIN_GPU" PYTHONUNBUFFERED=1 python -u -m ocrap.cli train \
  ...
  --set training.group_batch_hard_min_r_dep=${GROUP_HARD_MIN_R_DEP:-0.35} \
  --set training.group_batch_hard_boost=${GROUP_HARD_BOOST:-6.0} \
  ...
  --set loss_weights.teacher_pcd_direct=${TEACHER_PCD_DIRECT_WEIGHT:-18.0} \
  ...
  --set training.teacher_pcd_direct_regression_weight=${TEACHER_PCD_DIRECT_REG_WEIGHT:-1.5} \
  --set training.teacher_pcd_direct_ranking_weight=${TEACHER_PCD_DIRECT_RANK_WEIGHT:-5.0} \
  --set training.teacher_pcd_direct_nominal_penalty_weight=${TEACHER_PCD_DIRECT_NOMINAL_WEIGHT:-2.5} \
  --set training.teacher_pcd_direct_false_positive_weight=${TEACHER_PCD_DIRECT_FALSE_POS_WEIGHT:-1.4} \
  --set training.teacher_pcd_direct_margin=${TEACHER_PCD_DIRECT_MARGIN:-0.28} \
  ...
  --set training.teacher_pcd_direct_target_min_pred_pcd=${TEACHER_PCD_DIRECT_TARGET_MIN_PRED:-0.56} \
  --set training.teacher_pcd_direct_nominal_max_pred_pcd=${TEACHER_PCD_DIRECT_NOMINAL_MAX_PRED:-0.47} \
  ...
  --set training.teacher_pcd_direct_component_weight=${TEACHER_PCD_DIRECT_COMPONENT_WEIGHT:-1.2} \
  --set training.teacher_pcd_direct_positive_component_weight=${TEACHER_PCD_DIRECT_POS_COMPONENT_WEIGHT:-2.0} \
  --set training.teacher_pcd_direct_nominal_cap_weight=${TEACHER_PCD_DIRECT_NOMINAL_CAP_WEIGHT:-2.0} \
  ...
  2>&1 | tee "$MODEL_DIR/train_v32_pcd_rank2.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$VAL_MIX" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_mix_v32.json" \
...
PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  ...
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_safe_v32.json" \
...
PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  ...
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_near_v32.json" \
  --set calibration.required_min_for_delta=50 \
...
PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  ...
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_contact_v32.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_contact_v31.log"

python tools/write_gamma_by_bucket.py \
  --safe "$CAL_DIR/calibration_safe_v32.json" \
  --near "$CAL_DIR/calibration_near_v32.json" \
  --contact "$CAL_DIR/calibration_contact_v32.json" \
  --delta 0.05 \
  --output "$CAL_DIR/gamma_rec_by_bucket_v32.json" \
  2>&1 | tee "$CAL_DIR/write_gamma_v31.log"

printf '\n[v32] model: %s\n[v32] calibration: %s\n[v32] gamma map: %s\n' "$MODEL_DIR/best.pt" "$CAL_DIR/calibration_mix_v32.json" "$CAL_DIR/gamma_rec_by_bucket_v32.json"
