...
# Preserve the contract that actually generated already-complete population runs
# before replacing the top-level runtime contract with this engineering hotfix.
# This permits deterministic resume without pretending old GPU evidence was
# generated by post-run orchestration code.
...
if [[ "$FULL_RESULTS_PRESENT" == 1 && ! -f "$FULL_RUN_RUNTIME" && -f "$RUNTIME" ]]; then
  python - "$RUNTIME" "$FULL_RUN_RUNTIME" <<'PY'
...
try:
    ...
    doc={}
# Only the original production contract is licensed to provenance already-
...
# Fresh V48.124.2 runs use this runtime for the population evidence; resumed
# V48.124.1 population runs keep the preserved earlier contract above.
