...
SOURCE_RUN="${SOURCE_RUN:?Set SOURCE_RUN to a completed v48.13 training run}"
...
for raw_seed in "${seed_array[@]}"; do
  ...
  calibrate_variant() {
    ...
    contract="$SOURCE_RUN/candidates/$variant/POLICY_CONTRACT.env"
    [[ -f "$contract" ]] || { echo "missing policy contract: $contract" >&2; return 2; }
    set -a; source "$contract"; set +a
    proposal_args=(--proposal-top-k "${PROPOSAL_TOP_K:-1}")
    case "${EVIDENCE_RERANK_TOP_K:-false}" in
      1|true|TRUE|yes|YES) proposal_args+=(--evidence-rerank-top-k) ;;
    esac
    policy_args=()
    case "${POLICY_FIRST_NO_FALLBACK:-false}" in
      1|true|TRUE|yes|YES) policy_args+=(--policy-first-no-fallback) ;;
    esac
    set +e
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket near --risk-source "${RISK_SOURCE:-ordinal_evidence}" --conformal-alpha="${CONFORMAL_ALPHA:-0.10}" --conformal-temperature="${CONFORMAL_TEMPERATURE:-0.02}" --conformal-scope="${CONFORMAL_SCOPE:-policy_top1}" --conditional-recovery-ranking "${policy_args[@]}" "${proposal_args[@]}" \
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket contact --risk-source "${RISK_SOURCE:-ordinal_evidence}" --conformal-alpha="${CONFORMAL_ALPHA:-0.10}" --conformal-temperature="${CONFORMAL_TEMPERATURE:-0.02}" --conformal-scope="${CONFORMAL_SCOPE:-policy_top1}" --conditional-recovery-ranking "${policy_args[@]}" "${proposal_args[@]}" \
...
python tools/summarize_v48_7_multiseed.py \
  --root "$MULTISEED_ROOT" \
  --output "$MULTISEED_ROOT/multiseed_summary_v48_13.json"
printf '{"complete":true,"version":"v48.13","seeds":"%s","risk_source":"%s","macro_constraint_mode":"%s"}\n' "$SEEDS" "${RISK_SOURCE:-ordinal_evidence}" "${MACRO_CONSTRAINT_MODE:-opportunity_normalized}" \
