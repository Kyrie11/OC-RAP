...
export MODEL_DIR=${MODEL_DIR:-$RUN/model_v22}
export CAL_DIR=${CAL_DIR:-$RUN/calibration}
mkdir -p "$MODEL_DIR" "$CAL_DIR"

# v22 trains the model head that v20 implicitly assumed was already reliable:
...
PYTHONUNBUFFERED=1 python -u -m ocrap.cli train \
  ...
  2>&1 | tee "$MODEL_DIR/train_v22.log"
...
PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  ...
  --output "$CAL_DIR/calibration_mix_v22.json" \
  --set calibration.required_min_for_delta=100 \
  2>&1 | tee "$CAL_DIR/calibrate_mix_v22.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_safe" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_safe_v22.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_safe_v22.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_near_contact" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_near_v22.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_near_v22.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_contact" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_contact_v22.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_contact_v22.log"

python tools/write_gamma_by_bucket.py \
  --safe "$CAL_DIR/calibration_safe_v22.json" \
  --near "$CAL_DIR/calibration_near_v22.json" \
  --contact "$CAL_DIR/calibration_contact_v22.json" \
  --delta 0.05 \
  --output "$CAL_DIR/gamma_rec_by_bucket_v22.json" \
  2>&1 | tee "$CAL_DIR/write_gamma_v22.log"

printf '\n[v22] model: %s\n[v22] calibration: %s\n[v22] gamma map: %s\n' "$MODEL_DIR/best.pt" "$CAL_DIR/calibration_mix_v22.json" "$CAL_DIR/gamma_rec_by_bucket_v22.json"
