...
# v29 is an audit-frontier DDC/Guarded-NARC correction on top of the v22 model.  By default it
...
export RUN=${RUN:-runs/ocrap_v29_contact_frontier}
...
make_sel() {
  ...
  case "$tag" in
    ...
    v29|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=true; pcd_rescue=true; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    --set selection.rescue_challenge_max_candidate_gap_by_bucket.contact=${CONTACT_CHALLENGE_MAX_GAP:-0.95}
    --set selection.rescue_challenge_max_candidate_gap_by_bucket.test_contact=${CONTACT_CHALLENGE_MAX_GAP:-0.95}
    ...
    --set selection.rescue_challenge_min_pred_pcd_by_bucket.contact=${CONTACT_CHALLENGE_MIN_PCD:-0.22}
    --set selection.rescue_challenge_min_pred_pcd_by_bucket.test_contact=${CONTACT_CHALLENGE_MIN_PCD:-0.22}
  ...
  if [[ "$is_v27" == "true" ]]; then
    COMMON_SEL+=(
      ...
      --set selection.intervention_budget_rate_by_bucket.contact=${CONTACT_BUDGET_RATE:-0.10}
      --set selection.intervention_budget_rate_by_bucket.test_contact=${CONTACT_BUDGET_RATE:-0.10}
      --set selection.intervention_budget_hard_by_bucket.contact=false
      --set selection.intervention_budget_hard_by_bucket.test_contact=false
      --set selection.intervention_cooldown_steps_by_bucket.contact=${CONTACT_COOLDOWN_STEPS:-2}
      --set selection.intervention_cooldown_steps_by_bucket.test_contact=${CONTACT_COOLDOWN_STEPS:-2}
      ...
      --set selection.rescue_challenge_min_rec_lcb_gain_by_bucket.contact=${CONTACT_CHALLENGE_MIN_REC_GAIN:-0.000}
      --set selection.rescue_challenge_min_rec_lcb_gain_by_bucket.test_contact=${CONTACT_CHALLENGE_MIN_REC_GAIN:-0.000}
      ...
      --set selection.rescue_challenge_min_gap_reduction_by_bucket.contact=${CONTACT_CHALLENGE_MIN_GAP_REDUCTION:-0.000}
      --set selection.rescue_challenge_min_gap_reduction_by_bucket.test_contact=${CONTACT_CHALLENGE_MIN_GAP_REDUCTION:-0.000}
      ...
      --set selection.rescue_challenge_score_pcd_weight_by_bucket.contact=1.60
      --set selection.rescue_challenge_score_pcd_weight_by_bucket.test_contact=1.60
      ...
      --set selection.rescue_challenge_score_gap_weight_by_bucket.contact=0.25
      --set selection.rescue_challenge_score_gap_weight_by_bucket.test_contact=0.25
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v29_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v29_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v29_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v29_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v29.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v29.log"
  assert_json "$RUN/closed_loop_safe_fast_v29.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v29.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v29 =====")
for p in sorted(root.glob('eval_*_v29_*.json')):
...
for p in sorted(root.glob('*v29*.json')):
...
run_eval v29 near_contact 1 &
wait
run_eval scalar contact 0 &
run_eval v29 contact 1 &
wait

# Stress audits. Two GPUs run baseline and v24 concurrently.
run_audit scalar near_contact 0 32 384 &
run_audit v29 near_contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait
run_audit scalar contact 0 32 384 &
run_audit v29 contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
wait

if [[ "${RUN_NEAR_CHALLENGE_ABLATION:-0}" == "1" ]]; then
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_eval v29 near_contact 0
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_audit v29 near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384}
fi

if [[ "${RUN_DEEP_CONTACT:-0}" == "1" ]]; then
  run_audit v29 contact 0 ${DEEP_CONTACT_TARGETS:-48} ${DEEP_CONTACT_LABELS:-640}
fi
