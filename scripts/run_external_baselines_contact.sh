...
if runtime_bool_true "$RUN_LEGACY_CONTACT"; then
...
CONTACT_NONLEARNING_SPECS="$(python - "$CONFIG" "$METHODS_CSV" <<'PY_SPECS'
import sys
...
if runtime_bool_true "$DO_TRAIN"; then
  if ! runtime_bool_true "$FORCE_REREGISTER" && python tools/check_external_nonlearning_registration.py \
      --root "$RUN" --dataset "$TRAIN_CONTACT" --val-dataset "$VAL_CONTACT" --specs "$CONTACT_NONLEARNING_SPECS" >/dev/null 2>&1; then
    echo "[REUSE] Contact non-learning baseline registrations are compatible"
  else
...
if runtime_bool_true "$DO_CLOSED_LOOP"; then run_queue run_closed_loop_method "${METHODS[@]}"; fi
