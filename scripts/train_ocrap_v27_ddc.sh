...
export RUN=${RUN:-runs/ocrap_v27_ddc}
export MODEL_DIR=${MODEL_DIR:-$RUN/model_v27_ddc}
export CAL_DIR=${CAL_DIR:-$RUN/calibration}
export TRAIN_GPU=${TRAIN_GPU:-0}
mkdir -p "$MODEL_DIR" "$CAL_DIR"

# v27 trains the selector heads that v26 exposed as the limiting factor:
# post-contact/low-headroom deployability dominance over an over-confident nominal.
CUDA_VISIBLE_DEVICES="$TRAIN_GPU" PYTHONUNBUFFERED=1 python -u -m ocrap.cli train \
  --dataset "$TRAIN_MIX" \
  --val-dataset "$VAL_MIX" \
  --output "$MODEL_DIR" \
  --set training.epochs=${EPOCHS:-45} \
  --set training.batch_size=${BATCH_SIZE:-64} \
  --set training.lr=${LR:-0.00025} \
  --set training.weight_decay=0.0001 \
  --set training.artifact_sampler_weight=1.0 \
  --set training.negative_deployable_sampler_weight=1.25 \
  --set training.regime_balance_power=0.65 \
  ...
  --set loss_weights.group_ce=0.32 \
  --set loss_weights.group_distill=0.20 \
  --set loss_weights.nominal_switch=0.12 \
  --set loss_weights.safe_nominal=0.28 \
  --set loss_weights.protective_macro=0.65 \
  --set loss_weights.macro_drs=1.20 \
  --set loss_weights.ddc=${DDC_WEIGHT:-0.85} \
  ...
  --set training.ddc_margin=${DDC_MARGIN:-0.14} \
  --set training.ddc_min_teacher_pcd_gain=${DDC_MIN_TEACHER_PCD_GAIN:-0.035} \
  --set training.ddc_min_teacher_best_pcd=${DDC_MIN_TEACHER_BEST_PCD:-0.50} \
  --set training.ddc_max_nominal_teacher_pcd=${DDC_MAX_NOMINAL_TEACHER_PCD:-0.62} \
  --set training.ddc_pred_gap_weight=0.22 \
  --set training.ddc_pred_drs_weight=0.38 \
  --set training.ddc_utility_weight=0.00 \
  --set training.ddc_target_min_pred_pcd=${DDC_TARGET_MIN_PRED_PCD:-0.46} \
  --set training.ddc_nominal_max_pred_pcd=${DDC_NOMINAL_MAX_PRED_PCD:-0.54} \
  2>&1 | tee "$MODEL_DIR/train_v27_ddc.log"
...
PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  ...
  --output "$CAL_DIR/calibration_mix_v27.json" \
  --set calibration.required_min_for_delta=100 \
  2>&1 | tee "$CAL_DIR/calibrate_mix_v27.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_safe" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_safe_v27.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_safe_v27.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_near_contact" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_near_v27.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_near_v27.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_contact" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_contact_v27.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_contact_v27.log"

python tools/write_gamma_by_bucket.py \
  --safe "$CAL_DIR/calibration_safe_v27.json" \
  --near "$CAL_DIR/calibration_near_v27.json" \
  --contact "$CAL_DIR/calibration_contact_v27.json" \
  --delta 0.05 \
  --output "$CAL_DIR/gamma_rec_by_bucket_v27.json" \
  2>&1 | tee "$CAL_DIR/write_gamma_v27.log"

printf '\n[v27] model: %s\n[v27] calibration: %s\n[v27] gamma map: %s\n' "$MODEL_DIR/best.pt" "$CAL_DIR/calibration_mix_v27.json" "$CAL_DIR/gamma_rec_by_bucket_v27.json"
