...
SOURCE_RUN="${SOURCE_RUN:?Set SOURCE_RUN to a completed v48.12 training run}"
...
for raw_seed in "${seed_array[@]}"; do
  ...
  calibrate_variant() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket near --risk-source "${RISK_SOURCE:-ordinal_evidence}" --conformal-alpha="${CONFORMAL_ALPHA:-0.10}" --conformal-temperature="${CONFORMAL_TEMPERATURE:-0.02}" --conformal-scope="${CONFORMAL_SCOPE:-policy_top1}" --conditional-recovery-ranking --policy-first-no-fallback \
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket contact --risk-source "${RISK_SOURCE:-ordinal_evidence}" --conformal-alpha="${CONFORMAL_ALPHA:-0.10}" --conformal-temperature="${CONFORMAL_TEMPERATURE:-0.02}" --conformal-scope="${CONFORMAL_SCOPE:-policy_top1}" --conditional-recovery-ranking --policy-first-no-fallback \
...
python tools/summarize_v48_7_multiseed.py \
  --root "$MULTISEED_ROOT" \
  --output "$MULTISEED_ROOT/multiseed_summary_v48_12.json"
printf '{"complete":true,"version":"v48.12","seeds":"%s","risk_source":"%s","macro_constraint_mode":"%s"}\n' "$SEEDS" "${RISK_SOURCE:-ordinal_evidence}" "${MACRO_CONSTRAINT_MODE:-opportunity_normalized}" \
