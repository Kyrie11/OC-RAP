...
[[ -d "${WOMD_ROOT}/training" ]] || { echo "missing WOMD training directory" >&2; exit 2; }
mkdir -p "${SHARD_ROOT}" "${OCRAP_ROOT}/reports"
...
build_worker 0 "${GPU0}" >"${SHARD_ROOT}/worker0.launch.log" 2>&1 & P0=$!
build_worker 1 "${GPU1}" >"${SHARD_ROOT}/worker1.launch.log" 2>&1 & P1=$!
set +e
wait "${P0}"; S0=$?
wait "${P1}"; S1=$?
set -e
[[ "${S0}" == 0 && "${S1}" == 0 ]] || {
  echo "train_safe workers failed: worker0=${S0}, worker1=${S1}" >&2
  exit 3
}
