...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_27_factor_physics_dedicated_4827}"
...
failed={'event':'v48_27_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_27_factor_physics_bridge_controller_complete','created_unix':time.time(),
...
run_variant() {
  ...
  PROPOSAL_TOP_K="$PROPOSAL_TOP_K" \
    bash scripts/adapt_ocrap_v48_27_factor_physics_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_27.json" --version v48.27-FACTOR-PHYSICS-BRIDGE || true
  exit 30
fi
...
# v48.27 fail-closed preflight: training and downstream inference must
...
for variant in balanced precision; do
  ...
  set +e
  python tools/check_v48_27_model_contract.py \
    --checkpoint "$ckpt" \
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" \
  OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit bash scripts/calibrate_v48_27_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
