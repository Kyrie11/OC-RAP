...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_25_integrity_dedicated_4825}"
...
failed={'event':'v48_25_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_25_integrity_bridge_controller_complete','created_unix':time.time(),
...
(root/'V48_25_COMPLETE.json').write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
PY
}
...
run_variant() {
  ...
  PROPOSAL_TOP_K="$PROPOSAL_TOP_K" \
    bash scripts/adapt_ocrap_v48_25_integrity_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_25.json" --version v48.25-INTEGRITY-BRIDGE || true
  exit 30
fi
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" \
  bash scripts/calibrate_v48_25_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
