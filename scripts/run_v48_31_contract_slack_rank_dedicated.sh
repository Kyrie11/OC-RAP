...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_31_contract_slack_rank_dedicated_4831}"
...
failed={'event':'v48_31_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_31_contract_slack_rank_controller_complete','created_unix':time.time(),
...
(root/'V48_31_COMPLETE.json').write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
PY
}
...
run_variant() {
  ...
  bash scripts/adapt_ocrap_v48_31_contract_slack_rank_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_31.json" --version v48.31-CONTRACT-SLACK-RANK || true
  exit 30
fi
...
# v48.31 fail-closed preflight: training and downstream inference must
...
for variant in balanced precision; do
  ...
  set +e
  python tools/check_v48_31_training_contract.py \
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" \
  OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit bash scripts/calibrate_v48_31_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
