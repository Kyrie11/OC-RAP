...
ROOT="${ABLATION_ROOT:-runs/ocrap_v48_26_execution_physics_ablations_4826}"
...
doc={'complete':False,'version':'v48.26-EXECUTION-PHYSICS-BRIDGE','stage':stage,
...
run_task() {
  ...
  local proposal_k=3 safe_reg=0.0 safe_list=0.0 frontier_pairwise=0.25 benefit_listwise=0.0
  ...
  case "$group" in
    ...
    C_add_execution_exact_safe_utility)
      best_metric=direct_integrity_selection_risk
      safe_reg=1.0; safe_list=0.75
      ;;
    D_full_execution_physics_bridge)
      best_metric=direct_integrity_selection_risk; admission_bounded=false
      safe_reg=1.0; safe_list=0.75; benefit_listwise=0.75; frontier_pairwise=0.75
      ;;
  ...
  EVIDENCE_ADMISSION_BOUNDED="$admission_bounded" EVALUATE_INITIAL_CHECKPOINT=true BEST_METRIC="$best_metric" \
    bash scripts/adapt_ocrap_v48_26_execution_physics_variant.sh >"$out/logs/adapt.log" 2>&1
  local adapt_rc=$?
  set -e
  if [[ "$adapt_rc" != 0 ]]; then
  ...
  GPU0="$gpu" GPU1="$gpu" VARIANTS="$variant" CERTIFICATE_GRID_SIZE="${CERTIFICATE_GRID_SIZE:-31}" \
    bash scripts/calibrate_v48_26_certificate_pool.sh >"$out/logs/calibrate.log" 2>&1
  local cert_rc=$?
  set -e
...
doc={'complete':True,'version':'v48.26-EXECUTION-PHYSICS-BRIDGE','group':group,'variant':variant,
...
groups=(A_engineering_contract_only B_add_safe_checkpoint_contract C_add_execution_exact_safe_utility D_full_execution_physics_bridge)
...
python tools/summarize_v48_14_ablations.py --root "$ROOT" --output "$ROOT/ablation_summary_v48_26.json" --version v48.26-EXECUTION-PHYSICS-BRIDGE || true
set +e
...
doc={'complete':not missing and failures==0,'version':'v48.26-EXECUTION-PHYSICS-BRIDGE','max_concurrent_tasks':2,
