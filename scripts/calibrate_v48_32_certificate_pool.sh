...
# Finalise a v48.32 CONTRACT-SLACK-RANK run using a scene-disjoint dedicated certificate pool.
...
: > "$OUTPUTDIR/logs/v48_32_certificate_status.log"
...
protocol={
 'version':'v48.32-CONTRACT-SLACK-RANK',
 ...
 'threshold_source':'evidence_adapt_dev',
...
doc={'event':'v48_32_gate_protocol_preregistered','created_unix':time.time(),
...
calibrate_variant() {
  ...
  for i in 0 1 2 3; do
    CUDA_VISIBLE_DEVICES="$gpu" python -u -m ocrap.cli calibrate \
      ...
      2>&1 | tee "$run/logs/v48_32_calibrate_${buckets[$i]}.log"
  done
  python tools/write_gamma_by_bucket.py \
    ...
    2>&1 | tee "$run/logs/v48_32_gamma.log"

  local common=(
    --checkpoint "$ckpt" --method-version=v48_32_provenance_margin_bridge_dev_frozen_policy_risk_certificate --risk-source="${RISK_SOURCE:-ordinal_evidence}"
  ...
  set +e
  # Thresholds are fitted only on adaptation-dev. These files are diagnostic
  # selectors and are never deployment-authorized by themselves.
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    --development-fit-only --output "$tmp/dev_frozen_rule_near_v48.json" \
    --required-min-groups=1 --required-min-scenes=1 \
    --min-fit-selected="$NEAR_MIN_VERIFY_SELECTED" --min-fit-precision-lcb="$NEAR_MIN_VERIFY_PRECISION_LCB" \
    --max-fit-harmful-group-ucb="$NEAR_MAX_VERIFY_HARM_UCB" \
    --max-fit-harmful-selected-ucb="$NEAR_MAX_VERIFY_SELECTED_HARM_UCB" \
    2>&1 | tee "$run/logs/v48_32_dev_rule_near.log"; local dn=${PIPESTATUS[0]}
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    --dataset "$DEV_CONTACT" --allowed-splits=evidence_adapt_dev --bucket contact "${common[@]}" \
    --development-fit-only --output "$tmp/dev_frozen_rule_contact_v48.json" \
    --required-min-groups=1 --required-min-scenes=1 \
    --min-fit-selected="$CONTACT_MIN_VERIFY_SELECTED" --min-fit-precision-lcb="$CONTACT_MIN_VERIFY_PRECISION_LCB" \
    --max-fit-harmful-group-ucb="$CONTACT_MAX_VERIFY_HARM_UCB" \
    --max-fit-harmful-selected-ucb="$CONTACT_MAX_VERIFY_SELECTED_HARM_UCB" \
    2>&1 | tee "$run/logs/v48_32_dev_rule_contact.log"; local dc=${PIPESTATUS[0]}
  if [[ "$dn" != 0 || "$dc" != 0 ]]; then
    set -e
    echo "adaptation-dev threshold fitting failed: near=$dn contact=$dc" >&2
    return 30
  fi

  # Fail closed before certificate access if checkpoint selection and dev-rule
  # fitting did not see exactly the same eligible groups/opportunities.
  python tools/check_v48_32_metric_calibration_contract.py \
    --train-summary "$run/model_v48_trac_sr/train_summary.json" \
    --near-rule "$tmp/dev_frozen_rule_near_v48.json" \
    --contact-rule "$tmp/dev_frozen_rule_contact_v48.json" \
    --output "$tmp/METRIC_CALIBRATION_CONTRACT.json"

  # The complete dedicated certificate is now verification-only. No label from
  # this population can alter opportunity/harm/score/rank thresholds.
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    --dataset "$CERT_NEAR" --allowed-splits=certificate_pool --bucket near "${common[@]}" \
    --verification-only --frozen-rule-json "$tmp/dev_frozen_rule_near_v48.json" \
    ...
    2>&1 | tee "$run/logs/v48_32_policy_near.log"; local sn=${PIPESTATUS[0]}
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    --dataset "$CERT_CONTACT" --allowed-splits=certificate_pool --bucket contact "${common[@]}" \
    --verification-only --frozen-rule-json "$tmp/dev_frozen_rule_contact_v48.json" \
    ...
    2>&1 | tee "$run/logs/v48_32_policy_contact.log"; local sc=${PIPESTATUS[0]}
  set -e
...
required=[
 ...
 'gamma_rec_by_bucket_v48.json','dev_frozen_rule_near_v48.json','dev_frozen_rule_contact_v48.json',
...
safe_status={
 'event':'v48_32_safe_regime_status','created_unix':time.time(),'variant':variant,
...
doc={'event':'v48_32_certificate_pool_calibration_complete','created_unix':time.time(),
 ...
 'scene_roles_disjoint':True,'threshold_source':'evidence_adapt_dev',
 'certificate_mode':'external_rule_full_verification',
...
PY
  ...
  mv "$tmp" "$final"
  rm -rf "$final.old"
  echo "$variant near=$sn contact=$sc" >> "$OUTPUTDIR/logs/v48_32_certificate_status.log"
...
status={'event':'v48_32_certificate_candidate_selection','created_unix':time.time(),
...
def write_blocked(reason, exit_code):
    doc={
      'event':'v48_32_next_commands_blocked','created_unix':time.time(),
      'reason':reason,'exit_code':int(exit_code),'gate_evaluated':reason=='natural_gate_failed',
...
commands=(
 ...
 f'''OUT={root} bash scripts/run_v48_32_stress_if_authorized.sh\n''')
(root/'NEXT_COMMANDS.txt').write_text(commands)
(root/'NEXT_COMMANDS_STATUS.json').write_text(json.dumps({
  'event':'v48_32_next_commands_generated','created_unix':time.time(),
  'generated':True,'gate_evaluated':True,'gate_passed':True,'chosen_base_run':str(chosen),
  'test_roots_read':False,
