...
export RUN=${RUN:-runs/ocrap_v45_rave_${VARIANT}}
export MODEL_DIR=${MODEL_DIR:-$RUN/model_v45_rave}
...
for bucket in mix safe near contact; do
  ...
  CUDA_VISIBLE_DEVICES="$TRAIN_GPU" python -u -m ocrap.cli calibrate --dataset "$data" --checkpoint "$MODEL_DIR/best.pt" \
    --output "$CAL_DIR/calibration_${bucket}_v45.json" --set calibration.required_min_for_delta="$min" \
    2>&1 | tee "$CAL_DIR/calibrate_${bucket}_v45.log"
done
python tools/write_gamma_by_bucket.py --safe "$CAL_DIR/calibration_safe_v45.json" --near "$CAL_DIR/calibration_near_v45.json" \
  --contact "$CAL_DIR/calibration_contact_v45.json" --delta 0.05 --output "$CAL_DIR/gamma_rec_by_bucket_v45.json"

BASE_RUN="$RUN" CKPT="$MODEL_DIR/best.pt" CAL_DIR="$CAL_DIR" CAL_GPU="$TRAIN_GPU" \
  bash scripts/calibrate_ocrap_v45_rave.sh
