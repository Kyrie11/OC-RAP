...
# Finalise a v48.36 OCAF run on the scene-disjoint dedicated
...
: > "$OUTPUTDIR/logs/v48_36_certificate_status.log"
...
case "$ABSOLUTE_FEASIBILITY_MODE" in off|native|learned|support_reserve) ;; *) echo "invalid ABSOLUTE_FEASIBILITY_MODE=$ABSOLUTE_FEASIBILITY_MODE" >&2; exit 31;; esac
...
doc={'event':'v48_36_gate_protocol_preregistered','version':'v48.36-OCAF','implementation_version':os.environ.get('OCRAP_IMPLEMENTATION_VERSION','v48.36.4-IDEMPOTENT-TERMINAL-STATE-HOTFIX'),'created_unix':time.time(),
...
calibrate_variant() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    2>&1 | tee "$run/logs/v48_36_dev_diagnostic_near.log"; local dn=${PIPESTATUS[0]}
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    --min-fit-selected=1 --min-fit-precision-lcb=0 \
    --max-fit-harmful-group-ucb=1 --max-fit-harmful-selected-ucb=1 \
    2>&1 | tee "$run/logs/v48_36_dev_diagnostic_contact.log"; local dc=${PIPESTATUS[0]}
  ...
  python -u tools/calibrate_shared_continuous_rule_v48_36.py \
    ...
    2>&1 | tee "$run/logs/v48_36_shared_dev_rule.log"; local ds=${PIPESTATUS[0]}
  if [[ "$ds" != 0 && "$ds" != 3 ]]; then
    set -e
  ...
  python tools/check_v48_36_metric_calibration_contract.py \
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    2>&1 | tee "$run/logs/v48_36_policy_near.log"; local sn=${PIPESTATUS[0]}
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    2>&1 | tee "$run/logs/v48_36_policy_contact.log"; local sc=${PIPESTATUS[0]}
  set -e
...
safe_status={
 'event':'v48_36_safe_regime_status','version':'v48.36-OCAF','created_unix':time.time(),'attempt_id':attempt_id,'variant':variant,
...
doc={'event':'v48_36_certificate_pool_calibration_complete','version':'v48.36-OCAF','created_unix':time.time(),'attempt_id':attempt_id,
...
PY
  ...
  mv "$tmp" "$final"
  rm -rf "$final.old"
  echo "$variant near=$sn contact=$sc" >> "$OUTPUTDIR/logs/v48_36_certificate_status.log"
...
status={'event':'v48_36_certificate_candidate_selection','version':'v48.36-OCAF','implementation_version':os.environ.get('OCRAP_IMPLEMENTATION_VERSION','v48.36.4-IDEMPOTENT-TERMINAL-STATE-HOTFIX'),'created_unix':time.time(),'attempt_id':attempt_id,
...
def write_blocked(reason, exit_code):
    doc={
      'event':'v48_36_next_commands_blocked','version':'v48.36-OCAF','implementation_version':os.environ.get('OCRAP_IMPLEMENTATION_VERSION','v48.36.4-IDEMPOTENT-TERMINAL-STATE-HOTFIX'),'created_unix':time.time(),'attempt_id':attempt_id,
...
commands=(
 ...
 f'BASE_RUN={shlex.quote(str(chosen))} RUN={shlex.quote(str(root / "safe_paired"))} SAFE_NOMINAL_ONLY=1 RUN_SAFE_PAIRED_SCALAR=1 SAFE_WOMD_SOURCE={shlex.quote(safe_source)} SAFE_RAW_MAX_SCENARIOS=0 bash scripts/run_v48_36_safe_noninferiority.sh\n'
...
atomic_json(root/'NEXT_COMMANDS_STATUS.json',{
  'event':'v48_36_next_commands_generated','version':'v48.36-OCAF','implementation_version':os.environ.get('OCRAP_IMPLEMENTATION_VERSION','v48.36.4-IDEMPOTENT-TERMINAL-STATE-HOTFIX'),'created_unix':time.time(),'attempt_id':attempt_id,
