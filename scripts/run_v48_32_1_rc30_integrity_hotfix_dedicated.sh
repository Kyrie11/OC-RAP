...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_32_1_rc30_integrity_hotfix_dedicated_48321}"
...
rm -f "$OUTPUTDIR"/ADAPTATION_FAILED_*.json "$OUTPUTDIR"/FAILURE_SIGNATURE_*.json "$OUTPUTDIR/PIPELINE_FAILED.json" \
      "$OUTPUTDIR/V48_32_1_COMPLETE.json" "$OUTPUTDIR/NEXT_COMMANDS.txt" \
...
failed={'event':'v48_32_1_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_32_1_rc30_integrity_hotfix_controller_complete','created_unix':time.time(),
...
(root/'V48_32_1_COMPLETE.json').write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
blocked={'event':'v48_32_1_next_commands_blocked','created_unix':time.time(),
...
# Exercise the exact v48.32 adaptive multi-group loss path before any expensive
# index construction or GPU training. This regression catches the tensor/scalar
# variable-shadowing failure that caused both v48.32 variants to exit at val ep0.
set +e
...
run_variant() {
  ...
  local factor_cache=""
  case "$variant" in
    balanced) factor_cache="${V48321_FACTOR_CACHE_BALANCED:-}" ;;
    precision) factor_cache="${V48321_FACTOR_CACHE_PRECISION:-}" ;;
  esac
  ...
  V4832_IDENTITY_TRAIN_ALL=1 V4832_COUPLE_ADMISSION_PRIOR=1 \
  V4832_ADAPTIVE_IDENTITY_MARGIN=1 V4832_ENABLE_FINAL_CALIBRATION=1 \
  V48321_FACTOR_CACHE_RUN="$factor_cache" \
  ...
  bash scripts/adapt_ocrap_v48_32_1_identity_utility_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
PY_STAGE
    ...
    python tools/extract_v48_32_1_failure_signature.py \
...
(root/f'ADAPTATION_FAILED_{variant}.json').write_text(json.dumps({
    'event':'v48_32_1_adaptation_failed','variant':variant,'stage':stage,
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_32_1.json" --version v48.32.1-RC30-INTEGRITY-HOTFIX || true
  exit 30
fi
...
# v48.32 fail-closed preflight: training and downstream inference must
...
for variant in balanced precision; do
  ...
  set +e
  python tools/check_v48_32_training_contract.py \
    ...
    --expect-adaptive-margin true --expect-final-enabled true \
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" \
  OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit bash scripts/calibrate_v48_32_1_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
