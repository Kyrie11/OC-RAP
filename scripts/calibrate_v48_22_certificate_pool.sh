...
# Finalise a v48.22 COVENANT-BRIDGE run using a scene-disjoint dedicated certificate pool.
...
: > "$OUTPUTDIR/logs/v48_22_certificate_status.log"
...
protocol={
 'version':'v48.22-COVENANT-BRIDGE',
...
doc={'event':'v48_22_gate_protocol_preregistered','created_unix':time.time(),
...
calibrate_variant() {
  ...
  for i in 0 1 2 3; do
    CUDA_VISIBLE_DEVICES="$gpu" python -u -m ocrap.cli calibrate \
      ...
      2>&1 | tee "$run/logs/v48_22_calibrate_${buckets[$i]}.log"
  done
  python tools/write_gamma_by_bucket.py \
    ...
    2>&1 | tee "$run/logs/v48_22_gamma.log"
...
doc={'event':'v48_22_certificate_pool_calibration_complete','created_unix':time.time(),
...
PY
  ...
  mv "$tmp" "$final"
  rm -rf "$final.old"
  echo "$variant near=$sn contact=$sc" >> "$OUTPUTDIR/logs/v48_22_certificate_status.log"
...
status={'event':'v48_22_certificate_candidate_selection','created_unix':time.time(),
...
(root/'NEXT_COMMANDS.txt').write_text(
 ...
 f'''OUT={root} bash scripts/run_v48_22_stress_if_authorized.sh\n''')
print(json.dumps(status,ensure_ascii=False,indent=2))
print('chosen',chosen)
PY
