...
case "$ARM" in
  A) export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_54_dcp_drfc_bcde_ipbd_ablation_A}" ;;
  B) export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_54_dcp_drfc_bcde_ipbd_main}" ;;
...
[[ -s "$PROTOCOL_ROOT/V48_45_PROTOCOL_SEAL.json" ]] || {
  echo "v48.54 shared protocol seal missing: $PROTOCOL_ROOT/V48_45_PROTOCOL_SEAL.json" >&2
  exit 30
}
...
# v48.54 holds the validated selector, NCP, DRFC, smooth NAP and paper-consistent
...
# v48.54 IPBD is a strict single-axis test around the best-supported BC-FC +
# smooth-NAP q-hard reference.  The deployed/native hard DRS stays q-based in
# both arms.  The only causal factor is privileged selected-option physical
# boundary distillation during witness training.
#   A: IPBD=0  validated q-hard BC-FC + smooth NAP reference
#   B: IPBD=1  same deployment, plus teacher-q-selected m_star==0 distillation
# No physical teacher hard DRS, no physical student/native DRS, no new head,
# threshold, regime input/router, proposal expansion, resampling, or capacity.
...
export V4854_REQUIRE_IPBD_CONTRACT=1
...
case "$ARM" in
  A)
    export OCRAP_ALGORITHM_VERSION="v48.54-DCP-DRFC-BCDE-IPBD-ablation-A"
    export OCRAP_IMPLEMENTATION_VERSION="v48.54-A-qhard-reference"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.54-A-qhard-reference"
    ;;
  B)
    export OCRAP_ALGORITHM_VERSION="v48.54-DCP-DRFC-BCDE-IPBD"
    export OCRAP_IMPLEMENTATION_VERSION="v48.54-B-invariant-physical-boundary-distillation"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.54-B-invariant-physical-boundary-distillation"
    export V4854_INVARIANT_PHYSICAL_BOUNDARY_DISTILLATION=true
    ;;
  *) echo "unknown arm: $ARM" >&2; exit 2 ;;
esac

# Machine-readable factor contract is written before adaptation.  Notice that
# teacher/student/deployment hard-sign coordinates are deliberately identical
# across A/B; IPBD is training-only privileged supervision.
mkdir -p "$OUTPUTDIR"
python - "$OUTPUTDIR/V48_54_FACTOR_CONTRACT.json" "$ARM" "$V4854_INVARIANT_PHYSICAL_BOUNDARY_DISTILLATION" <<'PY_FACTOR'
import json,os,pathlib,sys,time
p=pathlib.Path(sys.argv[1]); arm=sys.argv[2]; ipbd=sys.argv[3].lower()=='true'
d={'event':'v48_54_factor_contract','version':'v48.54-DCP-DRFC-BCDE-IPBD','arm':arm,
   'reference_mechanism':'v48.53-A-BC-FC-smooth-NAP-qhard','invariant_physical_boundary_distillation':ipbd,
   'physical_boundary_distillation_coordinate':'teacher_q_selected_mstar_zero_to_predicted_margin',
   'physical_boundary_distillation_weight':(float(os.environ.get('V4847_FRONTIER_SIGN_WEIGHT','0.50')) if ipbd else 0.0),
   ...
   'teacher_sign_coordinate':'q_hard_proxy_drs_exact_pcd',
   'frontier_order_coordinate':'smooth_boundary_drs_smooth_pcd',
   'frontier_drs_gradient':'q_boundary_straight_through_sigmoid',
   ...
   'frontier_sign_temperature':float(os.environ.get('V4847_FRONTIER_SIGN_TEMPERATURE','0.08')),
