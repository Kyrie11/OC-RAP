...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_29_veto_rank_physics_dedicated_4829}"
...
failed={'event':'v48_29_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_29_veto_rank_physics_bridge_controller_complete','created_unix':time.time(),
...
(root/'V48_29_COMPLETE.json').write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
PY
}
...
run_variant() {
  ...
  EVIDENCE_ADMISSION_PRIOR_MODE=benefit_only EVIDENCE_ADMISSION_SCALE="${EVIDENCE_ADMISSION_SCALE:-2.0}" \
  ...
  bash scripts/adapt_ocrap_v48_29_veto_rank_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_29.json" --version v48.29-VETO-RANK-PHYSICS-BRIDGE || true
  exit 30
fi
...
# v48.29 fail-closed preflight: training and downstream inference must
...
for variant in balanced precision; do
  ...
  set +e
  python tools/check_v48_29_model_contract.py \
    --checkpoint "$ckpt" \
    ...
    --expect-admission-prior-mode benefit_only \
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" \
  OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit bash scripts/calibrate_v48_29_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
...
python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_29.json" --version v48.29-VETO-RANK-PHYSICS-BRIDGE || true
python tools/summarize_v48_29_gate_failure.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/GATE_FAILURE_DECOMPOSITION.json" || true
if [[ "$cert_rc" == 30 ]]; then
