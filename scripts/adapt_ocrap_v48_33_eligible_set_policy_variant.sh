...
ENABLE_SUPPORT_RELIABILITY="${V4833_ENABLE_SUPPORT_RELIABILITY:-1}"
IDENTITY_TRAIN_ALL="${V4833_IDENTITY_TRAIN_ALL:-1}"
COUPLE_ADMISSION_PRIOR="${V4833_COUPLE_ADMISSION_PRIOR:-1}"
ADAPTIVE_IDENTITY_MARGIN="${V4833_ADAPTIVE_IDENTITY_MARGIN:-0}"
ENABLE_FINAL_CALIBRATION="${V4833_ENABLE_FINAL_CALIBRATION:-0}"
FACTOR_CACHE_RUN="${V4833_FACTOR_CACHE_RUN:-}"
PROPOSAL_TOP_K="${PROPOSAL_TOP_K:-5}"
export PROPOSAL_TOP_K
...
doc={'event':'v48_33_variant_stage_failed','created_unix':time.time(),'variant':sys.argv[2],
...
if [[ -n "$FACTOR_CACHE_RUN" ]]; then
  ...
  python tools/materialize_v48_33_factor_cache.py \
  ...
  EVIDENCE_ADAPT_EPOCHS="${FACTOR_EPOCHS:-20}" EVIDENCE_ADAPT_PATIENCE="${FACTOR_PATIENCE:-6}" \
    bash scripts/adapt_ocrap_v48_33_eligible_set_policy_single_stage.sh
...
ORDINAL_EVIDENCE_ELIGIBLE_OPPORTUNITY_THRESHOLD="${POLICY_METRIC_OPPORTUNITY_THRESHOLD:-0.65}" \
ORDINAL_EVIDENCE_ELIGIBLE_HARM_THRESHOLD="${POLICY_METRIC_HARM_THRESHOLD:-0.30}" \
ORDINAL_EVIDENCE_SAFE_HARD_NEGATIVE_WEIGHT="${IDENTITY_SAFE_HARD_NEGATIVE_WEIGHT:-1.00}" \
...
ORDINAL_EVIDENCE_FRONTIER_PAIRWISE_WEIGHT="${IDENTITY_FRONTIER_PAIRWISE_WEIGHT:-0.25}" \
...
EVIDENCE_CALIBRATOR_ANCHOR_WEIGHT="${IDENTITY_CALIBRATOR_ANCHOR_WEIGHT:-0.25}" \
SELECTIVE_RISK_WEIGHT=0 SELECTIVE_COVERAGE_WEIGHT=0 \
OPPORTUNITY_ADMISSION_WEIGHT=0 HARM_ADMISSION_WEIGHT=0 \
GROUP_BATCH_STRATIFIED=false GROUP_BATCHING_REPLACEMENT=false \
POSITIVE_GROUP_BOOST="${IDENTITY_POSITIVE_GROUP_BOOST:-3.0}" POSITIVE_MACRO_BALANCE_POWER=0.0 \
EVIDENCE_ADAPT_LR="${IDENTITY_LR:-0.00006}" \
...
for p in (source,factor,identity,final,support,run/'STAGE_TRANSFER_INTEGRITY.json'):
    if not p.is_file(): raise SystemExit(f'missing v48.33 artifact: {p}')
transfer=json.load(open(run/'STAGE_TRANSFER_INTEGRITY.json'))
doc={
 'event':'v48_33_eligible_set_policy_complete','created_unix':time.time(),
 ...
 'continuous_unified_semantics':'top5 proposal plus support-weighted safety utility and soft eligibility',
 'independent_measured_hard_veto':True,
 'checkpoint_metric':'direct_contract_safe_rank_risk','exact_eligibility_metric':True,'selection_semantics':'rank_topk_then_filter_then_evidence_rerank',
