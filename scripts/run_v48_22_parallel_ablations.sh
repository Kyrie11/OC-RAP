...
ROOT="${ABLATION_ROOT:-runs/ocrap_v48_22_covenant_ablations}"
...
doc={'complete':False,'version':'v48.22-COVENANT-BRIDGE','stage':stage,
...
if [[ "$rebuild_index" != 1 && -f "$GROUP_INDEX" && -f "$GROUP_SUMMARY" ]]; then
  ...
  python tools/check_v48_19_target_support.py "${index_contract_args[@]}" --mode contract \
    --output "$ROOT/COVENANT_INDEX_CONTRACT.json" >"$ROOT/logs/check_teacher_index_contract.log" 2>&1
  contract_rc=$?
  set -e
...
contract_rc=$?
set -e
if [[ "$contract_rc" != 0 ]]; then fail_controller teacher_index_contract "$contract_rc" "$ROOT/COVENANT_INDEX_CONTRACT.json"; exit 30; fi
set +e
python tools/check_v48_19_target_support.py "${index_contract_args[@]}" --mode all \
  --output "$ROOT/COVENANT_TARGET_SUPPORT.json" >"$ROOT/logs/check_covenant_target_support.log" 2>&1
target_rc=$?
set -e
if [[ "$target_rc" != 0 ]]; then fail_controller target_support "$target_rc" "$ROOT/COVENANT_TARGET_SUPPORT.json"; exit 30; fi
...
run_task() {
  ...
  case "$group" in
    A_two_head_safe_probability)
      # Engineering-corrected two-head baseline: raw benefit + component harm,
      # with p(benefit)*(1-p(harm)) used consistently by MIL and deployment.
      admission=false
      admission_weight=0.0
      ;;
    B_triad_candidate_only)
      # Tests the third admission hypothesis without group-level objectives.
      setwise=0.0
      group_opportunity=0.0
      ;;
    C_triad_group_mil_aggregate)
      # Tests admission + group objectives with one aggregate risk tail.
  ...
  NUM_WORKERS="${NUM_WORKERS:-1}" PREFETCH_FACTOR="${PREFETCH_FACTOR:-2}" BATCH_SIZE="${BATCH_SIZE:-48}" \
  ...
  EVALUATE_INITIAL_CHECKPOINT=true BEST_METRIC=direct_covenant_selection_risk \
    bash scripts/adapt_ocrap_v48_22_covenant_variant.sh >"$out/logs/adapt.log" 2>&1
  local adapt_rc=$?
  set -e
  if [[ "$adapt_rc" != 0 ]]; then
  ...
  GPU0="$gpu" GPU1="$gpu" VARIANTS="$variant" CERTIFICATE_GRID_SIZE="${CERTIFICATE_GRID_SIZE:-31}" \
    bash scripts/calibrate_v48_22_certificate_pool.sh >"$out/logs/calibrate.log" 2>&1
  local cert_rc=$?
  set -e
  ...
  set +e
  python - "$out" "$group" "$variant" "$cert_rc" "$admission" "$component_heads" "$setwise" "$group_opportunity" "$component_tail" "$batch_balance" "$global_balance" <<'PY_TASK'
...
doc={'complete':True,'version':'v48.22-COVENANT-BRIDGE','group':group,'variant':variant,
...
groups=(A_two_head_safe_probability B_triad_candidate_only C_triad_group_mil_aggregate D_full_covenant)
...
python tools/summarize_v48_14_ablations.py --root "$ROOT" --output "$ROOT/ablation_summary_v48_22.json" --version v48.22-COVENANT-BRIDGE || true
set +e
python - "$ROOT" "$failures" <<'PY'
import json,pathlib,sys,time
root=pathlib.Path(sys.argv[1]); failures=int(sys.argv[2]); groups=['A_two_head_safe_probability','B_triad_candidate_only','C_triad_group_mil_aggregate','D_full_covenant']
...
doc={'complete':not missing and failures==0,'version':'v48.22-COVENANT-BRIDGE','max_concurrent_tasks':8,
