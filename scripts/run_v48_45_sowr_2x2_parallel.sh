...
arms=(A B C D)
run_arm() {
  local arm="$1" log="$BASE_OUT/ocrap_v48_45_sowr_${1}.launcher.log"
  BASE_OUT="$BASE_OUT" GPU0="$GPU0" GPU1="$GPU1" bash scripts/run_v48_45_sowr_ablation_arm.sh "$arm" >"$log" 2>&1
}
pids=(); names=(); rc=0
for arm in "${arms[@]}"; do
