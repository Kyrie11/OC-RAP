...
# v39 is a selector-only OC-RAC model evaluation with bounded residual-tail fallback. It keeps
...
export RUN=${RUN:-runs/ocrap_v39_stateful_margin_eval}
export BASE_RUN=${BASE_RUN:-runs/ocrap_v39_ocrac_balanced}
export CKPT=${CKPT:-$BASE_RUN/model_v39_ocrac/best.pt}
export CAL=${CAL:-$BASE_RUN/calibration/calibration_mix_v39.json}
...
make_sel() {
  ...
  case "$tag" in
    ...
    v39|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=${CONTACT_BRAKE_RESCUE:-false}; pcd_rescue=true; brake_tail=${CONTACT_BRAKE_TAIL:-true}; brake_tail_bypass_pcd=${CONTACT_TAIL_BYPASS_PCD_GAIN:-true}; is_v27=true ;;
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v39_${tag}.json" \
...
summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v39.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v39 =====")
for p in sorted(root.glob('eval_*_v39_*.json')):
...
for p in sorted(root.glob('*v39*.json')):
...
if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
  ...
  run_eval v39 near_contact 1 &
  wait
  run_eval scalar contact 0 &
  run_eval v39 contact 1 &
  wait
  run_audit scalar near_contact 0 32 384 &
  run_audit v39 near_contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  wait
  run_audit scalar contact 0 32 384 &
  run_audit v39 contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  wait
else
  run_eval v39 safe 0 &
  run_eval v39 near_contact 1 &
  wait
  run_eval v39 contact 0
  run_audit v39 near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  run_audit v39 contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  wait
fi

if [[ "${RUN_NEAR_CHALLENGE_ABLATION:-0}" == "1" ]]; then
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_eval v39 near_contact 0
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_audit v39 near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384}
fi

if [[ "${RUN_DEEP_CONTACT:-0}" == "1" ]]; then
  run_audit v39 contact 0 ${DEEP_CONTACT_TARGETS:-48} ${DEEP_CONTACT_LABELS:-640}
fi
