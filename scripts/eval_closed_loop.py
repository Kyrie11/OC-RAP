...
if __name__ == "__main__":
    ...
    ap.add_argument("--dataset", default="data/recap/test.zarr")
    ...
    res=evaluate_closed_loop_or_offline(arrays,args.method,calibration=calib,ablation=args.ablation)
    out=Path(args.output); out.mkdir(parents=True,exist_ok=True)
    (out/"metrics.json").write_text(json.dumps(res,indent=2))
    alignment={"uses_oracle_selector_for_ours": False,"uses_calibrated_crisp_for_ours": args.method.lower() in ("ours","ocrap","crisp"),"closed_loop_backend": res.get("closed_loop_backend"),"notes":"If MetaDrive/CARLA backend is not connected, this is an offline same-candidate closed-loop fallback, not an oracle selector."}
