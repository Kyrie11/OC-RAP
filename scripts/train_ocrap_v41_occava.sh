...
export RUN=${RUN:-runs/ocrap_v41_occava_${VARIANT}}
export MODEL_DIR=${MODEL_DIR:-$RUN/model_v41_occava}
...
# v41 OC-CAVA: OC-MERO certificates remain frozen. A candidate-specific value
...
case "$VARIANT" in
  balanced) POS_W=${POS_W:-4.0}; NEAR_W=${NEAR_W:-1.5}; CONTACT_W=${CONTACT_W:-1.0}; LR=${LR:-0.00012} ;;
  rescue_focused) POS_W=${POS_W:-6.0}; NEAR_W=${NEAR_W:-2.0}; CONTACT_W=${CONTACT_W:-1.25}; LR=${LR:-0.00010} ;;
...
CUDA_VISIBLE_DEVICES="$TRAIN_GPU" PYTHONUNBUFFERED=1 python -u -m ocrap.cli train \
  ...
  --set training.group_batch_hard_min_r_dep=0.10 --set training.group_batch_hard_boost=${GROUP_HARD_BOOST:-6.0} \
  ...
  --set training.direct_value_point_weight=${DIRECT_POINT_WEIGHT:-0.15} \
  --set training.direct_value_listwise_weight=${DIRECT_LISTWISE_WEIGHT:-0.35} \
  --set training.direct_value_centered_weight=${DIRECT_CENTERED_WEIGHT:-1.0} \
  --set training.direct_value_advantage_weight=${DIRECT_ADVANTAGE_WEIGHT:-1.2} \
  ...
  2>&1 | tee "$MODEL_DIR/train_v41_occava.log"
...
for bucket in mix safe near contact; do
  ...
  CUDA_VISIBLE_DEVICES="$TRAIN_GPU" python -u -m ocrap.cli calibrate --dataset "$data" --checkpoint "$MODEL_DIR/best.pt" \
    --output "$CAL_DIR/calibration_${bucket}_v41.json" --set calibration.required_min_for_delta="$min" \
    2>&1 | tee "$CAL_DIR/calibrate_${bucket}_v41.log"
done
python tools/write_gamma_by_bucket.py --safe "$CAL_DIR/calibration_safe_v41.json" --near "$CAL_DIR/calibration_near_v41.json" \
  --contact "$CAL_DIR/calibration_contact_v41.json" --delta 0.05 --output "$CAL_DIR/gamma_rec_by_bucket_v41.json"
for bucket in near contact; do
  ...
  CUDA_VISIBLE_DEVICES="$TRAIN_GPU" python -u tools/calibrate_direct_value_advantage_v41.py \
    --dataset "$data" --checkpoint "$MODEL_DIR/best.pt" --output "$CAL_DIR/direct_value_advantage_${bucket}_v41.json" \
    ...
    2>&1 | tee "$CAL_DIR/direct_value_advantage_${bucket}_v41.log"
done
