...
if [[ "$ARM" == D ]]; then
  export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_50_dcp_de_main}"
else
  export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_50_dcp_de_ablation_${ARM}}"
fi
...
# v48.50 DCP-DRFC-DE holds the validated selector, NCP, DRFC and paper-consistent
...
# v48.50 causal axes around v48.49-C:
#   X = Decision-Equivalent Frontier Calibration (DEFC): train the existing
#       margin witness on the exact deployed hard-DRS/dep/gap/PCD coordinates.
#   Y = Exact Native Advantage Preservation (E-NAP): transport exact hard-DRS
#       PCD at admission instead of the v48.49 smooth-boundary approximation.
# No new learned head, regime label, threshold router, top-k change, or resampling.
...
# v48.50 starts from the only positive v48.49 arm: NCP + DRFC + NAP.
# MC-NCP is intentionally disabled in every arm because v48.49 B/D showed
# severe safe-positive false-veto and ranking collapse.
...
# X: forward-exact/backward-smooth decision-equivalent frontier calibration.
...
case "$ARM" in
  A)
    export OCRAP_ALGORITHM_VERSION="v48.50-DCP-DRFC-DE-ablation-A"
    export OCRAP_IMPLEMENTATION_VERSION="v48.50-A-v48.49-C-reference"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.50-A-ncp-drfc-nap-reference"
    ;;
  B)
    export OCRAP_ALGORITHM_VERSION="v48.50-DCP-DRFC-DE-ablation-B"
    export OCRAP_IMPLEMENTATION_VERSION="v48.50-B-decision-equivalent-frontier"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.50-B-de-frontier"
    export V4850_DECISION_EQUIVALENT_FRONTIER=true
    ;;
  C)
    export OCRAP_ALGORITHM_VERSION="v48.50-DCP-DRFC-DE-ablation-C"
    export OCRAP_IMPLEMENTATION_VERSION="v48.50-C-exact-native-advantage"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.50-C-exact-native-advantage"
    export EVIDENCE_NATIVE_EXACT_ADVANTAGE_PRESERVATION=true
    ;;
  D)
    export OCRAP_ALGORITHM_VERSION="v48.50-DCP-DRFC-DE"
    export OCRAP_IMPLEMENTATION_VERSION="v48.50-D-decision-equivalent-dcp"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.50-D-decision-equivalent-dcp"
    export V4850_DECISION_EQUIVALENT_FRONTIER=true
    export EVIDENCE_NATIVE_EXACT_ADVANTAGE_PRESERVATION=true
    ;;
...
mkdir -p "$OUTPUTDIR"
python - "$OUTPUTDIR/V48_50_FACTOR_CONTRACT.json" "$ARM" "$V4850_DECISION_EQUIVALENT_FRONTIER" "$EVIDENCE_NATIVE_EXACT_ADVANTAGE_PRESERVATION" <<'PY_FACTOR'
import json,os,pathlib,sys,time
p=pathlib.Path(sys.argv[1]); arm=sys.argv[2]
de_frontier=sys.argv[3].lower()=='true'; exact_adv=sys.argv[4].lower()=='true'
d={'event':'v48_50_factor_contract','version':'v48.50-DCP-DRFC-DE','arm':arm,
   ...
   'decision_equivalent_frontier':de_frontier,'native_exact_advantage_preservation':exact_adv,
   'deployed_drs_coordinate':'hard_qbest_ge_zero_root_mass','frontier_drs_gradient':'straight_through_sigmoid',
   'frontier_predicted_root_weights':True,'frontier_gap_coordinate':'exp_neg_positive_gap',
   'native_advantage_coordinate':'hard_drs' if exact_adv else 'smooth_boundary_drs',
