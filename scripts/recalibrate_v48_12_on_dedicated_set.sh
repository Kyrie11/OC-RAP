...
recalibrate_variant(){
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    --dataset "$CAL_NEAR" --checkpoint "$ckpt" --bucket near --risk-source "${RISK_SOURCE:-ordinal_evidence}" --conditional-recovery-ranking --policy-first-no-fallback \
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    --dataset "$CAL_CONTACT" --checkpoint "$ckpt" --bucket contact --risk-source "${RISK_SOURCE:-ordinal_evidence}" --conditional-recovery-ranking --policy-first-no-fallback \
...
echo "v48.12 dedicated recalibration complete. No model was retrained and no test root was read."
