...
# Finalise a v48.14 run using a scene-disjoint dedicated certificate pool.
...
calibrate_variant() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    --min-fit-selected="${NEAR_MIN_FIT_SELECTED:-12}" --min-verify-selected="${NEAR_MIN_VERIFY_SELECTED:-8}" \
    --max-fit-harmful-group-ucb="${NEAR_MAX_FIT_HARM_UCB:-0.12}" \
    --max-verify-harmful-group-ucb="${NEAR_MAX_VERIFY_HARM_UCB:-0.14}" \
    --max-fit-harmful-selected-ucb="${NEAR_MAX_FIT_SELECTED_HARM_UCB:-0.22}" \
    --max-verify-harmful-selected-ucb="${NEAR_MAX_VERIFY_SELECTED_HARM_UCB:-0.25}" \
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    --min-fit-selected="${CONTACT_MIN_FIT_SELECTED:-16}" --min-verify-selected="${CONTACT_MIN_VERIFY_SELECTED:-10}" \
    --max-fit-harmful-group-ucb="${CONTACT_MAX_FIT_HARM_UCB:-0.14}" \
    --max-verify-harmful-group-ucb="${CONTACT_MAX_VERIFY_HARM_UCB:-0.16}" \
    --max-fit-harmful-selected-ucb="${CONTACT_MAX_FIT_SELECTED_HARM_UCB:-0.22}" \
    --max-verify-harmful-selected-ucb="${CONTACT_MAX_VERIFY_SELECTED_HARM_UCB:-0.25}" \
...
PY
  ...
  mv "$tmp" "$final"
  rm -rf "$final.old"
  echo "$variant near=$sn contact=$sc" >> "$OUTPUTDIR/logs/v48_14_certificate_status.log"
