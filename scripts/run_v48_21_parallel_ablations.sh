...
ROOT="${ABLATION_ROOT:-runs/ocrap_v48_21_concord_ablations}"
...
doc={'complete':False,'version':'v48.21-CONCORD-BRIDGE','stage':stage,
...
if [[ "$rebuild_index" != 1 && -f "$GROUP_INDEX" && -f "$GROUP_SUMMARY" ]]; then
  ...
  python tools/check_v48_19_target_support.py "${index_contract_args[@]}" --mode contract \
    --output "$ROOT/CONCORD_INDEX_CONTRACT.json" >"$ROOT/logs/check_teacher_index_contract.log" 2>&1
  contract_rc=$?
  set -e
...
contract_rc=$?
set -e
if [[ "$contract_rc" != 0 ]]; then fail_controller teacher_index_contract "$contract_rc" "$ROOT/CONCORD_INDEX_CONTRACT.json"; exit 30; fi
set +e
python tools/check_v48_19_target_support.py "${index_contract_args[@]}" --mode all \
  --output "$ROOT/CONCORD_TARGET_SUPPORT.json" >"$ROOT/logs/check_concord_target_support.log" 2>&1
target_rc=$?
set -e
if [[ "$target_rc" != 0 ]]; then fail_controller target_support "$target_rc" "$ROOT/CONCORD_TARGET_SUPPORT.json"; exit 30; fi
...
run_task() {
  ...
  local concord=true component_heads=true setwise=1.0 group_opportunity=1.25 component_tail=0.50 batch_balance=true global_balance=true class_balance=0.50
  case "$group" in
    A_safe_target_legacy_trunk)
      concord=false
      ;;
    B_concord_candidate_only)
      setwise=0.0
      group_opportunity=0.0
      ;;
    C_concord_group_mil_aggregate)
  ...
  EVIDENCE_CONCORD="$concord" EVIDENCE_COMPONENT_HEADS="$component_heads" SETWISE_W="$setwise" \
  ...
  BEST_METRIC=direct_concord_selection_risk \
    bash scripts/adapt_ocrap_v48_21_concord_variant.sh >"$out/logs/adapt.log" 2>&1
  local adapt_rc=$?
  set -e
  if [[ "$adapt_rc" != 0 ]]; then
  ...
  GPU0="$gpu" GPU1="$gpu" VARIANTS="$variant" CERTIFICATE_GRID_SIZE="${CERTIFICATE_GRID_SIZE:-31}" \
    bash scripts/calibrate_v48_21_certificate_pool.sh >"$out/logs/calibrate.log" 2>&1
  local cert_rc=$?
  set -e
  ...
  set +e
  python - "$out" "$group" "$variant" "$cert_rc" "$concord" "$component_heads" "$setwise" "$group_opportunity" "$component_tail" "$batch_balance" "$global_balance" <<'PY_TASK'
import hashlib,json,pathlib,sys,time
out=pathlib.Path(sys.argv[1]); group,variant=sys.argv[2:4]; rc=int(sys.argv[4])
concord=sys.argv[5].lower()=='true'; component_heads=sys.argv[6].lower()=='true'; setwise=float(sys.argv[7]); group_opportunity=float(sys.argv[8]); component_tail=float(sys.argv[9])
batch_balance=sys.argv[10].lower()=='true'; global_balance=sys.argv[11].lower()=='true'
...
doc={'complete':True,'version':'v48.21-CONCORD-BRIDGE','group':group,'variant':variant,
     'concord':concord,'component_heads':component_heads,'setwise_admission_weight':setwise,
     'group_opportunity_weight':group_opportunity,'component_tail_weight':component_tail,'batch_balance':batch_balance,'global_balance':global_balance,
...
groups=(A_safe_target_legacy_trunk B_concord_candidate_only C_concord_group_mil_aggregate D_full_concord)
run_wave() {
  local variant="$1"
...
set -e
python tools/summarize_v48_14_ablations.py --root "$ROOT" --output "$ROOT/ablation_summary_v48_21.json" --version v48.21-CONCORD-BRIDGE || true
set +e
python - "$ROOT" "$failures" <<'PY'
import json,pathlib,sys,time
root=pathlib.Path(sys.argv[1]); failures=int(sys.argv[2]); groups=['A_safe_target_legacy_trunk','B_concord_candidate_only','C_concord_group_mil_aggregate','D_full_concord']
...
doc={'complete':not missing and failures==0,'version':'v48.21-CONCORD-BRIDGE','max_concurrent_tasks':4,
