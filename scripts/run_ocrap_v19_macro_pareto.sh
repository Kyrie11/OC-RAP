...
export RUN=${RUN:-runs/ocrap_v19_macro_pareto}
...
make_sel() {
  ...
  COMMON_SEL=(
    ...
    # v19 macro-gated Pareto-LCB recovery certificate.
    ...
    # v19 only allows semantic recovery macros to use the recovery-feasibility pool
    ...
    --set selection.intervention_budget_rate_by_bucket.contact=0.12
    ...
    --set selection.intervention_budget_penalty_by_bucket.contact=8.0
    ...
    --set selection.intervention_penalty_by_bucket.contact=0.050
  )
}
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v19_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v19_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v19_${tag}.log"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v19.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v19.log"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v19.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v18 =====")
for p in sorted(root.glob('eval_*_v19_*.json')):
