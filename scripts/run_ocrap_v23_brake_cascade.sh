...
# v23 is a selector-level correction on top of the v22 model.  By default it
...
export RUN=${RUN:-runs/ocrap_v23_brake_cascade}
...
make_sel() {
  local tag="$1"  # scalar | v23
  local rel_near=false
  local rel_contact=false
  local rec_pool=false
  local brake_rescue=false
  case "$tag" in
    scalar) rel_near=false; rel_contact=false; rec_pool=false; brake_rescue=false ;;
    v23) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=true ;;
  ...
  COMMON_SEL=(
    ...
    # contact uses it only for yield/merge.  Brake is routed through the v23
    ...
    # v23: disable the old generic brake/stabilize protective channel in contact.
    ...
    # v23 macro-stratified brake-rescue certificate.  The candidate must be a
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v23_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v23_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v23_${tag}.log"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v23.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v23.log"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v23.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v23 =====")
for p in sorted(root.glob('eval_*_v23_*.json')):
...
for p in sorted(root.glob('*v23*.json')):
...
run_eval v23 near_contact 1 &
wait
run_eval scalar contact 0 &
run_eval v23 contact 1 &
wait

# Stress audits. Two GPUs run baseline and v23 concurrently.
run_audit scalar near_contact 0 32 384 &
run_audit v23 near_contact 1 32 384 &
wait
run_audit scalar contact 0 32 384 &
run_audit v23 contact 1 32 384 &
wait

if [[ "${RUN_DEEP_CONTACT:-0}" == "1" ]]; then
  run_audit v23 contact 0 48 640
fi
