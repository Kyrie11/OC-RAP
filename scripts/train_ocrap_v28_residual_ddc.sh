...
export INIT_CKPT=${INIT_CKPT:-$BASE_RUN/model_v22/best.pt}
export RUN=${RUN:-runs/ocrap_v28_residual_ddc}
export MODEL_DIR=${MODEL_DIR:-$RUN/model_v28_residual_ddc}
...
# v28 residual DDC: start from the calibrated v22/v27 selector model instead of
# scratch training, then apply a short contact-focused DDC fine-tune.  Freezing
# the representation/root decoder keeps the calibrated runtime scale intact;
# the trainable heads adjust deployability dominance for contact recovery macros.
CUDA_VISIBLE_DEVICES="$TRAIN_GPU" PYTHONUNBUFFERED=1 python -u -m ocrap.cli train \
  ...
  --set loss_weights.ddc=${DDC_WEIGHT:-6.0} \
  ...
  2>&1 | tee "$MODEL_DIR/train_v28_residual_ddc.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$VAL_MIX" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_mix_v28.json" \
  --set calibration.required_min_for_delta=100 \
  2>&1 | tee "$CAL_DIR/calibrate_mix_v28.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_safe" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_safe_v28.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_safe_v28.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_near_contact" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_near_v28.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_near_v28.log"

PYTHONUNBUFFERED=1 python -u -m ocrap.cli calibrate \
  --dataset "$OCRAP_ROOT/val_contact" \
  --checkpoint "$MODEL_DIR/best.pt" \
  --output "$CAL_DIR/calibration_contact_v28.json" \
  --set calibration.required_min_for_delta=50 \
  2>&1 | tee "$CAL_DIR/calibrate_contact_v28.log"

python tools/write_gamma_by_bucket.py \
  --safe "$CAL_DIR/calibration_safe_v28.json" \
  --near "$CAL_DIR/calibration_near_v28.json" \
  --contact "$CAL_DIR/calibration_contact_v28.json" \
  --delta 0.05 \
  --output "$CAL_DIR/gamma_rec_by_bucket_v28.json" \
  2>&1 | tee "$CAL_DIR/write_gamma_v28.log"

printf '\n[v28] model: %s\n[v28] calibration: %s\n[v28] gamma map: %s\n' "$MODEL_DIR/best.pt" "$CAL_DIR/calibration_mix_v28.json" "$CAL_DIR/gamma_rec_by_bucket_v28.json"
