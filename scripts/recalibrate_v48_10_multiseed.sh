...
SOURCE_RUN="${SOURCE_RUN:?Set SOURCE_RUN to a completed v48.10 training run}"
...
for raw_seed in "${seed_array[@]}"; do
  ...
  calibrate_variant() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket near --risk-source "${RISK_SOURCE:-ordinal_evidence}" --conformal-alpha="${CONFORMAL_ALPHA:-0.10}" --conformal-temperature="${CONFORMAL_TEMPERATURE:-0.02}" --conformal-scope="${CONFORMAL_SCOPE:-policy_top1}" --conditional-recovery-ranking \
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket contact --risk-source "${RISK_SOURCE:-ordinal_evidence}" --conformal-alpha="${CONFORMAL_ALPHA:-0.10}" --conformal-temperature="${CONFORMAL_TEMPERATURE:-0.02}" --conformal-scope="${CONFORMAL_SCOPE:-policy_top1}" --conditional-recovery-ranking \
...
python tools/summarize_v48_7_multiseed.py \
  --root "$MULTISEED_ROOT" \
  --output "$MULTISEED_ROOT/multiseed_summary_v48_10.json"
