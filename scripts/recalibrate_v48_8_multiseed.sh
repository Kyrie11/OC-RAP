...
SOURCE_RUN="${SOURCE_RUN:?Set SOURCE_RUN to a completed v48.8 training run}"
...
for raw_seed in "${seed_array[@]}"; do
  ...
  calibrate_variant() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket near --risk-source conformal_delta --conformal-alpha="${CONFORMAL_ALPHA:-0.10}" --conformal-temperature="${CONFORMAL_TEMPERATURE:-0.02}" \
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
      ...
      --bucket contact --risk-source conformal_delta --conformal-alpha="${CONFORMAL_ALPHA:-0.10}" --conformal-temperature="${CONFORMAL_TEMPERATURE:-0.02}" \
...
python tools/summarize_v48_7_multiseed.py \
  --root "$MULTISEED_ROOT" \
  --output "$MULTISEED_ROOT/multiseed_summary_v48_8.json"
printf '{"complete":true,"seeds":"%s","risk_source":"conformal_delta"}\n' "$SEEDS" \
