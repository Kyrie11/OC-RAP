...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_26_execution_physics_dedicated_4826}"
...
failed={'event':'v48_26_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_26_execution_physics_bridge_controller_complete','created_unix':time.time(),
...
(root/'V48_26_COMPLETE.json').write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
PY
}
...
run_variant() {
  ...
  PROPOSAL_TOP_K="$PROPOSAL_TOP_K" \
    bash scripts/adapt_ocrap_v48_26_execution_physics_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_26.json" --version v48.26-EXECUTION-PHYSICS-BRIDGE || true
  exit 30
fi
...
# v48.26 fail-closed preflight: training and downstream inference must
...
for variant in balanced precision; do
  ...
  python tools/check_v48_26_model_contract.py \
    ...
    --expect-frontier true --expect-admission-bounded false \
    --expect-component-prior-logit -2.0 \
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" \
  bash scripts/calibrate_v48_26_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
