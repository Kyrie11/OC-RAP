...
case "$ARM" in
  A) export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_53_dcp_drfc_bcde_cse_ablation_A}" ;;
  B) export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_53_dcp_drfc_bcde_cse_ablation_B}" ;;
  C) export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_53_dcp_drfc_bcde_cse_ablation_C}" ;;
  D) export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_53_dcp_drfc_bcde_cse_main}" ;;
...
[[ -s "$PROTOCOL_ROOT/V48_45_PROTOCOL_SEAL.json" ]] || {
  echo "v48.53 shared protocol seal missing: $PROTOCOL_ROOT/V48_45_PROTOCOL_SEAL.json" >&2
  exit 30
}
...
# v48.53 holds the validated selector, NCP, DRFC, smooth NAP and paper-consistent
# observation-class execution semantics fixed. The only causal axes are the
# teacher-side and student/deployment-side hard certificate composition; no
...
# v48.53 CSE is a strict 2x2 around the validated BC-FC + smooth-NAP base.
# Factor X = teacher physical sign (v48.52 PSA).
# Factor Y = student/deployment physical hard DRS.
#   A: X=0, Y=0  q-proxy teacher + q-hard student (v48.52 A reference)
#   B: X=1, Y=0  teacher-only PSA (v48.52 B; known negative)
#   C: X=0, Y=1  student-only physical certificate
#   D: X=1, Y=1  structurally equivalent physical teacher/student certificate
# Smooth q geometry remains unchanged in all four arms for local ordering.
# No new head, regime input/router, threshold, top-k, resampling, or capacity.
...
export V4853_REQUIRE_CSE_CONTRACT=1
...
case "$ARM" in
  A)
    export OCRAP_ALGORITHM_VERSION="v48.53-DCP-DRFC-BCDE-CSE-ablation-A"
    export OCRAP_IMPLEMENTATION_VERSION="v48.53-A-qproxy-qhard-reference"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.53-A-qproxy-qhard-reference"
    ;;
  B)
    export OCRAP_ALGORITHM_VERSION="v48.53-DCP-DRFC-BCDE-CSE-ablation-B"
    export OCRAP_IMPLEMENTATION_VERSION="v48.53-B-teacher-only-physical"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.53-B-teacher-only-physical"
    export V4852_PHYSICAL_TEACHER_SIGN_ALIGNMENT=true
    ;;
  C)
    export OCRAP_ALGORITHM_VERSION="v48.53-DCP-DRFC-BCDE-CSE-ablation-C"
    export OCRAP_IMPLEMENTATION_VERSION="v48.53-C-student-only-physical"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.53-C-student-only-physical"
    export V4853_PHYSICAL_STUDENT_SIGN_ALIGNMENT=true
    export EVIDENCE_PHYSICAL_STUDENT_DRS=true
    ;;
  D)
    export OCRAP_ALGORITHM_VERSION="v48.53-DCP-DRFC-BCDE-CSE"
    export OCRAP_IMPLEMENTATION_VERSION="v48.53-D-certificate-structural-equivalence"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.53-D-certificate-structural-equivalence"
    export V4852_PHYSICAL_TEACHER_SIGN_ALIGNMENT=true
    export V4853_PHYSICAL_STUDENT_SIGN_ALIGNMENT=true
    export EVIDENCE_PHYSICAL_STUDENT_DRS=true
    ;;
...
mkdir -p "$OUTPUTDIR"
python - "$OUTPUTDIR/V48_53_FACTOR_CONTRACT.json" "$ARM" "$V4852_PHYSICAL_TEACHER_SIGN_ALIGNMENT" "$V4853_PHYSICAL_STUDENT_SIGN_ALIGNMENT" <<'PY_FACTOR'
import json,os,pathlib,sys,time
p=pathlib.Path(sys.argv[1]); arm=sys.argv[2]
teacher=sys.argv[3].lower()=='true'; student=sys.argv[4].lower()=='true'
d={'event':'v48_53_factor_contract','version':'v48.53-DCP-DRFC-BCDE-CSE','arm':arm,
   'reference_mechanism':'v48.52-A-BC-FC-smooth-NAP','physical_teacher_sign_alignment':teacher,
   'physical_student_sign_alignment':student,'native_physical_student_drs':student,
   ...
   'decision_equivalent_frontier':False,'boundary_complete_frontier':True,
   'native_exact_advantage_preservation':False,'native_boundary_complete_advantage_preservation':False,
   'student_sign_coordinate':('q_selected_predicted_margin_physical_drs_exact_pcd' if student else 'hard_qbest_ge_zero_root_mass_exact_pcd'),
   'teacher_sign_coordinate':('q_selected_mstar_physical_drs_exact_pcd' if teacher else 'q_hard_proxy_drs_exact_pcd'),
   'frontier_order_coordinate':'smooth_boundary_drs_smooth_pcd',
   'frontier_drs_gradient':'selected_margin_straight_through_sigmoid' if student else 'q_boundary_straight_through_sigmoid',
   'frontier_predicted_root_weights':True,'frontier_gap_coordinate':'exp_neg_positive_gap',
   'native_advantage_coordinate':'smooth_boundary_drs','new_tuned_thresholds':False,
   'decision_weighted_observation':False,'root_logit_recalibration':False,
   'strategy_regime_conditioning':False,'same_downstream_dual_roct':True,
   'proposal_top_k':int(os.environ.get('PROPOSAL_TOP_K','5')),
   ...
   'standard_calibration_prediction_cache':True,'test_roots_read':False,'created_unix':time.time()}
