...
OUT="${OUT:?OUT is required; point it to the v48.32 dedicated run}"
...
status_path=out/'V48_32_COMPLETE.json'
reason=None; status={}
...
if reason is not None:
    doc={'event':'v48_32_dev_shadow_blocked','created_unix':time.time(),'reason':reason,
...
(root/'SHADOW_PREFLIGHT.json').write_text(json.dumps({
  'event':'v48_32_dev_shadow_preflight','created_unix':time.time(),
...
(root/'DIAGNOSTIC_ONLY_NO_PAPER.json').write_text(json.dumps({
 'event':'v48_32_dev_shadow_closed_loop','created_unix':time.time(),
...
doc={'event':'v48_32_dev_shadow_complete','created_unix':time.time(),
