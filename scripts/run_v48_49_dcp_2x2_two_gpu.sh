...
if [[ "$GPU0" == "$GPU1" && "${V4849_ALLOW_SHARED_GPU_IDS:-0}" != 1 ]]; then
  echo "v48.49 2x2 requires two distinct GPU ids (GPU0=$GPU0 GPU1=$GPU1); set V4849_ALLOW_SHARED_GPU_IDS=1 only for explicit debugging" >&2
  exit 2
fi
...
GPU_WAIT_TIMEOUT_MIN="${V4849_GPU_WAIT_TIMEOUT_MIN:-240}"
GPU_POLL_SECONDS="${V4849_GPU_POLL_SECONDS:-30}"
VARIANT_MODE="${V4849_VARIANT_MODE:-serial}"
case "$VARIANT_MODE" in
  ...
  *) echo "V4849_VARIANT_MODE must be serial|parallel" >&2; exit 2 ;;
esac
GPU_MIN_FREE_MB="${V4849_GPU_MIN_FREE_MB:-$default_min_free_mb}"
...
wait_for_gpu_lease() {
  ...
  if ! command -v nvidia-smi >/dev/null 2>&1; then
    echo "nvidia-smi missing; cannot establish v48.49 GPU memory lease" >&2
    return 30
  fi
  start="$(date +%s)"
  while true; do
...
import json,os,time
print(json.dumps({'event':'v48_49_gpu_lease_granted','unix':time.time(),'gpu':int(os.environ['GPU']),
...
PERF_LOG="$BASE_OUT/OC-RAP-v48.49-runtime-telemetry.jsonl"
: > "$PERF_LOG"
perf_pid=""
if command -v nvidia-smi >/dev/null 2>&1; then
  (
...
for line in sys.stdin:
 ...
 print(json.dumps({"unix":float(os.environ["TS"]),"loadavg":os.environ.get("LOAD",""),"mem_available_kb":int(os.environ["MEMKB"]) if os.environ.get("MEMKB") else None,"gpu":int(p[0]),"gpu_util_pct":float(p[1]),"gpu_mem_util_pct":float(p[2]),"gpu_mem_used_mb":float(p[3]),"gpu_mem_total_mb":float(p[4]),"power_w":float(p[5]) if p[5] not in {"N/A","[N/A]"} else None}))' >> "$PERF_LOG" || true
      sleep "${V4849_TELEMETRY_INTERVAL_S:-30}"
    done
  ) & perf_pid=$!
fi
...
arm_out(){
  case "$1" in
    D) printf '%s/ocrap_v48_49_dcp_main' "$BASE_OUT" ;;
    A|B|C) printf '%s/ocrap_v48_49_dcp_ablation_%s' "$BASE_OUT" "$1" ;;
    *) return 2 ;;
  esac
}
run_arm(){
  ...
  wait_for_gpu_lease "$gpu" "$arm" >"$out/logs/v48_49_gpu_lease.json" || return $?
  free="$(free_mb "$gpu")"; apps="$(compute_apps_json "$gpu")"
  GPU="$gpu" ARM="$arm" FREE="$free" APPS="$apps" SERIAL="$SERIAL_VARIANTS" MODE="$VARIANT_MODE" python - "$out/V48_49_GPU_SCHEDULER_DECISION.json" <<'PY'
import json,os,pathlib,sys,time
pathlib.Path(sys.argv[1]).write_text(json.dumps({'event':'v48_49_gpu_scheduler_decision','created_unix':time.time(),
...
PY
  date +%s.%N >"$out/logs/v48_49_launcher.start_unix"; log="$out/logs/v48_49_launcher.log"
  set +e
  BASE_OUT="$BASE_OUT" GPU0="$gpu" GPU1="$gpu" SERIAL_VARIANTS_ON_ONE_GPU="$SERIAL_VARIANTS" \
    bash scripts/run_v48_49_dcp_ablation_arm.sh "$arm" >"$log" 2>&1
  rc=$?
  set -e
  printf '%s\n' "$rc" >"$out/logs/v48_49_launcher.rc"; date +%s.%N >"$out/logs/v48_49_launcher.end_unix"
  python - "$out" "$arm" "$rc" <<'PY'
import json,pathlib,sys
out=pathlib.Path(sys.argv[1]); logs=out/'logs'; start=float((logs/'v48_49_launcher.start_unix').read_text()); end=float((logs/'v48_49_launcher.end_unix').read_text())
(out/'V48_49_RUNTIME.json').write_text(json.dumps({'arm':sys.argv[2],'launcher_start_unix':start,'launcher_end_unix':end,'wall_seconds':end-start,'exit_code':int(sys.argv[3])},indent=2)+'\n')
PY
  return "$rc"
}
wait_one(){
...
python tools/summarize_v48_46_runtime_telemetry.py \
  --input "$PERF_LOG" --output "$BASE_OUT/OC-RAP-v48.49-runtime-telemetry-summary.json" || true

if [[ "$engineering_failed" == 0 ]]; then
  python tools/compare_v48_49_dcp_2x2.py \
    --a "$(arm_out A)" --b "$(arm_out B)" --c "$(arm_out C)" --d "$(arm_out D)" \
    --output "$BASE_OUT/OC-RAP-v48.49-DCP-DRFC-2x2-audit.json"
else
  echo "one or more v48.49 arms had an engineering failure; all four waves were attempted but attribution is blocked" >&2
  exit 1
fi
