...
# v43 OC-RSC evaluation. OC-MERO/shared-option logic remains the admission certificate;
...
export RUN=${RUN:-runs/ocrap_v43_rsc_eval}
export BASE_RUN=${BASE_RUN:-runs/ocrap_v43_rsc_selective_balanced}
if [[ -z "${CKPT:-}" ]]; then
...
if [[ -z "${CAL:-}" ]]; then
  if [[ -f "$BASE_RUN/calibration/calibration_mix_v43.json" ]]; then CAL="$BASE_RUN/calibration/calibration_mix_v43.json";
  else CAL="$BASE_RUN/calibration/calibration_mix_v42.json"; fi
fi
if [[ -z "${GAMMA:-}" ]]; then
  if [[ -f "$BASE_RUN/calibration/gamma_rec_by_bucket_v43.json" ]]; then GAMMA="$BASE_RUN/calibration/gamma_rec_by_bucket_v43.json";
  else GAMMA="$BASE_RUN/calibration/gamma_rec_by_bucket_v42.json"; fi
fi
...
# v43 reads a deterministic score threshold that was fitted on one validation
# fold and verified on a disjoint fold. Refuse to execute an invalid certificate.
read_direct_threshold() {
  local path="$1"
  python - "$path" <<'PYQ'
import json, math, sys
p=sys.argv[1]; d=json.load(open(p))
t=float(d.get('direct_value_threshold', float('inf')))
if not d.get('valid_for_deployment', False) or not math.isfinite(t):
    raise SystemExit(f'invalid OC-RSC certificate: {p}')
print(t)
PYQ
}
if [[ -z "${NEAR_DIRECT_THRESHOLD:-}" ]]; then
  NEAR_DIRECT_THRESHOLD=$(read_direct_threshold "$BASE_RUN/calibration/direct_value_risk_near_v43.json")
fi
if [[ -z "${CONTACT_DIRECT_THRESHOLD:-}" ]]; then
  CONTACT_DIRECT_THRESHOLD=$(read_direct_threshold "$BASE_RUN/calibration/direct_value_risk_contact_v43.json")
fi
export NEAR_DIRECT_THRESHOLD CONTACT_DIRECT_THRESHOLD
echo "direct-value OC-RSC thresholds: near=$NEAR_DIRECT_THRESHOLD contact=$CONTACT_DIRECT_THRESHOLD"

make_sel() {
  local tag="$1"  # scalar | v43
  ...
  case "$tag" in
    ...
    v43|v40|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=${CONTACT_BRAKE_RESCUE:-false}; pcd_rescue=true; brake_tail=${CONTACT_BRAKE_TAIL:-false}; brake_tail_bypass_pcd=${CONTACT_TAIL_BYPASS_PCD_GAIN:-false}; direct_value=${RUN_DIRECT_VALUE:-true}; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    # v43 OC-RSC: a held-out-verified top-1 value certificate may augment
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v43_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v43_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee -a "$RUN/audit_${b}_selected_topk_v43_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v43_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v43.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    --set closed_loop.resume="$CL_RESUME" \
    --set closed_loop.resume_allow_legacy_partial=true \
    ...
    | tee -a "$RUN/closed_loop_safe_fast_v43.log"
  assert_json "$RUN/closed_loop_safe_fast_v43.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v43.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v43 =====")
for p in sorted(root.glob('eval_*_v43_*.json')):
...
for p in sorted(root.glob('*v43*.json')):
...
if [[ "$RUN_OFFLINE_EVAL" == "1" ]]; then
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    run_eval scalar safe 0 & run_eval v43 safe 1 & wait
    run_eval scalar near_contact 0 & run_eval v43 near_contact 1 & wait
    run_eval scalar contact 0 & run_eval v43 contact 1 & wait
  else
    run_eval v43 safe 0 & run_eval v43 near_contact 1 & wait
    run_eval v43 contact 0
  fi
fi
  ...
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" && "${PAIR_BUCKETS_ON_TWO_GPUS:-1}" == "1" ]]; then
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      (run_audit scalar near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v43 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_NEAR=$!
    fi
    if [[ "$RUN_CONTACT_AUDIT" == "1" ]]; then
      (run_audit scalar contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v43 contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_CONTACT=$!
    fi
    [[ -z "${P_NEAR:-}" ]] || wait "$P_NEAR"
    [[ -z "${P_CONTACT:-}" ]] || wait "$P_CONTACT"
  elif [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    [[ "$RUN_NEAR_AUDIT" != "1" ]] || { run_audit scalar near_contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v43 near_contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
    [[ "$RUN_CONTACT_AUDIT" != "1" ]] || { run_audit scalar contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v43 contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
  else
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      run_audit v43 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} &
      P_NEAR=$!
