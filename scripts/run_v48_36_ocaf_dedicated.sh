...
for variant in balanced precision; do
  ...
  python tools/check_v48_36_ocaf_model_contract.py \
    ...
    --expect-native-margin-complete-preservation "${EVIDENCE_NATIVE_MARGIN_COMPLETE_PRESERVATION:-false}" --expect-native-advantage-preservation "${EVIDENCE_NATIVE_ADVANTAGE_PRESERVATION:-false}" --expect-native-gap-tolerance "${EVIDENCE_NATIVE_GAP_TOLERANCE:-0.05}" --expect-native-positive-gain "${EVIDENCE_NATIVE_POSITIVE_GAIN:-${FACTOR_RECOVERY_ADVANTAGE_POSITIVE_GAIN:-0.015}}" \
