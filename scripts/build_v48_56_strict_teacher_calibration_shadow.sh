...
# v48.56 strict-teacher SHADOW calibration construction.
# Source: standard WOMD validation only.  This is a correctness experiment, not a
# replacement for the canonical dataset until the semantic audit is reviewed.
# The experiment changes only teacher-margin source semantics: post-min heuristic
# floors/ceilings are disabled via strict_min_slack.  The explicit branch-intent
# component is kept in this first shadow so artifact support/distribution is not
# simultaneously changed.  Full-margin artifact overrides remain disabled.
...
OUTPUT_ROOT="${OUTPUT_ROOT:-/data0/senzeyu2/dataset/OCRAP_v48_56_strict_teacher}"
...
COMMON=(
  ...
  --set teacher_margin_semantics.mode=strict_min_slack
...
build_near(){ local worker="$1" out="$2" gpu="$3";
  CUDA_VISIBLE_DEVICES="$gpu" XLA_PYTHON_CLIENT_PREALLOCATE=false run_ocrap build-dataset "${resume_args[@]}" "${COMMON[@]}" \
    ...
    --set dataset_quality.artifact_pass_use_margin_override=false --set dataset_quality.artifact_pass_skip_augmented_waymax=true \
