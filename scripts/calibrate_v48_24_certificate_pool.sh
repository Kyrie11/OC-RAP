...
# Finalise a v48.24 SUPPORT-BRIDGE run using a scene-disjoint dedicated certificate pool.
...
: > "$OUTPUTDIR/logs/v48_24_certificate_status.log"
...
doc={'event':'v48_24_gate_protocol_preregistered','created_unix':time.time(),
...
calibrate_variant() {
  ...
  for i in 0 1 2 3; do
    CUDA_VISIBLE_DEVICES="$gpu" python -u -m ocrap.cli calibrate \
      ...
      2>&1 | tee "$run/logs/v48_24_calibrate_${buckets[$i]}.log"
  done
  python tools/write_gamma_by_bucket.py \
    ...
    2>&1 | tee "$run/logs/v48_24_gamma.log"

  local common=(
    --checkpoint "$ckpt" --method-version=v48_24_support_bridge_safe_utility_policy_risk_certificate --risk-source="${RISK_SOURCE:-ordinal_evidence}" --allowed-splits=certificate_pool
    --conditional-recovery-ranking --proposal-top-k "${PROPOSAL_TOP_K:-8}" --evidence-rerank-top-k
...
doc={'event':'v48_24_certificate_pool_calibration_complete','created_unix':time.time(),
...
PY
  ...
  mv "$tmp" "$final"
  rm -rf "$final.old"
  echo "$variant near=$sn contact=$sc" >> "$OUTPUTDIR/logs/v48_24_certificate_status.log"
...
status={'event':'v48_24_certificate_candidate_selection','created_unix':time.time(),
...
(root/'NEXT_COMMANDS.txt').write_text(
 ...
 f'''OUT={root} bash scripts/run_v48_24_stress_if_authorized.sh\n''')
print(json.dumps(status,ensure_ascii=False,indent=2))
print('chosen',chosen)
PY
