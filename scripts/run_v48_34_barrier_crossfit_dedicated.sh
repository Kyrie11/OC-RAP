...
OUTPUTDIR="${OUTPUTDIR:-runs/ocrap_v48_34_barrier_crossfit_dedicated_4834}"
...
rm -f "$OUTPUTDIR"/ADAPTATION_FAILED_*.json "$OUTPUTDIR"/FAILURE_SIGNATURE_*.json "$OUTPUTDIR/PIPELINE_FAILED.json" \
      "$OUTPUTDIR/V48_34_COMPLETE.json" "$OUTPUTDIR/NEXT_COMMANDS.txt" \
...
failed={'event':'v48_34_pipeline_failed','created_unix':time.time(),'stage':stage,
...
doc={'event':'v48_34_barrier_crossfit_controller_complete','created_unix':time.time(),
...
(root/'V48_34_COMPLETE.json').write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
blocked={'event':'v48_34_next_commands_blocked','created_unix':time.time(),
...
run_variant() {
  ...
  local factor_cache=""
  case "$variant" in
    balanced) factor_cache="${V4834_FACTOR_CACHE_BALANCED:-}" ;;
    precision) factor_cache="${V4834_FACTOR_CACHE_PRECISION:-}" ;;
  esac
  ...
  V4834_IDENTITY_TRAIN_ALL=1 V4834_COUPLE_ADMISSION_PRIOR=1 \
  V4834_ADAPTIVE_IDENTITY_MARGIN=0 V4834_ENABLE_FINAL_CALIBRATION=0 \
  V4834_FACTOR_CACHE_RUN="$factor_cache" \
  ...
  EVIDENCE_ADMISSION_PRIOR_MODE=barrier_gated_slack EVIDENCE_ADMISSION_SCALE="${EVIDENCE_ADMISSION_SCALE:-2.0}" \
  EVIDENCE_SLACK_TEMPERATURE="${EVIDENCE_SLACK_TEMPERATURE:-0.025}" EVIDENCE_SLACK_PENALTY="${EVIDENCE_SLACK_PENALTY:-1.5}" \
  ...
  bash scripts/adapt_ocrap_v48_34_barrier_crossfit_variant.sh >"$OUTPUTDIR/logs/adapt_${variant}.log" 2>&1
  local rc=$?
  set -e
  if [[ "$rc" != 0 ]]; then
...
(root/f'ADAPTATION_FAILED_{variant}.json').write_text(json.dumps({
    'event':'v48_34_adaptation_failed','variant':variant,'stage':stage,
...
if [[ "$s0" != 0 && "$s1" != 0 ]]; then
  ...
  python tools/check_v48_16_learning_gates.py --run "$OUTPUTDIR" --output "$OUTPUTDIR/learning_gates_v48_34.json" --version v48.34-BARRIER-CROSSFIT || true
  exit 30
fi
...
for variant in balanced precision; do
  ...
  python tools/check_v48_34_model_contract.py \
    ...
    --expect-admission-prior-mode barrier_gated_slack --expect-slack-temperature "${EVIDENCE_SLACK_TEMPERATURE:-0.025}" --expect-slack-penalty "${EVIDENCE_SLACK_PENALTY:-1.5}" \
...
GPU0="$GPU0" GPU1="$GPU1" VARIANTS="$variants" PROPOSAL_TOP_K="$PROPOSAL_TOP_K" \
  OPPORTUNITY_LABEL_MODE=raw_benefit GATE_POSITIVE_MODE=safe_benefit bash scripts/calibrate_v48_34_certificate_pool.sh >"$OUTPUTDIR/logs/certificate_controller.log" 2>&1
raw_cert_rc=$?
set -e
case "$raw_cert_rc" in
