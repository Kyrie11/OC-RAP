...
ROOT="${ABLATION_ROOT:-runs/ocrap_v48_24_support_ablations_4824}"
...
doc={'complete':False,'version':'v48.24-SUPPORT-BRIDGE','stage':stage,
...
run_task() {
  ...
  local proposal_k=8 safe_reg=0.0 safe_list=0.0 frontier_pairwise=0.0
  case "$group" in
    ...
    B_top8_support_only)
      ...
      ;;
    C_top8_safe_utility)
      proposal_k=8; safe_reg=1.0; safe_list=0.75
      ;;
    D_full_support_bridge)
      proposal_k=8; safe_reg=1.0; safe_list=0.75; frontier_pairwise=0.25
      ;;
  ...
  ORDINAL_EVIDENCE_BENEFIT_LISTWISE_WEIGHT=0.0 \
  ...
  EVALUATE_INITIAL_CHECKPOINT=true BEST_METRIC=direct_frontier_selection_risk \
    bash scripts/adapt_ocrap_v48_24_support_variant.sh >"$out/logs/adapt.log" 2>&1
  local adapt_rc=$?
  set -e
  if [[ "$adapt_rc" != 0 ]]; then
  ...
  GPU0="$gpu" GPU1="$gpu" VARIANTS="$variant" CERTIFICATE_GRID_SIZE="${CERTIFICATE_GRID_SIZE:-31}" \
    bash scripts/calibrate_v48_24_certificate_pool.sh >"$out/logs/calibrate.log" 2>&1
  local cert_rc=$?
  set -e
  ...
  python - "$out" "$group" "$variant" "$cert_rc" "$proposal_k" "$safe_reg" "$safe_list" "$frontier_pairwise" <<'PY_TASK'
import hashlib,json,pathlib,sys,time
out=pathlib.Path(sys.argv[1]); group=sys.argv[2]; variant=sys.argv[3]; rc=int(sys.argv[4])
proposal_k=int(sys.argv[5]); safe_reg=float(sys.argv[6]); safe_list=float(sys.argv[7]); frontier=float(sys.argv[8])
...
doc={'complete':True,'version':'v48.24-SUPPORT-BRIDGE','group':group,'variant':variant,
...
groups=(A_top3_safe_label_baseline B_top8_support_only C_top8_safe_utility D_full_support_bridge)
...
python tools/summarize_v48_14_ablations.py --root "$ROOT" --output "$ROOT/ablation_summary_v48_24.json" --version v48.24-SUPPORT-BRIDGE || true
set +e
...
doc={'complete':not missing and failures==0,'version':'v48.24-SUPPORT-BRIDGE','max_concurrent_tasks':2,
