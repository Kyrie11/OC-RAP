...
if [[ -z "${CKPT:-}" ]]; then
  CKPT="$BASE_RUN/model_v45_rave/best.pt"
fi
if [[ -z "${CAL:-}" ]]; then
  CAL="$BASE_RUN/calibration/calibration_mix_v45.json"
fi
if [[ -z "${GAMMA:-}" ]]; then
  GAMMA="$BASE_RUN/calibration/gamma_rec_by_bucket_v45.json"
fi
...
# v45 reads both an opportunity threshold and a observation-conditioned score
...
if not d.get('valid_for_deployment', False) or not math.isfinite(s) or not math.isfinite(o):
    raise SystemExit(f'invalid OC-RAVE certificate: {p}')
print(s, o)
PYQ
}
if [[ -z "${NEAR_DIRECT_THRESHOLD:-}" || -z "${NEAR_DIRECT_OPPORTUNITY_THRESHOLD:-}" ]]; then
  read -r NEAR_DIRECT_THRESHOLD NEAR_DIRECT_OPPORTUNITY_THRESHOLD < <(read_direct_rule "$BASE_RUN/calibration/direct_value_risk_near_v45.json")
fi
if [[ -z "${CONTACT_DIRECT_THRESHOLD:-}" || -z "${CONTACT_DIRECT_OPPORTUNITY_THRESHOLD:-}" ]]; then
  read -r CONTACT_DIRECT_THRESHOLD CONTACT_DIRECT_OPPORTUNITY_THRESHOLD < <(read_direct_rule "$BASE_RUN/calibration/direct_value_risk_contact_v45.json")
fi
...
echo "direct-value OC-RAVE rules: near(score=$NEAR_DIRECT_THRESHOLD,opp=$NEAR_DIRECT_OPPORTUNITY_THRESHOLD) contact(score=$CONTACT_DIRECT_THRESHOLD,opp=$CONTACT_DIRECT_OPPORTUNITY_THRESHOLD)"

make_sel() {
  local tag="$1"  # scalar | v45
  ...
  case "$tag" in
    ...
    v45|v40|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=${CONTACT_BRAKE_RESCUE:-false}; pcd_rescue=true; brake_tail=${CONTACT_BRAKE_TAIL:-false}; brake_tail_bypass_pcd=${CONTACT_TAIL_BYPASS_PCD_GAIN:-false}; direct_value=${RUN_DIRECT_VALUE:-true}; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    # v45 OC-RAVE: a held-out-verified top-1 value certificate may augment
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v45_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v45_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee -a "$RUN/audit_${b}_selected_topk_v45_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v45_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v45.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    --set closed_loop.resume="$CL_RESUME" \
    --set closed_loop.resume_allow_legacy_partial=true \
    ...
    | tee -a "$RUN/closed_loop_safe_fast_v45.log"
  assert_json "$RUN/closed_loop_safe_fast_v45.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v45.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v45 =====")
for p in sorted(root.glob('eval_*_v45_*.json')):
...
for p in sorted(root.glob('*v45*.json')):
...
if [[ "$RUN_OFFLINE_EVAL" == "1" ]]; then
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    run_eval scalar safe 0 & run_eval v45 safe 1 & wait
    run_eval scalar near_contact 0 & run_eval v45 near_contact 1 & wait
    run_eval scalar contact 0 & run_eval v45 contact 1 & wait
  else
    if [[ "${OFFLINE_SINGLE_GPU:-0}" == "1" ]]; then
      g=${GPU_SAFE:-0}
      run_eval v45 safe "$g"
      run_eval v45 near_contact "$g"
      run_eval v45 contact "$g"
    else
      run_eval v45 safe ${GPU_SAFE:-0} & run_eval v45 near_contact ${GPU_NEAR:-1} & wait
      run_eval v45 contact ${GPU_CONTACT:-0}
    fi
  fi
fi
  ...
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" && "${PAIR_BUCKETS_ON_TWO_GPUS:-1}" == "1" ]]; then
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      (run_audit scalar near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v45 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_NEAR=$!
    fi
    if [[ "$RUN_CONTACT_AUDIT" == "1" ]]; then
      (run_audit scalar contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v45 contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_CONTACT=$!
    fi
    [[ -z "${P_NEAR:-}" ]] || wait "$P_NEAR"
    [[ -z "${P_CONTACT:-}" ]] || wait "$P_CONTACT"
  elif [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    [[ "$RUN_NEAR_AUDIT" != "1" ]] || { run_audit scalar near_contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v45 near_contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
    [[ "$RUN_CONTACT_AUDIT" != "1" ]] || { run_audit scalar contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v45 contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
  else
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      run_audit v45 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} &
      P_NEAR=$!
