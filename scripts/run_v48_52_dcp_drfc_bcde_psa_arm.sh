...
if [[ "$ARM" == B ]]; then
  export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_52_dcp_drfc_bcde_psa_main}"
else
  export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_52_dcp_drfc_bcde_psa_ablation_A}"
fi
...
[[ -s "$PROTOCOL_ROOT/V48_45_PROTOCOL_SEAL.json" ]] || {
  echo "v48.52 shared protocol seal missing: $PROTOCOL_ROOT/V48_45_PROTOCOL_SEAL.json" >&2
  exit 30
}
...
# v48.52 PSA holds the validated selector, NCP, DRFC, smooth NAP and
# paper-consistent observation-class execution semantics fixed.  The only new
# causal axis is the teacher-side physical sign definition inside BC-FC; no
# Safe/Near/Contact identifier, router, threshold, loss budget, top-k, or policy
# branch is added.
...
# v48.52 single causal axis around the validated v48.51-B mechanism:
#   A = BC-FC with the v48.51 q-hard teacher-sign proxy.
#   B/Main = the same BC-FC, but teacher q only selects the legal recovery
#            option and physical m_star>=0 defines teacher root success (PSA).
# Both arms retain the validated smooth NAP. BC-NAP and v48.50 exact-only NAP
# are disabled because v48.51 showed negative interaction / no clean benefit.
...
export V4852_REQUIRE_PSA_CONTRACT=1
...
case "$ARM" in
  A)
    export OCRAP_ALGORITHM_VERSION="v48.52-DCP-DRFC-BCDE-PSA-ablation-A"
    export OCRAP_IMPLEMENTATION_VERSION="v48.52-A-v48.51-B-reference"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.52-A-bcfc-qproxy-reference"
    ;;
  B)
    export OCRAP_ALGORITHM_VERSION="v48.52-DCP-DRFC-BCDE-PSA"
    export OCRAP_IMPLEMENTATION_VERSION="v48.52-B-physical-sign-alignment"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.52-B-bcfc-physical-sign-alignment"
...
mkdir -p "$OUTPUTDIR"
python - "$OUTPUTDIR/V48_52_FACTOR_CONTRACT.json" "$ARM" "$V4852_PHYSICAL_TEACHER_SIGN_ALIGNMENT" <<'PY_FACTOR'
import json,os,pathlib,sys,time
p=pathlib.Path(sys.argv[1]); arm=sys.argv[2]; psa=sys.argv[3].lower()=='true'
d={'event':'v48_52_factor_contract','version':'v48.52-DCP-DRFC-BCDE-PSA','arm':arm,
   'reference_mechanism':'v48.51-B-BC-FC','physical_teacher_sign_alignment':psa,
   ...
   'teacher_sign_coordinate':('q_selected_mstar_physical_drs_exact_pcd' if psa else 'q_hard_proxy_drs_exact_pcd'),
   ...
   'frontier_gap_coordinate':'exp_neg_positive_gap','native_advantage_coordinate':'smooth_boundary_drs',
   'new_tuned_thresholds':False,'decision_weighted_observation':False,'root_logit_recalibration':False,
   ...
   'standard_calibration_prediction_cache':True,
