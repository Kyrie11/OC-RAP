...
SOURCE_RUN="${SOURCE_RUN:?Set SOURCE_RUN to a completed v48.11 training run}"
...
python tools/summarize_v48_7_multiseed.py \
  --root "$MULTISEED_ROOT" \
  --output "$MULTISEED_ROOT/multiseed_summary_v48_11.json"
printf '{"complete":true,"seeds":"%s","risk_source":"%s"}\n' "$SEEDS" "${RISK_SOURCE:-ordinal_evidence}" \
