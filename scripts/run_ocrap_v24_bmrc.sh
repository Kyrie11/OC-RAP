...
# v24 is a selector-level BMRC correction on top of the v22 model.  By default it
...
export RUN=${RUN:-runs/ocrap_v24_bmrc}
...
make_sel() {
  ...
  case "$tag" in
    ...
    v24) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=true; pcd_rescue=true ;;
  ...
  COMMON_SEL=(
    ...
    --set selection.intervention_budget_rate_by_bucket.contact=0.06
    ...
    --set selection.intervention_budget_penalty_by_bucket.contact=12.0
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v24_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v24_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee "$RUN/audit_${b}_selected_topk_v24_${tag}.log"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v24.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    ...
    | tee "$RUN/closed_loop_safe_fast_v24.log"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v24.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v24 =====")
for p in sorted(root.glob('eval_*_v24_*.json')):
...
for p in sorted(root.glob('*v24*.json')):
