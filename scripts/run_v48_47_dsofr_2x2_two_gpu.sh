...
PERF_LOG="$BASE_OUT/OC-RAP-v48.47-runtime-telemetry.jsonl"
: > "$PERF_LOG"
perf_pid=""
...
for line in sys.stdin:
 ...
 print(json.dumps({"unix":ts,"loadavg":load,"mem_available_kb":int(mem) if mem else None,"gpu":int(p[0]),"gpu_util_pct":float(p[1]),"gpu_mem_util_pct":float(p[2]),"gpu_mem_used_mb":float(p[3]),"gpu_mem_total_mb":float(p[4]),"power_w":float(p[5]) if p[5] not in {"[N/A]","N/A"} else None}))' \
        >>"$PERF_LOG" || true
      sleep "${V4847_TELEMETRY_INTERVAL_S:-30}"
    done
  ) & perf_pid=$!
fi
...
V4847_VARIANT_MODE="${V4847_VARIANT_MODE:-parallel}"
case "$V4847_VARIANT_MODE" in
  parallel) serial_variants=0 ;;
  serial) serial_variants=1 ;;
  *) echo "V4847_VARIANT_MODE must be parallel|serial" >&2; exit 2 ;;
esac

arm_out() {
  case "$1" in
    D) printf '%s/ocrap_v48_47_dsofr_main' "$BASE_OUT" ;;
    A|B|C) printf '%s/ocrap_v48_47_dsofr_ablation_%s' "$BASE_OUT" "$1" ;;
    *) return 2 ;;
  esac
}
run_arm() {
  local arm="$1" gpu="$2" out log rc
  out="$(arm_out "$arm")"; rm -rf "$out"; mkdir -p "$out/logs"
  date +%s.%N >"$out/logs/v48_47_launcher.start_unix"
  log="$out/logs/v48_47_launcher.log"
  set +e
  BASE_OUT="$BASE_OUT" GPU0="$gpu" GPU1="$gpu" SERIAL_VARIANTS_ON_ONE_GPU="$serial_variants" \
    bash scripts/run_v48_47_dsofr_ablation_arm.sh "$arm" >"$log" 2>&1
  rc=$?
  set -e
  printf '%s\n' "$rc" >"$out/logs/v48_47_launcher.rc"
  date +%s.%N >"$out/logs/v48_47_launcher.end_unix"
  python - "$out" "$arm" "$rc" <<'PY_TIMING'
import json,pathlib,sys
out=pathlib.Path(sys.argv[1]); arm=sys.argv[2]; rc=int(sys.argv[3]); logs=out/'logs'
start=float((logs/'v48_47_launcher.start_unix').read_text()); end=float((logs/'v48_47_launcher.end_unix').read_text())
(out/'V48_47_RUNTIME.json').write_text(json.dumps({'arm':arm,'launcher_start_unix':start,'launcher_end_unix':end,'wall_seconds':end-start,'exit_code':rc},indent=2)+'\n')
PY_TIMING
  return "$rc"
}
wait_one() {
  local pid="$1" arm="$2" out rc recorded
  set +e; wait "$pid"; rc=$?; set -e
  out="$(arm_out "$arm")"
  recorded="$(cat "$out/logs/v48_47_launcher.rc" 2>/dev/null || true)"
  if [[ -n "$recorded" && "$recorded" != "$rc" ]]; then
    echo "arm $arm launcher rc mismatch: wait=$rc recorded=$recorded" >&2
    return 1
  fi
  case "$rc" in
...
# A/B first: they warm/reuse the persistent tensor cache. C/D then reuse it.
# D also reuses B's byte-identical decision-observation witness checkpoint after
# a fail-closed source/input/hyperparameter contract, avoiding one full stage.
...
if [[ "$engineering_failed" == 0 ]]; then
  python tools/compare_v48_47_dsofr_2x2.py \
    --a "$(arm_out A)" --b "$(arm_out B)" --c "$(arm_out C)" --d "$(arm_out D)" \
    --output "$BASE_OUT/OC-RAP-v48.47-DS-OFR-2x2-audit.json"
  python tools/summarize_v48_46_runtime_telemetry.py \
    --input "$PERF_LOG" --output "$BASE_OUT/OC-RAP-v48.47-runtime-telemetry-summary.json" || true
else
  exit 1
fi
