...
run_one_status() {
  ...
  started="$(v50_iso_now)"; write_phase "$regime" running 0 "$started" ""
...
failed=0
if ((${#GPUS[@]} >= 2)); then
  run_one_status safe "$SAFE_WOMD" "$SAFE_BUCKET" "$SAFE_LABEL_MODE" "$RENDER_SAFE" "${GPUS[0]}" "$SAFE_GAMMA_REC" "$SAFE_TARGET_KEYS_FILE" & p_safe=$!
  run_one_status near "$NEAR_WOMD" "$NEAR_BUCKET" "$NEAR_LABEL_MODE" "$RENDER_NEAR" "${GPUS[1]}" "$NEAR_GAMMA_REC" "$NEAR_TARGET_KEYS_FILE" & p_near=$!
  # Contact starts as soon as GPU 0 is free, even when Safe failed.
  wait "$p_safe" || failed=1
  run_one_status contact "$CONTACT_WOMD" "$CONTACT_BUCKET" "$CONTACT_LABEL_MODE" "$RENDER_CONTACT" "${GPUS[0]}" "$CONTACT_GAMMA_REC" "$CONTACT_TARGET_KEYS_FILE" & p_contact=$!
  wait "$p_near" || failed=1
  wait "$p_contact" || failed=1
else
  run_one_status safe "$SAFE_WOMD" "$SAFE_BUCKET" "$SAFE_LABEL_MODE" "$RENDER_SAFE" "${GPUS[0]}" "$SAFE_GAMMA_REC" "$SAFE_TARGET_KEYS_FILE" || failed=1
  run_one_status near "$NEAR_WOMD" "$NEAR_BUCKET" "$NEAR_LABEL_MODE" "$RENDER_NEAR" "${GPUS[0]}" "$NEAR_GAMMA_REC" "$NEAR_TARGET_KEYS_FILE" || failed=1
  run_one_status contact "$CONTACT_WOMD" "$CONTACT_BUCKET" "$CONTACT_LABEL_MODE" "$RENDER_CONTACT" "${GPUS[0]}" "$CONTACT_GAMMA_REC" "$CONTACT_TARGET_KEYS_FILE" || failed=1
fi
