...
if [[ "$ARM" == D ]]; then
  export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_49_dcp_main}"
else
  export OUTPUTDIR="${OUTPUTDIR:-$BASE_OUT/ocrap_v48_49_dcp_ablation_${ARM}}"
fi
...
# v48.49 DCP-DRFC holds the validated v48.48-D selector, NCP, DRFC and
# paper-consistent observation-class execution semantics fixed.  The strict 2x2
# below changes only two deterministic, regime-agnostic interface transports:
# margin-complete native risk evidence and native positive recovery advantage.
...
# v48.49 DCP-DRFC starts from the validated v48.48 D/Main mechanism in every
# arm: native-certificate preservation + DRFC.  The strict 2x2 changes only two
# regime-agnostic deterministic transports:
#   X = margin-complete native preservation (boundary-resolved DRS + gap quality)
#   Y = native recovery-advantage preservation (candidate-minus-nominal PCD value)
# No new learned head, regime label, threshold router, top-k change, or dataset
...
case "$ARM" in
  A)
    export OCRAP_ALGORITHM_VERSION="v48.49-DCP-DRFC-ablation-A"
    export OCRAP_IMPLEMENTATION_VERSION="v48.49-A-v48.48-D-reference"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.49-A-ncp-drfc-reference"
    ;;
  B)
    export OCRAP_ALGORITHM_VERSION="v48.49-DCP-DRFC-ablation-B"
    export OCRAP_IMPLEMENTATION_VERSION="v48.49-B-margin-complete-native-preservation"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.49-B-margin-complete"
    export EVIDENCE_NATIVE_MARGIN_COMPLETE_PRESERVATION=true
    ;;
  C)
    export OCRAP_ALGORITHM_VERSION="v48.49-DCP-DRFC-ablation-C"
    export OCRAP_IMPLEMENTATION_VERSION="v48.49-C-native-advantage-preservation"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.49-C-native-advantage"
    export EVIDENCE_NATIVE_ADVANTAGE_PRESERVATION=true
    ;;
  D)
    export OCRAP_ALGORITHM_VERSION="v48.49-DCP-DRFC"
    export OCRAP_IMPLEMENTATION_VERSION="v48.49-D-decision-complete-preservation"
    export V4838_FACTOR_ALGORITHM_FAMILY="v48.49-D-dcp-drfc"
    export EVIDENCE_NATIVE_MARGIN_COMPLETE_PRESERVATION=true
    export EVIDENCE_NATIVE_ADVANTAGE_PRESERVATION=true
    ;;
...
mkdir -p "$OUTPUTDIR"
python - "$OUTPUTDIR/V48_49_FACTOR_CONTRACT.json" "$ARM" "$EVIDENCE_NATIVE_MARGIN_COMPLETE_PRESERVATION" "$EVIDENCE_NATIVE_ADVANTAGE_PRESERVATION" <<'PY_FACTOR'
import json,os,pathlib,sys,time
p=pathlib.Path(sys.argv[1]); arm=sys.argv[2]
margin_complete=sys.argv[3].lower()=='true'; native_adv=sys.argv[4].lower()=='true'
d={'event':'v48_49_factor_contract','version':'v48.49-DCP-DRFC','arm':arm,
   ...
   'native_margin_complete_preservation':margin_complete,'native_advantage_preservation':native_adv,
   'boundary_coordinate':'shared_feasible_mass_sigmoid_qbest','native_gap_quality':'exp_neg_positive_gap',
