...
# v40 OC-UVRA evaluation. OC-MERO/shared-option logic remains the admission certificate;
...
export RUN=${RUN:-runs/ocrap_v40_ocuvra_head_only_eval}
export BASE_RUN=${BASE_RUN:-runs/ocrap_v40_ocuvra_head_only}
export CKPT=${CKPT:-$BASE_RUN/model_v40_ocuvra/best.pt}
export CAL=${CAL:-$BASE_RUN/calibration/calibration_mix_v40.json}
export GAMMA=${GAMMA:-$BASE_RUN/calibration/gamma_rec_by_bucket_v40.json}
mkdir -p "$RUN"
...
read_direct_z() {
  local path="$1"
  local fallback="$2"
  python - "$path" "$fallback" <<'PYZ'
import json, math, sys
p, fallback = sys.argv[1], float(sys.argv[2])
try:
    z = float(json.load(open(p))["direct_value_lcb_z"])
    print(z if math.isfinite(z) else fallback)
except Exception:
    print(fallback)
PYZ
}
export NEAR_DIRECT_LCB_Z=${NEAR_DIRECT_LCB_Z:-$(read_direct_z "$BASE_RUN/calibration/direct_value_advantage_near_v40.json" 1.25)}
export CONTACT_DIRECT_LCB_Z=${CONTACT_DIRECT_LCB_Z:-$(read_direct_z "$BASE_RUN/calibration/direct_value_advantage_contact_v40.json" 1.0)}
echo "direct-value LCB z: near=$NEAR_DIRECT_LCB_Z contact=$CONTACT_DIRECT_LCB_Z"

make_sel() {
  local tag="$1"  # scalar | v40
  ...
  case "$tag" in
    ...
    v40|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=${CONTACT_BRAKE_RESCUE:-false}; pcd_rescue=true; brake_tail=${CONTACT_BRAKE_TAIL:-true}; brake_tail_bypass_pcd=${CONTACT_TAIL_BYPASS_PCD_GAIN:-false}; direct_value=${RUN_DIRECT_VALUE:-true}; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    # v40 OC-UVRA: direct value is only a preference certificate. A candidate
    ...
    --set selection.direct_value_lcb_z_by_bucket.near_contact=${NEAR_DIRECT_LCB_Z:-1.25}
    --set selection.direct_value_lcb_z_by_bucket.test_near_contact=${NEAR_DIRECT_LCB_Z:-1.25}
    --set selection.direct_value_lcb_z_by_bucket.contact=${CONTACT_DIRECT_LCB_Z:-1.0}
    --set selection.direct_value_lcb_z_by_bucket.test_contact=${CONTACT_DIRECT_LCB_Z:-1.0}
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v40_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v40_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee -a "$RUN/audit_${b}_selected_topk_v40_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v40_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v40.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    --set closed_loop.resume="$CL_RESUME" \
    --set closed_loop.resume_allow_legacy_partial=true \
    ...
    | tee -a "$RUN/closed_loop_safe_fast_v40.log"
  assert_json "$RUN/closed_loop_safe_fast_v40.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v40.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v40 =====")
for p in sorted(root.glob('eval_*_v40_*.json')):
...
for p in sorted(root.glob('*v40*.json')):
...
# Use both GPUs. The first candidate normally runs scalar controls as paired
# references. Later candidates can skip duplicate controls and evaluate near/contact
# simultaneously with RUN_SCALAR_BASELINES=0.
if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
  run_eval scalar safe 0 &
  run_eval v40 safe 1 &
  wait
  run_eval scalar near_contact 0 &
  run_eval v40 near_contact 1 &
  wait
  run_eval scalar contact 0 &
  run_eval v40 contact 1 &
  wait
  run_audit scalar near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  run_audit v40 near_contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  wait
  run_audit scalar contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  run_audit v40 contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  wait
else
  run_eval v40 safe 0 &
  run_eval v40 near_contact 1 &
  wait
  run_eval v40 contact 0
  run_audit v40 near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  run_audit v40 contact 1 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384} &
  wait
fi

if [[ "${RUN_NEAR_CHALLENGE_ABLATION:-0}" == "1" ]]; then
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_eval v40 near_contact 0
  RUN_NEAR_CHALLENGE=true RUN_NEAR_BRAKE_RESCUE=false run_audit v40 near_contact 0 ${AUDIT_TARGETS:-32} ${AUDIT_LABELS:-384}
fi

if [[ "${RUN_DEEP_CONTACT:-0}" == "1" ]]; then
  run_audit v40 contact 0 ${DEEP_CONTACT_TARGETS:-48} ${DEEP_CONTACT_LABELS:-640}
fi
