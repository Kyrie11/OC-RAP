...
# v44 OC-RAVA evaluation. The frozen OC-MERO backbone remains unchanged.
# A shared observation-only value head plus an opportunity gate may augment admission
# for verified merge/stabilize recoveries in Near/Contact; Safe remains nominal-locked.
export RUN=${RUN:-runs/ocrap_v44_rava_eval}
export BASE_RUN=${BASE_RUN:-runs/ocrap_v44_rava_regime_balanced}
if [[ -z "${CKPT:-}" ]]; then
  CKPT="$BASE_RUN/model_v44_rava/best.pt"
fi
if [[ -z "${CAL:-}" ]]; then
  CAL="$BASE_RUN/calibration/calibration_mix_v44.json"
fi
if [[ -z "${GAMMA:-}" ]]; then
  GAMMA="$BASE_RUN/calibration/gamma_rec_by_bucket_v44.json"
fi
...
# v44 reads both an opportunity threshold and a observation-conditioned score
...
if not d.get('valid_for_deployment', False) or not math.isfinite(s) or not math.isfinite(o):
    raise SystemExit(f'invalid OC-RAVA certificate: {p}')
print(s, o)
PYQ
}
if [[ -z "${NEAR_DIRECT_THRESHOLD:-}" || -z "${NEAR_DIRECT_OPPORTUNITY_THRESHOLD:-}" ]]; then
  read -r NEAR_DIRECT_THRESHOLD NEAR_DIRECT_OPPORTUNITY_THRESHOLD < <(read_direct_rule "$BASE_RUN/calibration/direct_value_risk_near_v44.json")
fi
if [[ -z "${CONTACT_DIRECT_THRESHOLD:-}" || -z "${CONTACT_DIRECT_OPPORTUNITY_THRESHOLD:-}" ]]; then
  read -r CONTACT_DIRECT_THRESHOLD CONTACT_DIRECT_OPPORTUNITY_THRESHOLD < <(read_direct_rule "$BASE_RUN/calibration/direct_value_risk_contact_v44.json")
fi
...
echo "direct-value OC-RAVA rules: near(score=$NEAR_DIRECT_THRESHOLD,opp=$NEAR_DIRECT_OPPORTUNITY_THRESHOLD) contact(score=$CONTACT_DIRECT_THRESHOLD,opp=$CONTACT_DIRECT_OPPORTUNITY_THRESHOLD)"

make_sel() {
  local tag="$1"  # scalar | v44
  ...
  case "$tag" in
    ...
    v44|v40|v27) rel_near=true; rel_contact=true; rec_pool=true; brake_rescue=${CONTACT_BRAKE_RESCUE:-false}; pcd_rescue=true; brake_tail=${CONTACT_BRAKE_TAIL:-false}; brake_tail_bypass_pcd=${CONTACT_TAIL_BYPASS_PCD_GAIN:-false}; direct_value=${RUN_DIRECT_VALUE:-true}; is_v27=true ;;
  ...
  COMMON_SEL=(
    ...
    # v44 OC-RAVA: a held-out-verified top-1 value certificate may augment
...
run_eval() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli evaluate \
    ...
    --output "$RUN/eval_${d}_v44_${tag}.json" \
...
run_audit() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/audit_${b}_selected_topk_v44_${tag}.json" \
    "${COMMON_SEL[@]}" \
    ...
    | tee -a "$RUN/audit_${b}_selected_topk_v44_${tag}.log"
  assert_json "$RUN/audit_${b}_selected_topk_v44_${tag}.json"
}
...
run_safe_closed_loop() {
  ...
  CUDA_VISIBLE_DEVICES=${GPU_SAFE:-0} PYTHONUNBUFFERED=1 python -u -m ocrap.cli closed-loop \
    ...
    --output "$RUN/closed_loop_safe_fast_v44.json" \
    "${COMMON_SEL[@]}" \
    --set closed_loop.method=ocrap \
    --set closed_loop.resume="$CL_RESUME" \
    --set closed_loop.resume_allow_legacy_partial=true \
    ...
    | tee -a "$RUN/closed_loop_safe_fast_v44.log"
  assert_json "$RUN/closed_loop_safe_fast_v44.json"
}

summarize() {
  python - <<'PY' "$RUN" | tee "$RUN/summary_all_v44.txt"
import json, pathlib, sys
root=pathlib.Path(sys.argv[1])
print("\n===== Offline eval v44 =====")
for p in sorted(root.glob('eval_*_v44_*.json')):
...
for p in sorted(root.glob('*v44*.json')):
...
if [[ "$RUN_OFFLINE_EVAL" == "1" ]]; then
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    run_eval scalar safe 0 & run_eval v44 safe 1 & wait
    run_eval scalar near_contact 0 & run_eval v44 near_contact 1 & wait
    run_eval scalar contact 0 & run_eval v44 contact 1 & wait
  else
    if [[ "${OFFLINE_SINGLE_GPU:-0}" == "1" ]]; then
      g=${GPU_SAFE:-0}
      run_eval v44 safe "$g"
      run_eval v44 near_contact "$g"
      run_eval v44 contact "$g"
    else
      run_eval v44 safe ${GPU_SAFE:-0} & run_eval v44 near_contact ${GPU_NEAR:-1} & wait
      run_eval v44 contact ${GPU_CONTACT:-0}
    fi
  fi
fi
  ...
  if [[ "${RUN_SCALAR_BASELINES:-1}" == "1" && "${PAIR_BUCKETS_ON_TWO_GPUS:-1}" == "1" ]]; then
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      (run_audit scalar near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v44 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_NEAR=$!
    fi
    if [[ "$RUN_CONTACT_AUDIT" == "1" ]]; then
      (run_audit scalar contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}; \
       run_audit v44 contact ${GPU_CONTACT:-1} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192}) &
      P_CONTACT=$!
    fi
    [[ -z "${P_NEAR:-}" ]] || wait "$P_NEAR"
    [[ -z "${P_CONTACT:-}" ]] || wait "$P_CONTACT"
  elif [[ "${RUN_SCALAR_BASELINES:-1}" == "1" ]]; then
    [[ "$RUN_NEAR_AUDIT" != "1" ]] || { run_audit scalar near_contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v44 near_contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
    [[ "$RUN_CONTACT_AUDIT" != "1" ]] || { run_audit scalar contact 0 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & run_audit v44 contact 1 ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} & wait; }
  else
    if [[ "$RUN_NEAR_AUDIT" == "1" ]]; then
      run_audit v44 near_contact ${GPU_NEAR:-0} ${AUDIT_TARGETS:-16} ${AUDIT_LABELS:-192} &
      P_NEAR=$!
