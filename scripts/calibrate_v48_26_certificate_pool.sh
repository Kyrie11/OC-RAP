...
# Finalise a v48.26 EXECUTION-PHYSICS-BRIDGE run using a scene-disjoint dedicated certificate pool.
...
: > "$OUTPUTDIR/logs/v48_26_certificate_status.log"
...
export OPPORTUNITY_LABEL_MODE="${OPPORTUNITY_LABEL_MODE:-safe_benefit}"
...
protocol={
 'version':'v48.26-EXECUTION-PHYSICS-BRIDGE',
...
doc={'event':'v48_26_gate_protocol_preregistered','created_unix':time.time(),
...
calibrate_variant() {
  ...
  for i in 0 1 2 3; do
    CUDA_VISIBLE_DEVICES="$gpu" python -u -m ocrap.cli calibrate \
      ...
      2>&1 | tee "$run/logs/v48_26_calibrate_${buckets[$i]}.log"
  done
  python tools/write_gamma_by_bucket.py \
    ...
    2>&1 | tee "$run/logs/v48_26_gamma.log"

  local common=(
    --checkpoint "$ckpt" --method-version=v48_26_execution_physics_bridge_dev_frozen_policy_risk_certificate --risk-source="${RISK_SOURCE:-ordinal_evidence}"
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    2>&1 | tee "$run/logs/v48_26_dev_rule_near.log"; local dn=${PIPESTATUS[0]}
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    2>&1 | tee "$run/logs/v48_26_dev_rule_contact.log"; local dc=${PIPESTATUS[0]}
  if [[ "$dn" != 0 || "$dc" != 0 ]]; then
    set -e
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    2>&1 | tee "$run/logs/v48_26_policy_near.log"; local sn=${PIPESTATUS[0]}
  CUDA_VISIBLE_DEVICES="$gpu" python -u tools/calibrate_policy_risk_v48.py \
    ...
    2>&1 | tee "$run/logs/v48_26_policy_contact.log"; local sc=${PIPESTATUS[0]}
  set -e
...
safe_status={
 'event':'v48_26_safe_regime_status','created_unix':time.time(),'variant':variant,
...
doc={'event':'v48_26_certificate_pool_calibration_complete','created_unix':time.time(),
...
PY
  ...
  mv "$tmp" "$final"
  rm -rf "$final.old"
  echo "$variant near=$sn contact=$sc" >> "$OUTPUTDIR/logs/v48_26_certificate_status.log"
...
status={'event':'v48_26_certificate_candidate_selection','created_unix':time.time(),
...
(root/'NEXT_COMMANDS.txt').write_text(
 ...
 f'''OUT={root} bash scripts/run_v48_26_stress_if_authorized.sh\n''')
print(json.dumps(status,ensure_ascii=False,indent=2))
print('chosen',chosen)
PY
