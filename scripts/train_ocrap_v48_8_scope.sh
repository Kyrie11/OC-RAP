...
# v48.8 OC-TRAC-SCOPE staged optimization.
# Stage P learns a conflict-free, nominal-inclusive set preference using only
# invariant relative context. Stage C freezes ranking and learns a robust
# candidate-minus-nominal gain mean; deployment confidence is calibrated by a
# held-out split-conformal residual bound rather than learned variance.
...
COMMON_ENV=(
  ...
  POLICY_METRIC_RANK_FALSE_WEIGHT="${POLICY_METRIC_RANK_FALSE_WEIGHT:-0.15}"
...
# v48.7 preference fit on only a few hundred positive groups.
env "${COMMON_ENV[@]}" \
  ...
  PREFERENCE_CONTEXT_HIDDEN="${PREFERENCE_CONTEXT_HIDDEN:-48}" \
  ...
  PREFERENCE_NOMINAL_MARGIN="${PREFERENCE_NOMINAL_MARGIN:-0.018}" \
  PREFERENCE_HARM_MARGIN="${PREFERENCE_HARM_MARGIN:-0.030}" \
  PREFERENCE_TIE_EPS_NEAR="${PREFERENCE_TIE_EPS_NEAR:-0.030}" \
...
# actual uncertainty is obtained from split-conformal residuals during proxy
# calibration, eliminating v48.7 variance collapse.
env "${COMMON_ENV[@]}" \
  ...
  DELTA_SIGN_WEIGHT="${CERTIFICATE_DELTA_SIGN_WEIGHT:-0.50}" \
...
doc={
  'event':'v48_8_scope_staged_training_complete','created_unix':time.time(),
