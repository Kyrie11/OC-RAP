...
This repository is the cleaned engineering workspace for the current V48.122 OC-SVRT audit plus the
publication-facing evaluation utilities used by the deployed OC-RAP stack.

## What is active

- `scripts/run_constraint_native_orientation_audit.sh`: the stable semantic scientific launcher. It currently runs V48.122 OC-SVRT.
  V48.122 is an **audit-only** equal-capacity signed viability rank-state transport experiment. It retains candidate-independent nominal recovery ordering and same-option candidate causal correspondence, and adds only the absolute signed nominal viability level relative to the physical zero boundary as the preregistered missing primitive. Exposed-support and full-set families remain matched at 220-D. It does not train or mutate the frozen OC-RAP planner.
...
The current launcher consumes versioned frozen scientific assets by design, including the authoritative V48.121 STOP result plus V48.80/V48.93/V48.96 checkpoint/evidence assets. These are provenance inputs, not old-code dependencies. Source/tool/test filenames remain semantic and unversioned.
